...
using namespace std;
//
// NomJIT::NomJIT() :  TM(EngineBuilder().selectTarget()),
// DL(TM->createDataLayout()), ObjectLayer([]() { return
// std::make_shared<SectionMemoryManager>(); }), CompileLayer(ObjectLayer,
// SimpleCompiler(*TM)) {
...
public:
  ...
  virtual void
  notifyObjectLoaded(ObjectKey K, const object::ObjectFile &Obj,
...
const llvm::DataLayout &GetNomJITDataLayout() {
  return NomJIT::Instance().getDataLayout();
}
...
NomJIT::NomJIT(std::unique_ptr<LLJIT> &&llj)
  ...
  // llvm::sys::DynamicLibrary::LoadLibraryPermanently(nullptr);
  llvm::sys::DynamicLibrary::AddSymbol("CPP_NOM_GCALLOC",
                                       (void *)&CPP_NOM_GCALLOC);
  llvm::sys::DynamicLibrary::AddSymbol("CPP_NOM_Print", (void *)&CPP_NOM_Print);
  llvm::sys::DynamicLibrary::AddSymbol("CPP_NOM_NEWALLOC",
                                       (void *)&CPP_NOM_NEWALLOC);
  llvm::sys::DynamicLibrary::AddSymbol("CPP_NOM_RECORDALLOC",
                                       (void *)&CPP_NOM_RECORDALLOC);
  llvm::sys::DynamicLibrary::AddSymbol("CPP_NOM_CLOSUREALLOC",
                                       (void *)&CPP_NOM_CLOSUREALLOC);
...
llvm::Expected<std::unique_ptr<NomJIT>> NomJIT::Create() {
  ...
  JITbuilder.CreateObjectLinkingLayer =
    ...
    auto oll = std::make_unique<llvm::orc::RTDyldObjectLinkingLayer>(
...
NomJIT::optimizeModule(ThreadSafeModule TSM,
                       const MaterializationResponsibility &R) {
  // std::unique_ptr<Module>
  // Nom::Runtime::NomJIT::optimizeModule(std::unique_ptr<Module> M) {
  ...
  TSM.withModuleDo([](Module &M) {
    ...
    if (NomFunctionTimingLevel > 2) {
      ...
      for (auto &F : M) {
        ...
        auto funnameid =
            NomNameRepository::ProfilingInstance().GetNameID(*fname);
        ...
        for (auto &b : F) {
          if (&b == &entryBlock) {
            for (auto &i : b) {
              ...
              builder.CreateCall(
                  GetEnterFunctionFunction(M),
                  llvm::ArrayRef<llvm::Value *>{MakeInt<size_t>(funnameid)});
              break;
            }
          }
    ...
    if (NomDebugPrintLevel >= 0 || NomStatsLevel > 1) {
      ...
      // std::forward_list<std::string>* strlist = new
      ...
      for (auto &F : M) {
        ...
        for (auto &b : F) {
          for (auto &i : b) {
            builder.SetInsertPoint(&i);
            // builder.CreateCall(dbgfun, { GetLLVMPointer(&strlist->front()),
            // ConstantInt::get(inttype, 0),
            // MakeInt<decltype(NomDebugPrintLevel)>(2) });
            builder.CreateCall(dbgfun,
                               {MakeInt<size_t>(funnameid), MakeInt<size_t>(0),
                                MakeInt<NomDebugPrintValueType>(
                                    NomDebugPrintValueType::Nothing),
                                MakeInt<int64_t>(0),
                                MakeInt<decltype(NomDebugPrintLevel)>(2)});
            break;
          }
          ...
          for (auto &i : b) {
            ...
            if (lastInstruction != nullptr) {
              ...
              if (type->isPointerTy()) {
                ...
                // builder.CreateCall(dbgfunhex, { GetLLVMPointer(&label),
                // builder.CreatePtrToInt(lastInstruction, inttype),
                // MakeInt<decltype(NomDebugPrintLevel)>(isDebugFun ? 1 : 3) });
              } else if (type->isIntegerTy(1)) {
                debugValue =
                    builder.CreateSExtOrTrunc(lastInstruction, inttype);
                valueType = NomDebugPrintValueType::Bool;
                // static std::string label = "Bool ";
                // builder.CreateCall(dbgfun, { GetLLVMPointer(&label),
                // builder.CreateSExtOrTrunc(lastInstruction, inttype),
                // MakeInt<decltype(NomDebugPrintLevel)>(isDebugFun ? 1 : 3) });
                ...
                // builder.CreateCall(dbgfun, { GetLLVMPointer(&label),
                // builder.CreateFPToSI(lastInstruction, inttype),
                // MakeInt<decltype(NomDebugPrintLevel)>(isDebugFun ? 1 : 3) });
              }
              // builder.CreateCall(dbgfun, { GetLLVMPointer(&strlist->front()),
              // ConstantInt::get(inttype, pos++),
              // MakeInt<decltype(NomDebugPrintLevel)>(isDebugFun ? 1 : 3) });
            }
            builder.CreateCall(
                dbgfun,
                {MakeInt<size_t>(funnameid), MakeInt<size_t>(pos++),
                 MakeInt<NomDebugPrintValueType>(valueType), debugValue,
                 MakeInt<decltype(NomDebugPrintLevel)>(isDebugFun ? 1 : 3)});
            lastInstruction = &i;
          }
        }
...
void *GetVoidObject() {
  static void *voidObj =
      (void *)((intptr_t)NomJIT::Instance().getSymbolAddress("RT_NOM_VOIDOBJ"));
  return voidObj;
}
} // namespace Runtime
} // namespace Nom
