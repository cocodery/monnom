...
public:
  NomJIT(
      std::unique_ptr<llvm::orc::LLJIT> &&
          llj /*llvm::orc::JITTargetMachineBuilder JTMB, llvm::DataLayout DL*/);
  ...
  // llvm::orc::IRTransformLayer<decltype(CompileLayer), OptimizeFunction>
  // OptimizeLayer;
public:
  static NomJIT &Instance() {
    static std::unique_ptr<NomJIT> instance(std::move(*Create()));
    return *instance;
  }
  // using ModuleHandle = llvm::orc::VModuleKey;
  // //decltype(CompileLayer)::ModuleHandleT;
  ...
  //{
  //	return (void *) llvm::cantFail(OptimizeLayer.findSymbolIn(h, Name,
  // true).getAddress());
  // }
...
private:
  ...
  // std::unique_ptr<llvm::Module> optimizeModule(std::unique_ptr<llvm::Module>
  // M);
};
