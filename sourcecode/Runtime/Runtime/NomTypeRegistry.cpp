...
llvm::Function *
TypeRegistry::createLLVMElement(llvm::Module &mod,
                                llvm::GlobalValue::LinkageTypes linkage) const {
  FunctionType *funtype = GetLLVMFunctionType();
  Function *fun =
      Function::Create(funtype, linkage, "RT_NOM_EnsureUniquedType", &mod);
  ...
  auto stackPHI = builder->CreatePHI(
      RTSubtyping::TypeArgumentListStackType()->getPointerTo(), 2,
      "substStack");
...

  {
    ...
    typePHI->addIncoming(
        MakeLoad(builder, mod,
                 builder->CreateGEP(
                     targsArr, builder->CreateSub(MakeInt32(-1), varIndex))),
        varBlock);
    stackPHI->addIncoming(
        MakeLoad(builder, mod,
                 builder->CreateGEP(
                     stackPHI, {MakeInt32(0),
  ...
  {
    BasicBlock *uniqueTypeArgsBlock =
        BasicBlock::Create(LLVMCONTEXT, "uniqueTypeArgs", fun);
    BasicBlock *uniqueTypeArgsLoopBlock =
        BasicBlock::Create(LLVMCONTEXT, "uniqueTypeArgsLoop", fun);
    BasicBlock *uniqueTypeArgsDoneBlock =
        BasicBlock::Create(LLVMCONTEXT, "uniqueTypeArgsDone", fun);
    BasicBlock *newInstanceBlock =
        BasicBlock::Create(LLVMCONTEXT, "needNewInstance", fun);
    ...
    auto newTypeArgsArr = builder->CreateGEP(
        builder->CreateAlloca(TYPETYPE, targcount), targcount);
    auto hashArr = builder->CreateGEP(
        builder->CreateAlloca(numtype(size_t), targcount), targcount);
    ...
    auto currentTypeArg =
        MakeLoad(builder, builder->CreateGEP(typeArgsArr, typeArgIndexPHI));
    auto uniquedTypeArg = builder->CreateCall(fun, {currentTypeArg, stackPHI});
    uniquedTypeArg->setCallingConv(fun->getCallingConv());

    MakeStore(builder, uniquedTypeArg,
              builder->CreateGEP(newTypeArgsArr, typeArgIndexPHI));
    auto uniquedHash =
        RTTypeHead::GenerateReadTypeHash(builder, uniquedTypeArg);
    MakeStore(builder, uniquedHash,
              builder->CreateGEP(hashArr, typeArgIndexPHI));

    auto newFoundDifference = builder->CreateOr(
        foundDifference, builder->CreateNot(CreatePointerEq(
                             builder, uniquedTypeArg, currentTypeArg)));
    foundDifference->addIncoming(newFoundDifference, builder->GetInsertBlock());
    typeArgIndexPHI->addIncoming(
        builder->CreateAdd(typeArgIndexPHI, MakeInt32(1)),
        builder->GetInsertBlock());
    auto typeArgsDone = builder->CreateICmpEQ(typeArgIndexPHI, MakeInt32(-1));
    builder->CreateCondBr(typeArgsDone, uniqueTypeArgsDoneBlock,
                          uniqueTypeArgsLoopBlock);
    ...
    static auto uniqueInstantiationFun =
        NomInterface::GetGetUniqueInstantiationFunction(mod);
    if (NomCastStats) {
      builder->CreateCall(
          GetIncTypeInstanceAlloactionsFunction(*fun->getParent()), {});
    }
    auto uniquedResult = builder->CreateCall(
        uniqueInstantiationFun, {RTInterface::GenerateReadIRPtr(builder, iface),
                                 iface, newTypeArgsArr, hashArr, targcount});
...

  {
    ...
    auto iface =
        RTInstanceType::GenerateReadClassDescriptorLink(builder, typePHI);
    auto targcount = RTInterface::GenerateReadTypeArgCount(builder, iface);
    auto noTargs = builder->CreateICmpEQ(targcount, MakeIntLike(targcount, 0));
    builder->CreateCondBr(noTargs, idBlock, uniqueTypeArgsBlock);
    ...
    builder->CreateCondBr(typeArgsDone, newInstanceBlock,
                          uniqueTypeArgsLoopBlock);
  ...
  builder
      ->CreateCall(RTOutput_Fail::GetLLVMElement(mod),
                   GetLLVMPointer(invalidKind_errorMessage))
      ->setCallingConv(RTOutput_Fail::GetLLVMElement(mod)->getCallingConv());
