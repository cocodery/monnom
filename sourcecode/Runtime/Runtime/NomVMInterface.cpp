...
llvm::Function *GetAlloc(llvm::Module *mod) {
  ...
  if (ret == nullptr) {
    FunctionType *allocFunType =
        FunctionType::get(POINTERTYPE, {INTTYPE}, false);
    ret = Function::Create(allocFunType, Function::ExternalLinkage,
                           "CPP_NOM_GCALLOC", mod);
  }
  return ret;
}
...
llvm::Function *GetNewAlloc(llvm::Module *mod) {
  ...
  if (ret == nullptr) {
    FunctionType *allocFunType =
        FunctionType::get(REFTYPE, {INTTYPE, INTTYPE}, false);
    ret = Function::Create(allocFunType, Function::ExternalLinkage,
                           "CPP_NOM_NEWALLOC", mod);
  }
  return ret;
}
...
llvm::Function *GetClosureAlloc(llvm::Module *mod) {
  ...
  if (ret == nullptr) {
    FunctionType *allocFunType = FunctionType::get(
        LambdaHeader::GetLLVMType()->getPointerTo(), {INTTYPE, INTTYPE}, false);
    ret = Function::Create(allocFunType, Function::ExternalLinkage,
                           "CPP_NOM_CLOSUREALLOC", mod);
  }
  return ret;
}
...
llvm::Function *GetRecordAlloc(llvm::Module *mod) {
  ...
  if (ret == nullptr) {
    FunctionType *allocFunType = FunctionType::get(
        RecordHeader::GetLLVMType()->getPointerTo(), {INTTYPE, INTTYPE}, false);
    ret = Function::Create(allocFunType, Function::ExternalLinkage,
                           "CPP_NOM_RECORDALLOC", mod);
  }
  return ret;
}
...
llvm::Function *GetClassTypeAlloc(llvm::Module *mod) {
  ...
  if (ret == nullptr) {
    FunctionType *allocFunType = FunctionType::get(
        RTClassType::GetLLVMType()->getPointerTo(), {INTTYPE}, false);
    ret = Function::Create(allocFunType, Function::ExternalLinkage,
                           "CPP_NOM_CLASSTYPEALLOC", mod);
  }
  return ret;
}

void GenerateLLVMDebugPrint(IRBuilder<> &builder, llvm::Module *mod,
...
extern "C" DLLEXPORT void *CPP_NOM_NEWALLOC(size_t numfields, size_t numtargs) {
  ...
  auto ret =
      (void *)(((char **)gcalloc(ObjectHeader::SizeOf() +
                                 (sizeof(char *) * (numfields + numtargs)))) +
               numtargs);
  return ret;
}
...
extern "C" DLLEXPORT void *CPP_NOM_CLOSUREALLOC(size_t numfields,
  ...
  auto ret =
      (void *)(((char **)gcalloc(LambdaHeader::SizeOf() +
                                 (sizeof(char *) * (numfields + numtargs)))) +
               numtargs);
  return ret;
}
...
extern "C" DLLEXPORT void *CPP_NOM_RECORDALLOC(size_t numfields,
  ...
  auto ret =
      (void *)(((char **)gcalloc(RecordHeader::SizeOf() +
                                 (sizeof(char *) * (numfields + numtargs)))) +
               numtargs);
  // auto retDictRoot = (void*)(((char*)(((intptr_t)ret))) +
  // RecordHeader::GetLLVMLayout()->getElementOffset((unsigned
  // int)StructHeaderFields::InstanceDictionary));
...
extern "C" DLLEXPORT void *CPP_NOM_CLASSTYPEALLOC(size_t numtargs) {
  ...
  auto ret = (void *)(((char **)gcalloc(
                          GetNomJITDataLayout()
                              .getTypeAllocSize(RTClassType::GetLLVMType())
                              .getFixedSize() +
                          (sizeof(char *) * (numtargs)))) +
                      numtargs);
  return ret;
}
