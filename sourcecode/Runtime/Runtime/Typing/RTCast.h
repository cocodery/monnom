...
public:
  ...
  static llvm::Value *GenerateMonotonicCast(NomBuilder &builder,
                                            CompileEnv *env, NomValue &value,
                                            NomClassTypeRef type);
  static llvm::Value *GenerateMonotonicCast(NomBuilder &builder,
                                            CompileEnv *env, NomValue &value,
                                            llvm::Value *type);
  static uint64_t nextCastSiteID();
  static llvm::Value *GenerateCast(NomBuilder &builder, CompileEnv *env,
...
public:
  static FailingAdjustFun *GetInstance() {
    static FailingAdjustFun instance;
    return &instance;
  }
  ~FailingAdjustFun() {};
