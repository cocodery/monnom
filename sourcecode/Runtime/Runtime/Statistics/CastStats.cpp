...
DLLEXPORT void RT_NOM_STATS_IncCasts() { castCount++; }

DLLEXPORT void RT_NOM_STATS_IncMonotonicCasts() { monoCastCount++; }

DLLEXPORT void RT_NOM_STATS_IncSubtypingChecks() { subtypingChecksCount++; }
...
DLLEXPORT void RT_NOM_STATS_IncImpositions() { impositionsCount++; }

DLLEXPORT void RT_NOM_STATS_IncIntPacks() { intPacksCount++; }
DLLEXPORT void RT_NOM_STATS_IncIntUnpacks() { intUnpacksCount++; }
DLLEXPORT void RT_NOM_STATS_IncIntBoxes() { intBoxesCount++; }
DLLEXPORT void RT_NOM_STATS_IncIntUnboxes() { intUnboxesCount++; }
DLLEXPORT void RT_NOM_STATS_IncFloatPacks() { floatPacksCount++; }
DLLEXPORT void RT_NOM_STATS_IncFloatUnpacks() { floatUnpacksCount++; }
DLLEXPORT void RT_NOM_STATS_IncFloatBoxes() { floatBoxesCount++; }
DLLEXPORT void RT_NOM_STATS_IncFloatUnboxes() { floatUnboxesCount++; }
...
DLLEXPORT void RT_NOM_STATS_IncCastTime(TIMERTYPE origTimerVal) {
  ...
  if (QueryPerformanceCounter(&current)) {
    ...
    // double floatdiff = difference.QuadPart /
    // Nom::Runtime::find_timer_frequency(); printf("%f Seconds\n", floatdiff);
    // return
    // (void*)((intptr_t)(Nom::Runtime::NomJIT::Instance().getSymbolAddress("RT_NOM_VOIDOBJ")));
  } else {
    std::cout << "ERROR obtaining performance counter value";
    // throw new std::exception();
  ...
  if (clock_gettime(CLOCK_THREAD_CPUTIME_ID, &current) == 0) {
    timeUnitsInCasts += ((current.tv_sec - origTimerVal.tv_sec) * 10000000) +
                        ((current.tv_nsec - origTimerVal.tv_nsec) / 100);
...
DLLEXPORT void RT_NOM_STATS_IncTypedRawInvokes() { typedRawInvokes++; }
DLLEXPORT void RT_NOM_STATS_IncFinalInstanceMethodCalls() {
  finalInstanceMethodCalls++;
}
DLLEXPORT void RT_NOM_STATS_IncStaticMethodCalls() { staticMethodCalls++; }
DLLEXPORT void RT_NOM_STATS_IncDynamicInvokes() { dynamicInvokes++; }
DLLEXPORT void RT_NOM_STATS_IncDynamicMethodCalls() { dynamicMethodCalls++; }
DLLEXPORT void RT_NOM_STATS_IncDynamicFieldLookups() { dynamicFieldLookups++; }
...
public:
  ProgramPos() {}
  ProgramPos(size_t funname, size_t linenum, double seconds)
      : FunName(funname), LineNum(linenum), Seconds(seconds) {}
  size_t FunName;
  size_t LineNum;
  double Seconds;
};
...
DLLEXPORT void RT_NOM_STATS_DebugLine(size_t funnameid, size_t linenum,
                                      NomDebugPrintValueType valueType,
                                      int64_t value,
                                      decltype(NomDebugPrintLevel) level) {
  ...
  if (NomStatsLevel > 3) {
    ...
    if (timestampcount > 0) {
      ...
      if (difference > avgtime * 2 && timestampcount > 10) {
        double seconds =
            ((double)(difference - avgtime)) / find_timer_frequency();
        if (seconds > slowestSoFar || slowPositions.size() < 100) {
          ...
          for (; iter != slowPositions.end(); iter++) {
            ...
            if (pp.Seconds < seconds && !found) {
              ...
              outstream << "SLOW: " << (*debugFunNames[funnameid]) << ":"
                        << linenum << " - " << seconds << "s\n";
              found = true;
              break;
            }
...
void InitializeProfileCounter() {
  llvm::sys::DynamicLibrary::AddSymbol("RT_NOM_STATS_DebugLine",
                                       (void *)&RT_NOM_STATS_DebugLine);
  auto maxid = NomNameRepository::ProfilingInstance().GetMaxID();
  debugFunNames =
      (const std::string **)nmalloc(sizeof(std::string *) * (maxid + 1));
...
void PrintCastStats() {
  ...
  outstream << "\n#Type-Arg Recursions: " << std::dec
            << typeArgumentRecursionsCount;
  outstream << "\n#Type Instance Allocations: " << std::dec
            << typeInstanceAllocationsCount;
  outstream << "\n#Perfect Call Tag Matches: " << std::dec
            << perfectCallTagTypeMatches;
  ...
  outstream << "\n#Class Type Allocations: " << std::dec
            << classtype_allocations;
  outstream << "\n#Total Allocations: " << std::dec
            << (general_allocations + object_allocations + closure_allocations +
                record_allocations + classtype_allocations);
  outstream << "\n\nCall/Lookup Stats:\n----------------------";
  outstream << "\n#Direct Class Method Calls: " << std::dec
            << directClassMethodCalls;
  outstream << "\n#Interface Method Calls: " << std::dec
            << (interfaceMethodCalls + extendedInterfaceMethodCalls);
  outstream << "\n  #Interface Method Calls with Extended Argument Arrays: "
            << std::dec << extendedInterfaceMethodCalls;
  outstream << "\n#Typed Raw Invocations: " << std::dec << typedRawInvokes;
  outstream << "\n#Final Instance Method Calls: " << std::dec
            << finalInstanceMethodCalls;
...
#endif
#endif
  outstream << "\nTime spent in casts: " << std::dec << castSeconds
            << " Seconds";
  ...
  if (NomStatsLevel > 1) {
    ...
    auto maxid = NomNameRepository::ProfilingInstance().GetMaxID();
    for (decltype(maxid) i = 1; i <= maxid; i++) {
      outstream << (*debugFunNames[i]) << ": " << std::dec
                << profileCounterArray[i] << "\n";
    }

    if (NomStatsLevel > 3) {
      outstream << "\n\nSlow Timings:\n----------------------\n\n";
      for (auto pp : slowPositions) {
        outstream << (*debugFunNames[pp.FunName]) << ":" << pp.LineNum << " - "
                  << pp.Seconds << "s\n";
      }
    }
...
llvm::Function *GetIncCastsFunction(llvm::Module &mod) {
  ...
  if (fun == nullptr) {
    fun =
        Function::Create(FunctionType::get(Type::getVoidTy(LLVMCONTEXT), false),
                         GlobalValue::LinkageTypes::ExternalLinkage,
                         "RT_NOM_STATS_IncCasts", &mod);
  }
  return fun;
}
llvm::Function *GetIncMonotonicCastsFunction(llvm::Module &mod) {
  ...
  if (fun == nullptr) {
    fun =
        Function::Create(FunctionType::get(Type::getVoidTy(LLVMCONTEXT), false),
                         GlobalValue::LinkageTypes::ExternalLinkage,
                         "RT_NOM_STATS_IncMonotonicCasts", &mod);
  }
  return fun;
}
llvm::Function *GetIncSubtypingChecksFunction(llvm::Module &mod) {
  ...
  if (fun == nullptr) {
    fun =
        Function::Create(FunctionType::get(Type::getVoidTy(LLVMCONTEXT), false),
                         GlobalValue::LinkageTypes::ExternalLinkage,
                         "RT_NOM_STATS_IncSubtypingChecks", &mod);
  }
  return fun;
}
llvm::Function *GetIncTypeArgumentRecursionsFunction(llvm::Module &mod) {
  ...
  if (fun == nullptr) {
    fun =
        Function::Create(FunctionType::get(Type::getVoidTy(LLVMCONTEXT), false),
                         GlobalValue::LinkageTypes::ExternalLinkage,
                         "RT_NOM_STATS_IncTypeArgumentRecursions", &mod);
  }
  return fun;
}
llvm::Function *GetIncTypeInstanceAlloactionsFunction(llvm::Module &mod) {
  ...
  if (fun == nullptr) {
    fun =
        Function::Create(FunctionType::get(Type::getVoidTy(LLVMCONTEXT), false),
                         GlobalValue::LinkageTypes::ExternalLinkage,
                         "RT_NOM_STATS_IncTypeInstanceAlloactions", &mod);
  }
  return fun;
}
llvm::Function *GetIncPerfectCallTagTypeMatchesFunction(llvm::Module &mod) {
  ...
  if (fun == nullptr) {
    fun =
        Function::Create(FunctionType::get(Type::getVoidTy(LLVMCONTEXT), false),
                         GlobalValue::LinkageTypes::ExternalLinkage,
                         "RT_NOM_STATS_IncPerfectCallTagTypeMatches", &mod);
  }
  return fun;
}
llvm::Function *GetIncCallTagTypeMismatchesFunction(llvm::Module &mod) {
  ...
  if (fun == nullptr) {
    fun =
        Function::Create(FunctionType::get(Type::getVoidTy(LLVMCONTEXT), false),
                         GlobalValue::LinkageTypes::ExternalLinkage,
                         "RT_NOM_STATS_IncCallTagTypeMismatches", &mod);
  }
  return fun;
}
llvm::Function *GetIncImpositionsFunction(llvm::Module &mod) {
  ...
  if (fun == nullptr) {
    fun =
        Function::Create(FunctionType::get(Type::getVoidTy(LLVMCONTEXT), false),
                         GlobalValue::LinkageTypes::ExternalLinkage,
                         "RT_NOM_STATS_IncImpositions", &mod);
  }
  return fun;
}
...
llvm::Function *GetIncIntUnpacksFunction(llvm::Module &mod) {
  ...
  if (fun == nullptr) {
    fun =
        Function::Create(FunctionType::get(Type::getVoidTy(LLVMCONTEXT), false),
                         GlobalValue::LinkageTypes::ExternalLinkage,
                         "RT_NOM_STATS_IncIntUnpacks", &mod);
  }
  return fun;
}
...
llvm::Function *GetIncIntUnboxesFunction(llvm::Module &mod) {
  ...
  if (fun == nullptr) {
    fun =
        Function::Create(FunctionType::get(Type::getVoidTy(LLVMCONTEXT), false),
                         GlobalValue::LinkageTypes::ExternalLinkage,
                         "RT_NOM_STATS_IncIntUnboxes", &mod);
  }
  return fun;
}
llvm::Function *GetIncFloatPacksFunction(llvm::Module &mod) {
  ...
  if (fun == nullptr) {
    fun =
        Function::Create(FunctionType::get(Type::getVoidTy(LLVMCONTEXT), false),
                         GlobalValue::LinkageTypes::ExternalLinkage,
                         "RT_NOM_STATS_IncFloatPacks", &mod);
  }
  return fun;
}
llvm::Function *GetIncFloatUnpacksFunction(llvm::Module &mod) {
  ...
  if (fun == nullptr) {
    fun =
        Function::Create(FunctionType::get(Type::getVoidTy(LLVMCONTEXT), false),
                         GlobalValue::LinkageTypes::ExternalLinkage,
                         "RT_NOM_STATS_IncFloatUnpacks", &mod);
  }
  return fun;
}
llvm::Function *GetIncFloatBoxesFunction(llvm::Module &mod) {
  ...
  if (fun == nullptr) {
    fun =
        Function::Create(FunctionType::get(Type::getVoidTy(LLVMCONTEXT), false),
                         GlobalValue::LinkageTypes::ExternalLinkage,
                         "RT_NOM_STATS_IncFloatBoxes", &mod);
  }
  return fun;
}
llvm::Function *GetIncFloatUnboxesFunction(llvm::Module &mod) {
  ...
  if (fun == nullptr) {
    fun =
        Function::Create(FunctionType::get(Type::getVoidTy(LLVMCONTEXT), false),
                         GlobalValue::LinkageTypes::ExternalLinkage,
                         "RT_NOM_STATS_IncFloatUnboxes", &mod);
  }
  return fun;
}
llvm::Function *GetGetTimestampFunction(llvm::Module &mod) {
  ...
  if (fun == nullptr) {
    fun = Function::Create(FunctionType::get(inttype(bitsin(TIMERTYPE)), false),
                           GlobalValue::LinkageTypes::ExternalLinkage,
                           "RT_NOM_STATS_GetTimestamp", &mod);
  }
  return fun;
}
  ...
  if (fun == nullptr) {
    fun =
        Function::Create(FunctionType::get(Type::getVoidTy(LLVMCONTEXT),
                                           {inttype(bitsin(TIMERTYPE))}, false),
                         GlobalValue::LinkageTypes::ExternalLinkage,
                         "RT_NOM_STATS_IncCastTime", &mod);
  }
  return fun;
}
...
llvm::Function *GetDebugFunction(llvm::Module &mod) {
  ...
  if (fun == nullptr) {
    fun = Function::Create(
        FunctionType::get(Type::getVoidTy(LLVMCONTEXT),
                          {numtype(size_t), numtype(size_t),
                           numtype(NomDebugPrintValueType), numtype(int64_t),
                           numtype(decltype(NomDebugPrintLevel))},
                          false),
        GlobalValue::LinkageTypes::ExternalLinkage, "RT_NOM_STATS_DebugLine",
        &mod);
  }
  return fun;
}
llvm::Function *GetIncAllocationsFunction(llvm::Module &mod) {
  ...
  if (fun == nullptr) {
    fun = Function::Create(FunctionType::get(Type::getVoidTy(LLVMCONTEXT),
                                             {inttype(bitsin(AllocationType))},
                                             false),
                           GlobalValue::LinkageTypes::ExternalLinkage,
                           "RT_NOM_STATS_IncAllocations", &mod);
  }
  return fun;
}
...
llvm::Function *GetIncDirectClassMethodCalls(llvm::Module &mod) {
  ...
  if (fun == nullptr) {
    fun =
        Function::Create(FunctionType::get(Type::getVoidTy(LLVMCONTEXT), false),
                         GlobalValue::LinkageTypes::ExternalLinkage,
                         "RT_NOM_STATS_IncDirectClassMethodCalls", &mod);
  }
  return fun;
}
llvm::Function *GetIncInterfaceMethodCalls(llvm::Module &mod) {
  ...
  if (fun == nullptr) {
    fun =
        Function::Create(FunctionType::get(Type::getVoidTy(LLVMCONTEXT), false),
                         GlobalValue::LinkageTypes::ExternalLinkage,
                         "RT_NOM_STATS_IncInterfaceMethodCalls", &mod);
  }
  return fun;
}
llvm::Function *GetIncExtendedInterfaceMethodCalls(llvm::Module &mod) {
  ...
  if (fun == nullptr) {
    fun =
        Function::Create(FunctionType::get(Type::getVoidTy(LLVMCONTEXT), false),
                         GlobalValue::LinkageTypes::ExternalLinkage,
                         "RT_NOM_STATS_IncExtendedInterfaceMethodCalls", &mod);
  }
  return fun;
}
llvm::Function *GetIncTypedRawInvokes(llvm::Module &mod) {
  ...
  if (fun == nullptr) {
    fun =
        Function::Create(FunctionType::get(Type::getVoidTy(LLVMCONTEXT), false),
                         GlobalValue::LinkageTypes::ExternalLinkage,
                         "RT_NOM_STATS_IncTypedRawInvokes", &mod);
  }
  return fun;
}
llvm::Function *GetIncFinalInstanceMethodCalls(llvm::Module &mod) {
  ...
  if (fun == nullptr) {
    fun =
        Function::Create(FunctionType::get(Type::getVoidTy(LLVMCONTEXT), false),
                         GlobalValue::LinkageTypes::ExternalLinkage,
                         "RT_NOM_STATS_IncFinalInstanceMethodCalls", &mod);
  }
  return fun;
}
llvm::Function *GetIncStaticMethodCalls(llvm::Module &mod) {
  ...
  if (fun == nullptr) {
    fun =
        Function::Create(FunctionType::get(Type::getVoidTy(LLVMCONTEXT), false),
                         GlobalValue::LinkageTypes::ExternalLinkage,
                         "RT_NOM_STATS_IncStaticMethodCalls", &mod);
  }
  return fun;
}
llvm::Function *GetIncDynamicInvokes(llvm::Module &mod) {
  ...
  if (fun == nullptr) {
    fun =
        Function::Create(FunctionType::get(Type::getVoidTy(LLVMCONTEXT), false),
                         GlobalValue::LinkageTypes::ExternalLinkage,
                         "RT_NOM_STATS_IncDynamicInvokes", &mod);
  }
  return fun;
}
llvm::Function *GetIncDynamicMethodCalls(llvm::Module &mod) {
  ...
  if (fun == nullptr) {
    fun =
        Function::Create(FunctionType::get(Type::getVoidTy(LLVMCONTEXT), false),
                         GlobalValue::LinkageTypes::ExternalLinkage,
                         "RT_NOM_STATS_IncDynamicMethodCalls", &mod);
  }
  return fun;
}
llvm::Function *GetIncDynamicFieldLookups(llvm::Module &mod) {
  ...
  if (fun == nullptr) {
    fun =
        Function::Create(FunctionType::get(Type::getVoidTy(LLVMCONTEXT), false),
                         GlobalValue::LinkageTypes::ExternalLinkage,
                         "RT_NOM_STATS_IncDynamicFieldLookups", &mod);
  }
  return fun;
}
} // namespace Runtime
