...
FunctionTiming *timingAlloc() { return ftbuffer++; }
void freeTimingAlloc() { ftbuffer--; }
...
DLLEXPORT void NOM_RT_FT_EnterFunction(size_t funid) {
  ...
  currentFunction->AccTime.QuadPart +=
      timestamp.QuadPart - ft.LastStartTime.QuadPart;
#endif
  currentFunction = &ft;
}
DLLEXPORT void NOM_RT_FT_LeaveFunction() {
  ...
  LONGLONG difference =
      currentFunction->AccTime.QuadPart +
      (timestamp.QuadPart - currentFunction->LastStartTime.QuadPart);
...
llvm::Function *GetEnterFunctionFunction(llvm::Module &mod) {
  ...
  if (fun == nullptr) {
    fun = Function::Create(FunctionType::get(Type::getVoidTy(LLVMCONTEXT),
                                             {numtype(size_t)}, false),
                           GlobalValue::LinkageTypes::ExternalLinkage,
                           "NOM_RT_FT_EnterFunction", &mod);
  }
  return fun;
}
...
void InitFunctionTimings() {
  ...
  for (decltype(maxid) i = 1; i <= maxid; i++) {
    ftdebugFunNames[i] =
        NomNameRepository::ProfilingInstance().GetNameFromID(i);
#ifdef _WIN32
...
void PrintFunctionTimings() {
  ...
  for (decltype(maxid) i = 1; i <= maxid; i++) {
    ...
    if (functionTimingsArray[i] > 0) {
      std::cout << (*ftdebugFunNames[i]) << ": " << std::fixed
                << (((double)functionTimingsArray[i]) / frequency)
                << " seconds\n";
    }
#endif
  }
