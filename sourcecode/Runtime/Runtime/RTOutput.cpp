...
llvm::Function *RTOutput_Fail::GetLLVMElement(llvm::Module &mod) {
  ...
  if (fun == nullptr) {
    fun = Function::Create(GetFunctionType(),
...
llvm::BasicBlock *
RTOutput_Fail::GenerateFailUnimplementedBlock(NomBuilder &builder) {
  static const char *msg = "UNIMPLEMENTED!";
  return RTOutput_Fail::GenerateFailOutputBlock(builder, msg);
}
llvm::BasicBlock *
RTOutput_Fail::GenerateFailOutputBlock(NomBuilder &builder,
                                       const char *errorMessage) {
  BasicBlock *origBlock = builder->GetInsertBlock();

  BasicBlock *errorBlock =
      BasicBlock::Create(LLVMCONTEXT, "errorBlock", origBlock->getParent());
...
void RTOutput_Fail::MakeBlockFailOutputBlock(NomBuilder &builder,
                                             const char *errorMessage,
  ...
  builder->CreateCall(errorFun, GetLLVMPointer(errorMessage))
      ->setCallingConv(errorFun->getCallingConv());
...
llvm::FunctionType *RTOutput_Name::GetFunctionType() {
  static auto ft = FunctionType::get(Type::getVoidTy(LLVMCONTEXT),
                                     {POINTERTYPE, numtype(size_t)}, false);
  return ft;
}
