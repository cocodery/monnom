...
public:
  ...
  LibraryVersion(const std::string *name, const std::string &mainclassname,
                 const VersionNumber &version,
                 const VersionNumber &compatibleFrom,
                 const VersionNumber &compatibleTo, bool isDeprecated,
                 bool isSecurityRisk,
  ...
  bool CompatibleWith(const LibraryDependency &dependency) const {
    return dependency.RequiredVersion.compare(CompatibleFrom) >= 0 &&
           dependency.RequiredVersion.compare(CompatibleTo) <= 0;
  }
