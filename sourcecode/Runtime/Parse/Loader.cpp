...
Loader::Loader() {
  ...
  std::string *globalPathStr = new std::string(
      (std::filesystem::weakly_canonical(std::filesystem::path(NomRuntimePath))
           .parent_path() /
       "libraries")
          .string());
...
/*const NomClass * const Loader::GetClass(const std::string *libraryName, const
...
/*bool Loader::LoadAssemblyUnit(const std::string & name, NomProgram * parent)
const
{
        return false;
}*/
...
/*const Library * Loader::GetLibrary(const std::string &name) const {
        ...
        libraries.emplace(std::piecewise_construct, std::forward_as_tuple(name),
std::forward_as_tuple(name, std::move(versions)));
}*/
