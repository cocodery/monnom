...
BytecodeReader::BytecodeReader(CharStream &stream) : stream(stream) {
  ...
  if (versionflag != 2) {
    if (versionflag < 2) {
      std::cout << "The given bytecode file uses an older format that is not "
                   "supported by this runtime!\n";
    } else {
      std::cout << "The given bytecode file uses an newer format that is not "
                   "supported by this runtime!\n";
    }
...
NomInstruction *BytecodeReader::ReadInstruction() {
  ...
  case OpCode::EnsureCheckedMethod: {
    ...
    return new EnsureCheckedMethodInstruction(context.GetGlobalID(methodName),
                                              receiver);
  }
  case OpCode::EnsureDynamicMethod: {
    LocalConstantID methodName = stream.read<LocalConstantID>();
    RegIndex receiver = stream.read<RegIndex>();
    return new EnsureDynamicMethodInstruction(context.GetGlobalID(methodName),
                                              receiver);
  }
  ...
  case OpCode::LoadStringConstant: {
    ...
    RegIndex reg = stream.read<RegIndex>();
    return new LoadStringConstantInstruction(reg,
                                             context.GetGlobalID(strConstant));
  }
  case OpCode::InvokeCheckedInstance: {
    ...
    RegIndex reg = stream.read<RegIndex>();
    RegIndex receiver = stream.read<RegIndex>();
    return new CallCheckedInstanceMethod(reg, context.GetGlobalID(method),
                                         context.GetGlobalID(typeArgs),
                                         receiver);
  }
  case OpCode::CallFinal: {
    ...
    // RegIndex reg = stream.read<RegIndex>();
    // RegIndex receiver = stream.read<RegIndex>();
    // return new CallFinalInstanceMethod(reg, receiver,
    // context.GetGlobalID(method)); }
  ...
  case OpCode::CallCheckedStatic: {
    ...
    LocalConstantID typeArgs = stream.read<LocalConstantID>();
    RegIndex reg = stream.read<RegIndex>();
    return new CallCheckedStaticMethod(context.GetGlobalID(method),
                                       context.GetGlobalID(typeArgs), reg);
  }
  case OpCode::CallDispatchBest: {
    ...
    LocalConstantID methodName = stream.read<LocalConstantID>();
    LocalConstantID typeArgs = stream.read<LocalConstantID>();
    return new CallDispatchBestMethod(reg, receiver,
                                      context.GetGlobalID(methodName),
                                      context.GetGlobalID(typeArgs));
  }
  case OpCode::BinOp: {
  ...
  case OpCode::CallConstructor: {
    ...
    LocalConstantID typeArgs = stream.read<LocalConstantID>();
    return new CallConstructor(reg, context.GetGlobalID(cls),
                               context.GetGlobalID(typeArgs));
  }
  case OpCode::CreateClosure: {
    ...
    LocalConstantID typeArgs = stream.read<LocalConstantID>();
    RegIndex reg = stream.read<RegIndex>();
    return new CreateClosure(reg, context.GetGlobalID(lambda),
                             context.GetGlobalID(typeArgs));
  }
  case OpCode::ConstructStruct: {
    ...
    LocalConstantID typeArgs = stream.read<LocalConstantID>();
    RegIndex reg = stream.read<RegIndex>();
    return new ConstructStructInstruction(reg, context.GetGlobalID(structure),
                                          context.GetGlobalID(typeArgs));
  }
  case OpCode::Cast: {
    RegIndex reg = stream.read<RegIndex>();
  ...
  case OpCode::WriteField: {
    ...
    return new WriteFieldInstruction(receiver, value,
                                     context.GetGlobalID(fieldName),
                                     context.GetGlobalID(receiverType));
  }
  case OpCode::ReadField: {
    ...
    LocalConstantID fieldName = stream.read<LocalConstantID>();
    LocalConstantID receiverType = stream.read<LocalConstantID>();
    return new ReadFieldInstruction(reg, receiver,
                                    context.GetGlobalID(fieldName),
                                    context.GetGlobalID(receiverType));
  }
  case OpCode::PhiNode: {
  ...
  case OpCode::CondBranch: {
    ...
    CondBranchInstruction *cbi =
        new CondBranchInstruction(condition, thenTarget, elseTarget);
...
void BytecodeReader::readToEnd(NomModule *mod,
                               BytecodeTopReadHandler *handler) {
  bool cont = true;
  while (cont) {
...
NomClass *BytecodeReader::ReadClass(NomModule *mod,
                                    BytecodeTopReadHandler *handler) {
  ...
  ConstantID superInterfaces =
      context.GetGlobalID(stream.read<LocalConstantID>());
  ConstantID superClass = context.GetGlobalID(stream.read<LocalConstantID>());
  NomClassLoaded *cls =
      mod->AddClass(name, typeArgs, superClass, superInterfaces);
...
NomInterface *BytecodeReader::ReadInterface(NomModule *mod,
                                            BytecodeTopReadHandler *handler) {
  ...
  ConstantID superInterfaces =
      context.GetGlobalID(stream.read<LocalConstantID>());
  NomInterfaceLoaded *iface =
      mod->AddInterface(name, typeParameters, superInterfaces);
  uint64_t methodCount = stream.read<uint64_t>();
  while (methodCount > 0) {
    ReadMethod(iface);
    methodCount--;
...
NomLambda *BytecodeReader::ReadLambda(NomClassLoaded *cls) {
  if (stream.read_char() !=
      (unsigned char)BytecodeInternalElementType::Lambda) {
  ...
  ConstantID closureTypeParameters =
      context.GetGlobalID(stream.read<LocalConstantID>());
  ...
  RegIndex regcount = stream.read<RegIndex>();
  NomLambda *lambda =
      cls->AddLambda(lambdaID, regcount, closureTypeParameters,
                     closureArguments, typeParameters, argTypes, returnType);
...
NomRecord *BytecodeReader::ReadStruct(NomClassLoaded *cls) {
  if (stream.read_char() !=
      (unsigned char)BytecodeInternalElementType::Record) {
  ...
  NomRecord *structure =
      cls->AddStruct(structID, closureTypeParameters, regcount, endargregcount,
                     initializerArgTypes);
  size_t fieldCount = stream.read<size_t>();
  while (fieldCount > 0) {
    ConstantID fieldName = context.GetGlobalID(stream.read<LocalConstantID>());
...
NomRecordMethod *BytecodeReader::ReadStructMethod(NomRecord *structure) {
  if (stream.read_char() !=
      (unsigned char)BytecodeInternalElementType::Method) {
  ...
  RegIndex regcount = stream.read<RegIndex>();
  std::string qname =
      "STRUCT_" + to_string(structure->StructID) + "." + namestr;
  NomRecordMethod *meth = structure->AddMethod(namestr, qname, typeParameters,
...
NomMethod *BytecodeReader::ReadMethod(NomInterfaceLoaded *iface) {
  ...
  NomMethodLoaded *meth =
      iface->AddMethod(namestr, iface->GetName()->ToStdString() + "." + namestr,
                       typeParameters, returnType, argTypes, regcount, isFinal);
  uint64_t instructionCount = stream.read<uint64_t>();
  while (instructionCount > 0) {
    auto instr = ReadInstruction();
    if (NomVerbose) {
...
NomStaticMethod *BytecodeReader::ReadStaticMethod(NomClassLoaded *cls) {
  if (stream.read_char() !=
      (unsigned char)BytecodeInternalElementType::StaticMethod) {
  ...
  NomStaticMethodLoaded *meth = cls->AddStaticMethod(
      namestr, qnamestr, typeargs, returnType, argTypes, regcount);
  uint64_t instructionCount = stream.read<uint64_t>();
  while (instructionCount > 0) {
    auto instr = ReadInstruction();
    if (NomVerbose) {
...
NomConstructor *BytecodeReader::ReadConstructor(NomClassLoaded *cls) {
  if (stream.read_char() !=
      (unsigned char)BytecodeInternalElementType::Constructor) {
...
NomTypedField *BytecodeReader::ReadField(NomClassLoaded *cls) {
  ...
  unsigned char flags = stream.read<unsigned char>();
  NomTypedField *field = cls->AddField(
      name, type, visibility, (flags & '\1') == '\1', (flags & '\2') == '\2');
  return field;
}
  ...
  if (paramcount > 0) {
    ...
    for (uint64_t i = 0; i < paramcount; i++) {
      ...
      ConstantID upperBound =
          context.GetGlobalID(stream.read<LocalConstantID>());
      parameters[i] = new NomTypeParameterConstant(ParameterVariance::Invariant,
                                                   lowerBound, upperBound);
    }
  }
  return context.AddTypeParameters(
      lcid, llvm::ArrayRef<NomTypeParameterConstant *>(parameters, paramcount));
}
