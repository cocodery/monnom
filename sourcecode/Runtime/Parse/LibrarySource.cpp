...
const llvm::ArrayRef<const NomInterfaceInternal *> GetStdLibInterfaces() {
  static const NomInterfaceInternal *ret[] = {
      IPairInterface::GetInstance(), IComparableInterface::GetInstance(),
...
void InternalLibrarySource::GetLibraryVersions(
    const std::string *name, std::list<LibraryVersion> &versions) {
...
void LocalLibrarySource::GetLibraryVersions(
    const std::string *name, std::list<LibraryVersion> &versions) {
  std::string path =
      std::string(*Path).append("/").append(*name).append(".manifest");
  ...
  if (file != nullptr) {
    ...
    //	{
    //		libVersion =
    // VersionNumber(versionnode.attribute("major").as_uint(),
    // versionnode.attribute("minor").as_uint(),
    // versionnode.attribute("revision").as_uint(),
    // versionnode.attribute("build").as_uint());
    //	}
    ...
    // versionnode.attribute("build").as_uint());
    //		}
    //		dependencies.emplace_back(depnode.attribute("name").as_string(),
    // depversion);
    //	}
    ...
    //	{
    //		mainclassname =
    // std::string(mainclassentry.attribute("name").as_string());
    //	}
    versions.emplace_back(
        name, mainclassname, libVersion, compFromVersion, compToVersion,
        isDeprecated, isSecurityRisk, std::move(dependencies),
        new ManifestComponentSource(Path, manifest, lcs.Program));
  }
  ////return std::move(versions);
}

InternalComponentSource::InternalComponentSource(NomProgram *program)
    : ComponentSource(program) {
...
const NomClass *InternalComponentSource::LoadClass(NomStringRef name,
                                                   NomModule *mod) {
  for (auto cls : LibrarySourceNS::GetStdLibClasses()) {
...
const NomInterface *InternalComponentSource::LoadInterface(NomStringRef name,
                                                           NomModule *mod) {
  for (auto iface : LibrarySourceNS::GetStdLibInterfaces()) {
...
const NomClass *LocalComponentSource::LoadClass(NomStringRef name,
                                                NomModule *mod) {
  std::string filepath = (*Path) + "/" + (name->ToStdString()) + ".gnil";
  // std::cout << "FILE: " + filepath + "\n";
  std::cout.flush();
  // std::basic_ifstream<unsigned char> fstream(filepath, std::ios::in |
  // std::ios::binary); std::basic_istream<char> stream =
  // std::basic_istream<char>(fstream.rdbuf());
...
const NomInterface *LocalComponentSource::LoadInterface(NomStringRef name,
                                                        NomModule *mod) {
...
const NomClass *ManifestComponentSource::LoadClass(NomStringRef name,
                                                   NomModule *mod) {
...
const NomInterface *ManifestComponentSource::LoadInterface(NomStringRef name,
                                                           NomModule *mod) {
  std::string stdname = name->ToStdString();
