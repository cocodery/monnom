...
Manifest *ManifestReader::ReadManifest(xml_document &doc) {
  ...
  VersionNumber libVersion =
      VersionNumber(libnode.attribute("major").as_uint(),
                    libnode.attribute("minor").as_uint(),
                    libnode.attribute("revision").as_uint(),
                    libnode.attribute("build").as_uint());
  VersionNumber compFromVersion = libVersion;
  xml_node versionnode = libnode.child("compatiblefrom");
  if (versionnode) {
    compFromVersion = VersionNumber(versionnode.attribute("major").as_uint(),
  ...
  if (versionnode) {
    compToVersion = VersionNumber(versionnode.attribute("major").as_uint(),
                                  versionnode.attribute("minor").as_uint(),
                                  versionnode.attribute("revision").as_uint(),
                                  versionnode.attribute("build").as_uint());
  }
  ...
  Manifest *manifest =
      new Manifest(libname.as_string(), libVersion, compFromVersion,
                   compToVersion, isSecurityRisk, isDeprecated);
  if (!deps.empty()) {
    for (auto child : deps.children()) {
      manifest->Dependencies.emplace_back(
          child.attribute("name").as_string(),
          VersionNumber(child.attribute("major").as_int(),
                        child.attribute("minor").as_int(),
                        child.attribute("revision").as_int(),
                        child.attribute("buld").as_int()));
    }
  }
  if (!classes.empty()) {
    for (auto child : classes.children()) {
      manifest->Classes.emplace_back(child.attribute("qname").as_string(),
                                     child.attribute("file").as_string());
    }
  }
  if (!interfaces.empty()) {
    for (auto child : interfaces.children()) {
      manifest->Interfaces.emplace_back(child.attribute("qname").as_string(),
                                        child.attribute("file").as_string());
    }
  }
  if (!mainclass.empty()) {
