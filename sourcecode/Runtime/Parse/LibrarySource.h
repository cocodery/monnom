...
public:
  ...
  virtual void GetLibraryVersions(const std::string *name,
                                  std::list<LibraryVersion> &versions) = 0;
...
class LocalComponentSource : public ComponentSource,
                             public BytecodeTopReadHandler {
  /*protected:
          NomProgram * const program;*/
public:
  const std::string *Path;
  LocalComponentSource(const std::string *path, NomProgram *program)
      : ComponentSource(program), Path(path) //, program(program)
  {}
...
class ManifestComponentSource : public ComponentSource,
                                public BytecodeTopReadHandler {
public:
  const std::string *Path;
  const Nom::Runtime::Manifest *const Manifest;
  ManifestComponentSource(const std::string *path,
                          const Nom::Runtime::Manifest *manifest,
                          NomProgram *program)
      : ComponentSource(program), Path(path),
...
public:
  InternalLibrarySource(NomProgram *program)
      : LibrarySource(),
        stdlib(new std::string("stdlib"), "", VersionNumber(1, 0, 0, 1),
               VersionNumber(1, 0, 0, 1), VersionNumber(1, 0, 0, 1), false,
               false, std::vector<LibraryDependency>(),
               new InternalComponentSource(program)) {}

  virtual void GetLibraryVersions(const std::string *name,
                                  std::list<LibraryVersion> &versions) override;
...
public:
  const std::string *Path;
  LocalLibrarySource(const std::string *path, NomProgram *program)
      : LibrarySource(), lcs(path, program), Path(path) {}
