...
namespace Nom {
namespace Runtime {
void LoadStringConstantInstruction::Compile(NomBuilder &builder,
                                            CompileEnv *env, int lineno) {
  env->basicBlockTerminated = false;
  RegisterValue(
      env,
      NomValue(
          NomConstants::GetString(this->constantID)->getObject(*(env->Module)),
          NomStringClass::GetInstance()->GetType()));
  // GenerateLLVMDebugPrint(builder, env->Module, "Loading String Constant\n");
  // std::array<llvm::Value *, 1> args = { {
  // llvm::ConstantInt::get(LLVMCONSTANTINDEXTYPE, constantID, false) } };
...
void LoadStringConstantInstruction::FillConstantDependencies(
