...
CondBranchInstruction::CondBranchInstruction(RegIndex condition, int thenTarget,
                                             int elseTarget)
    : NomInstruction(OpCode::CondBranch), Condition(condition),
      ThenTarget(thenTarget), ElseTarget(elseTarget) {}

CondBranchInstruction::~CondBranchInstruction() {}
void CondBranchInstruction::Compile(NomBuilder &builder, CompileEnv *env,
                                    int lineno) {
  ...
  for (auto ti : ThenIncomings) {
    ...
    auto incval = EnsureType(builder, env, (*env)[std::get<1>(ti)],
                             nv.GetNomType(), llvmPHI->getType());
    incPairs[incPos] = std::make_pair(llvmPHI, incval);
    incPos++;
    //((llvm::PHINode*)(llvm::Value*)nv)->addIncoming(incval,
    // builder->GetInsertBlock());
  }
  for (auto ei : ElseIncomings) {
    NomValue nv = (*env)[std::get<0>(ei)];
    llvm::PHINode *llvmPHI = ((llvm::PHINode *)(llvm::Value *)nv);
    auto incval = EnsureType(builder, env, (*env)[std::get<1>(ei)],
                             nv.GetNomType(), llvmPHI->getType());
    incPairs[incPos] = std::make_pair(llvmPHI, incval);
    incPos++;
    //((llvm::PHINode*)(llvm::Value*)nv)->addIncoming(incval,
...
void CondBranchInstruction::FillConstantDependencies(
