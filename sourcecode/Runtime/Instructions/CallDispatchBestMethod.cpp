...
namespace Nom {
namespace Runtime {
CallDispatchBestMethod::CallDispatchBestMethod(RegIndex reg, RegIndex receiver,
                                               ConstantID methodName,
                                               ConstantID typeArgs)
    : NomValueInstruction(reg, OpCode::CallDispatchBest), Receiver(receiver),
      MethodName(methodName), TypeArguments(typeArgs) {}

CallDispatchBestMethod::~CallDispatchBestMethod() {}

void CallDispatchBestMethod::Compile(NomBuilder &builder, CompileEnv *env,
                                     int lineno) {
  auto methodName =
  ...
  argbuf[0] = NomLambdaCallTag::GetCallTag(typeargcount, argcount)
                  ->GetLLVMElement(
  ...
  if (dispargcount > RTConfig_NumberOfVarargsArguments) {
    ...
    argbuf[RTConfig_NumberOfVarargsArguments + 2] =
        builder->CreatePointerCast(extArgArray, POINTERTYPE);
  }
  while (argbufpos < typeargcount) {
    Value *targ = builder->CreatePointerCast(
        typeargs[argbufpos]->GetLLVMElement(*(env->Module)), POINTERTYPE);
    if (argbufpos <
        RTConfig_NumberOfVarargsArguments -
            (dispargcount <= RTConfig_NumberOfVarargsArguments ? 0 : 1)) {
      ...
      MakeStore(
          ...
          builder->CreateGEP(
              extArgArray,
              MakeInt32(argbufpos - (RTConfig_NumberOfVarargsArguments - 1))));
    }
    argbufpos++;
  }
  while (argbufpos < dispargcount) {
    Value *arg = builder->CreatePointerCast(
        EnsurePacked(builder, env->GetArgument(argbufpos - typeargcount)),
        POINTERTYPE);
    if (argbufpos <
        RTConfig_NumberOfVarargsArguments -
            (dispargcount <= RTConfig_NumberOfVarargsArguments ? 0 : 1)) {
  ...
  auto dispatcherCallInst = builder->CreateCall(
      GetIMTFunctionType(), builder->CreateExtractValue(dispatcherPair, {0}),
      llvm::ArrayRef<Value *>(argbuf, 2 + RTConfig_NumberOfVarargsArguments),
      methodName + "()");
  dispatcherCallInst->setCallingConv(NOMCC);
  env->ClearArguments();
  RegisterValue(
      env, NomValue(EnsurePackedUnpacked(builder, dispatcherCallInst, REFTYPE),
                    &NomDynamicType::Instance(), true));
}
...
void CallDispatchBestMethod::FillConstantDependencies(
