...
void BranchInstruction::Compile(NomBuilder &builder, CompileEnv *env,
                                int lineno) {
  ...
  for (auto &i : Incomings) {
    ...
    auto incVal = EnsureType(builder, env, (*env)[std::get<1>(i)],
...
void BranchInstruction::FillConstantDependencies(
