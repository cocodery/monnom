...
CallCheckedStaticMethod::CallCheckedStaticMethod(const ConstantID method,
                                                 const ConstantID typeArgs,
                                                 RegIndex reg)
    : NomValueInstruction(reg, OpCode::CallCheckedStatic), Method(method),
      TypeArgs(typeArgs) {}

CallCheckedStaticMethod::~CallCheckedStaticMethod() {}
void CallCheckedStaticMethod::Compile(NomBuilder &builder, CompileEnv *env,
                                      int lineno) {
  if (NomCastStats) {
    builder->CreateCall(
        GetIncStaticMethodCalls(
  ...
  auto argarr =
      makealloca(Value *, method.Elem->GetArgumentCount() + typeArgs.size());
  ...
  for (int i = argcount - 1; i >= 0; i--) {
    ...
    argarr[i + targcount] = CastInstruction::MakeCast(
        builder, env, arg, signature.ArgumentTypes[i]);
  }
  ...
  env->ClearArguments();
  auto args = llvm::ArrayRef<llvm::Value *>(
      argarr, method.Elem->GetArgumentCount() + typeArgs.size());
...
void CallCheckedStaticMethod::FillConstantDependencies(
