...
ReadFieldInstruction::ReadFieldInstruction(const RegIndex reg,
                                           const RegIndex receiver,
                                           const ConstantID fieldName,
                                           const ConstantID type)
    : NomValueInstruction(reg, OpCode::ReadField), Register(reg),
      Receiver(receiver), FieldName(fieldName), ReceiverType(type) {}

ReadFieldInstruction::~ReadFieldInstruction() {}
void ReadFieldInstruction::Compile(NomBuilder &builder, CompileEnv *env,
                                   int lineno) {
  if (ReceiverType != 0) {
    ...
    RegisterValue(env, NomDictField::GetInstance(
                           NomConstants::GetString(FieldName)->GetText())
                           ->GenerateRead(builder, env, (*env)[Receiver]));
  }
}
...
void ReadFieldInstruction::FillConstantDependencies(
