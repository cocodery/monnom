...
RT_NOM_SetDebugLevel(decltype(NomDebugPrintLevel) dbglvl) {
  NomDebugPrintLevel = dbglvl;
}

namespace Nom {
namespace Runtime {
RTCmdInstruction::RTCmdInstruction(const ConstantID cmd)
    : NomInstruction(OpCode::Debug), cmdConstant(cmd) {}

RTCmdInstruction::~RTCmdInstruction() {}

void RTCmdInstruction::Compile(NomBuilder &builder, CompileEnv *env,
                               int lineno) {
  static bool once = true;
  if (once) {
    once = false;
    llvm::sys::DynamicLibrary::AddSymbol("RT_NOM_SetDebugLevel",
                                         (void *)&RT_NOM_SetDebugLevel);
  }
  ...
  if (cmd.substr(0, 14) == "SetDebugLevel ") {
    ...
    if (setDebugLevelFun == nullptr) {
      FunctionType *setDebugLevelFT =
          FunctionType::get(llvm::Type::getVoidTy(LLVMCONTEXT),
                            {numtype(decltype(NomDebugPrintLevel))}, false);
      setDebugLevelFun = Function::Create(
          setDebugLevelFT, GlobalValue::LinkageTypes::ExternalLinkage,
          "RT_NOM_SetDebugLevel", env->Module);
    }
    builder->CreateCall(setDebugLevelFun,
                        {MakeInt<decltype(NomDebugPrintLevel)>(lvl)});
    return;
  }
...
void RTCmdInstruction::FillConstantDependencies(
