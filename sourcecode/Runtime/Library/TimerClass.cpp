...
LIB_NOM_Timer_PrintDifference_1(void *timer) noexcept(false) {
#ifdef _WIN32
  ...
  if (QueryPerformanceCounter(&current)) {
    ...
    double floatdiff =
        ((double)(difference.QuadPart)) / Nom::Runtime::find_timer_frequency();
    if (!isInWarmup()) {
      printf("%f Seconds\n", floatdiff);
    }
  ...
  struct timespec *myvalptr =
      (struct timespec *)(((char *)timer) + sizeof(intptr_t));
  if (clock_gettime(CLOCK_THREAD_CPUTIME_ID, &current) == 0) {
    long differenceMS = ((current.tv_sec - myvalptr->tv_sec) * 10000) +
                        ((current.tv_nsec - myvalptr->tv_nsec) / 100000);
...
extern "C" DLLEXPORT void *
LIB_NOM_Timer_Constructor_0(void *timer) noexcept(false) {
#ifdef _WIN32
  char *valueptr = ((char *)timer) + sizeof(LARGE_INTEGER);
...
NomTimerClass::NomTimerClass()
    : NomInterface("Timer_0"), NomClassInternal(new NomString("Timer_0"))
// NomClass(NomConstants::AddString(NomString("Timer")), 0,
...
// NomConstants::AddString(NomString("Object"))), 0), 0, nullptr)
{
  ...
  NomMethodInternal *printDifference = new NomMethodInternal(
      this, "PrintDifference", "LIB_NOM_Timer_PrintDifference_1", true);
  ...
  NomConstructorInternal *constructor =
      new NomConstructorInternal("LIB_NOM_Timer_Constructor_0", this);
  ...
  // NomMethod* print = AddMethod("PrintDifference",
  // "LIB_NOM_Timer_PrintDifference_1", 0, voidClassTypeID, 0, 0, true, true,
  // true);
  ...
  // NomConstructor* constructor = new NomConstructor(this,
  // "LIB_NOM_Timer_Constructor", "LIB_NOM_Timer_Constructor_0", 0, 0, 0, true,
  // true); Constructors.push_back(constructor);
  ////this->PreprocessInheritance();

  llvm::sys::DynamicLibrary::AddSymbol("LIB_NOM_Timer_Constructor_0",
                                       (void *)&LIB_NOM_Timer_Constructor_0);
  llvm::sys::DynamicLibrary::AddSymbol(
      "LIB_NOM_Timer_PrintDifference_1",
      (void *)&LIB_NOM_Timer_PrintDifference_1);
}
