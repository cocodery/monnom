...
// Nom::Runtime::NomStringClass *_NomStringClass=
// Nom::Runtime::NomStringClass::GetInstance(); const
// Nom::Runtime::RTStringClass _RTStringClass; const
// Nom::Runtime::NomStringClass * const _NomStringClassRef = &_NomStringClass;
// const Nom::Runtime::RTStringClass * const _RTStringClassRef =
// &_RTStringClass; const Nom::Runtime::NomClass * const _NomStringClassNC =
// &_NomStringClass; const Nom::Runtime::RTClass * const _RTStringClassRTC =
// &_RTStringClass;
...
LIB_NOM_DEBUG_PRINT(void *str, int64_t index,
...
extern "C" DLLEXPORT void
LIB_NOM_DEBUG_PRINT_HEX(void *str, int64_t index,
                        decltype(NomDebugPrintLevel) level) {
  if (level <= 3) {
...
namespace Nom {
namespace Runtime {
NomStringClass::NomStringClass()
    : NomInterface("String_0"), NomClassInternal(new NomString("String_0"))
// NomClass(NomConstants::AddString(NomString("String")), 0,
// NomConstants::AddSuperClass(NomConstants::AddClass(NomConstants::AddString(NomString("std")),
// NomConstants::AddString(NomString("Object"))), 0), 0, nullptr)
{
  this->SetDirectTypeParameters();
  this->SetSuperClass();
  this->SetSuperInterfaces();

  NomMethodInternal *print =
      new NomMethodInternal(this, "Print", "LIB_NOM_String_Print_1", true);
  ...
  // NomMethod* print = AddMethod("Print", "LIB_NOM_String_Print_1", 0,
  // voidClassTypeID, 0, 0, true, true, true);
  ////MethodTable = NomObjectClass::GetInstance()->MethodTable;
  ////print->SetOffset(-1-MethodTable.size());
  ////MethodTable.push_back(new NomMethodTableEntry(print,
  /// print->GetLLVMFunctionType(), print->GetOffset()));

  // llvm::sys::DynamicLibrary::AddSymbol("LIB_NOM_DEBUG_PRINT",
  // (void*)&LIB_NOM_DEBUG_PRINT);
  // llvm::sys::DynamicLibrary::AddSymbol("LIB_NOM_DEBUG_PRINT_HEX",
  // (void*)&LIB_NOM_DEBUG_PRINT_HEX);
  // llvm::sys::DynamicLibrary::AddSymbol("LIB_NOM_String_Print_1",
  // (void*)&LIB_NOM_String_Print_1);
}
...
extern "C" DLLEXPORT void *LIB_NOM_String_Print_1(void *str) {
  ...
  // std::cout <<
  // ((NomStringRef)(ObjectHeader(str).Fields()-sizeof(intptr_t)))->ToStdString();
  return (void *)((
      intptr_t)(NomJIT::Instance().getSymbolAddress("RT_NOM_VOIDOBJ")));
}
...
llvm::Function *GetDebugPrint(llvm::Module *mod) {
  ...
  if (ret == nullptr) {
    ...
    ret = Function::Create(printFunType, Function::ExternalLinkage,
                           "LIB_NOM_DEBUG_PRINT", mod);
    // NomJIT::Instance().RegisterGlobalMapping(ret,(void*)
    // &CPP_NOM_LoadStringConstant);
  }
  return ret;
}
