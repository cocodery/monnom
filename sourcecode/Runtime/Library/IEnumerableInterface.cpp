...
IEnumerableInterface::IEnumerableInterface()
    : NomInterface("IEnumerable_1"),
      NomInterfaceInternal(new NomString("IEnumerable_1")) {
...
IEnumerableInterface *IEnumerableInterface::GetInstance() {
  ...
  if (once) {
    once = false;
    NomMethodDeclInternal *meth = new NomMethodDeclInternal(
        &iei, "GetEnumerator", "IEnumerable$$GetEnumerator");
    ...
    auto enumeratorType =
        IEnumeratorInterface::GetInstance()->GetType(TypeList(tvararr, 1));
...
void IEnumerableInterface::GetInterfaceDependencies(
