...
namespace Nom {
namespace Runtime {
template <typename T, typename S> class AvailableExternallyVariants {
private:
        ...
        for (auto &var : getVariants()) {
          if (var == variant) {
            ret = this->createLLVMElement(
                mod, llvm::GlobalValue::LinkageTypes::ExternalLinkage, var);
            found = true;
          } else {
            this->createLLVMElement(
                mod, llvm::GlobalValue::LinkageTypes::ExternalLinkage, var);
          }
        }
        if (!found) {
          ret = this->createLLVMElement(
              mod, llvm::GlobalValue::LinkageTypes::ExternalLinkage, variant);
        }
      } else {
        ret = this->createLLVMElement(
...
protected:
  virtual T *createLLVMElement(llvm::Module &mod,
