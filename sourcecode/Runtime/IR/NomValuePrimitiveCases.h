...
namespace Nom {
namespace Runtime {
// void GeneratePrimitiveTypeCase(NomBuilder &builder, NomValue & value, []void
// (**handleInt)(NomBuilder& builder, NomValue&, NomValue*), []void
// (*handleFloat)(NomBuilder& builder, NomValue&, NomValue*), []void
// (*handleRef)(NomBuilder& builder, NomValue&, llvm::Value*, NomValue*),
// NomValue *returnValues, int numReturns=1);
...
void GeneratePrimitiveTypeCase(NomBuilder &builder, NomValue &value,
                               HandleInt handleInt, HandleFloat handleFloat,
                               HandleRef handleRef, NomValue *returnValues,
                               int numReturns) {
  ...
  if (value->getType()->isFloatingPointTy() ||
      vtype->IsSubtype(floattype, false)) {
  ...
  llvm::BasicBlock *intBlock =
      BasicBlock::Create(LLVMCONTEXT, "INTBLOCK", startBlock->getParent());
  ...
  if (intPossible) {
    ...
    negintBlock = BasicBlock::Create(LLVMCONTEXT, "NINTTAG",
                                     startBlock->getParent(), intBlock);
  ...
  llvm::BasicBlock *floatBlock =
      BasicBlock::Create(LLVMCONTEXT, "FLOATBLOCK", startBlock->getParent());
  llvm::BasicBlock *floatUnpackBlock = floatBlock;
  llvm::BasicBlock *extractfloatBlock = floatBlock;

  if (floatPossible) {
    floatUnpackBlock =
        BasicBlock::Create(LLVMCONTEXT, "FLOATTAG", startBlock->getParent());
    extractfloatBlock = BasicBlock::Create(LLVMCONTEXT, "FLOATEXTRACT",
                                           startBlock->getParent());
  }

  llvm::BasicBlock *refBlock =
      BasicBlock::Create(LLVMCONTEXT, "REFBLOCK", startBlock->getParent());
  llvm::BasicBlock *refCheckBlock =
      BasicBlock::Create(LLVMCONTEXT, "REFTAG", startBlock->getParent());
  llvm::BasicBlock *returnBlock =
      BasicBlock::Create(LLVMCONTEXT, "TCRETURN", startBlock->getParent());
  ...
  if (!intPossible) {
    auto isFloat = builder->CreateICmpEQ(
        vtableptr,
        ConstantExpr::getPtrToInt(NomFloatClass::GetInstance()->GetLLVMElement(
                                      *startBlock->getParent()->getParent()),
                                  numtype(intptr_t)));
    builder->CreateCondBr(isFloat, extractfloatBlock, refBlock);
  } else if (!floatPossible) {
    auto isInt = builder->CreateICmpEQ(
        vtableptr,
        ConstantExpr::getPtrToInt(NomIntClass::GetInstance()->GetLLVMElement(
                                      *startBlock->getParent()->getParent()),
                                  numtype(intptr_t)));
    ...
    auto isFloat = builder->CreateICmpEQ(
        ...
        ConstantExpr::getPtrToInt(NomFloatClass::GetInstance()->GetLLVMElement(
                                      *startBlock->getParent()->getParent()),
                                  numtype(intptr_t)));
    auto combo = builder->CreateOr(
        builder->CreateShl(builder->CreateZExt(isInt, inttype(2)),
                           MakeUInt(2, 1)),
        builder->CreateZExt(isFloat, inttype(2)));
  ...
  if (intPossible) {
    ...
    auto extractInt = builder->CreatePtrToInt(
    ...
    auto posFsh = builder->CreateIntrinsic(
        Intrinsic::fshr, {prefunnel->getType()},
        {prefunnel, prefunnel, ConstantInt::get(prefunnel->getType(), 3)});
    builder->CreateBr(intBlock);

    builder->SetInsertPoint(negintBlock);
    auto negFsh = builder->CreateIntrinsic(
        Intrinsic::fshr, {refAsInt->getType()},
  ...
  if (floatPossible) {
    BasicBlock *normalValueBlock = BasicBlock::Create(
        LLVMCONTEXT, "normalValueBlock", startBlock->getParent());
    BasicBlock *posZeroBlock = BasicBlock::Create(LLVMCONTEXT, "posZeroBlock",
                                                  startBlock->getParent());
    BasicBlock *negZeroBlock = BasicBlock::Create(LLVMCONTEXT, "negZeroBlock",
                                                  startBlock->getParent());
    ...
    auto unmaskedFloat = builder->CreateBitCast(
  ...
  for (int i = 0; i < numReturns; i++) {
    auto returnPHI = builder->CreatePHI(refReturns[i]->getType(), casenum,
                                        Twine("tcret").concat(Twine(i)));
