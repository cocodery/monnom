...
private:
  mutable llvm::ArrayRef<NomTypeParameterRef> allParameters =
      llvm::ArrayRef<NomTypeParameterRef>((NomTypeParameterRef *)nullptr,
                                          (size_t)0);
  mutable llvm::ArrayRef<NomTypeRef> allVariables =
      llvm::ArrayRef<NomTypeRef>((NomTypeRef *)nullptr, (size_t)0);
  mutable llvm::ArrayRef<NomTypeRef> directVariables =
      llvm::ArrayRef<NomTypeRef>((NomTypeRef *)nullptr, (size_t)0);
...
public:
  ...
  virtual const llvm::ArrayRef<NomTypeParameterRef>
  GetDirectTypeParameters() const = 0;
...
public:
  NomMemberContextLoaded(const NomMemberContext *parent,
                         ConstantID typeParametersID);
  virtual ~NomMemberContextLoaded() override;

  const llvm::ArrayRef<NomTypeParameterRef>
  GetDirectTypeParameters() const override;
...
public:
  NomMemberContextList(llvm::ArrayRef<NomTypeParameterRef> typeParameters,
                       const NomMemberContext *parent = nullptr)
      : typeParameters(typeParameters), parent(parent) {}
  virtual ~NomMemberContextList() override = default;

  virtual const std::string *GetSymbolName() const override {
    throw new std::exception(); // this is not a thing you should ask of this
                                // version of MemberContext
  }
  ...
  virtual const NomMemberContext *GetParent() const override { return parent; }
};

class NomMemberContextInternal : public virtual NomMemberContext {
private:
  llvm::ArrayRef<NomTypeParameterRef> typeParameters =
      llvm::ArrayRef<NomTypeParameterRef>((NomTypeParameterRef *)nullptr,
                                          (size_t)0);
  const NomMemberContext *const parent;

public:
  // One version of SetDirectTypeParameters must be called eventually
  // (TypeParameters need to be able to access "this" context object to set
  // parent)
  NomMemberContextInternal(const NomMemberContext *parent = nullptr)
      : parent(parent) {}
  virtual ~NomMemberContextInternal() override = default;

  void
  SetDirectTypeParameters(llvm::ArrayRef<NomTypeParameterRef> typeParameters) {
    if (this->typeParameters.data() != nullptr ||
        typeParameters.data() == nullptr) {
  ...
  void SetDirectTypeParameters() {
    this->typeParameters = llvm::ArrayRef<NomTypeParameterRef>(
        (NomTypeParameterRef *)this, (size_t)0);
  }
...
// public:
//	SimpleNomMemberContext(const NomMemberContext* parent, ConstantID
// typeParametersID, const std::string &symbolName);
// SimpleNomMemberContext(const NomMemberContext* parent,
// llvm::ArrayRef<NomTypeVarRef> typeParameters, const std::string& symbolName);
//	// Inherited via NomMemberContext
//	virtual const std::string* GetSymbolName() const override;
// };
