...
namespace Nom {
namespace Runtime {
NomMemberContextLoaded::NomMemberContextLoaded(const NomMemberContext *parent,
                                               ConstantID typeParametersID)
    : parent(parent), typeParametersID(typeParametersID) {}
...
size_t NomMemberContextLoaded::GetDirectTypeParametersCount() const {
  if (initialized) {
    ...
    NomTypeParametersConstant *ntlc =
        NomConstants::GetTypeParameters(typeParametersID);
    return ntlc->GetSize();
  }
}
NomTypeParameterRef
NomMemberContextLoaded::GetLocalTypeParameter(int index) const {
  return GetDirectTypeParameters()[index];
}
const llvm::ArrayRef<NomTypeParameterRef>
NomMemberContextLoaded::GetDirectTypeParameters() const {
  if (!initialized) {
    NomTypeParametersConstant *ntlc =
        NomConstants::GetTypeParameters(typeParametersID);
    llvm::ArrayRef<NomTypeParameterConstant *> tparams = ntlc->GetParameters();
    size_t varcount = tparams.size();
    NomTypeParameterLoaded *vars = (NomTypeParameterLoaded *)nmalloc(
        sizeof(NomTypeParameterLoaded) * varcount);
    NomTypeParameterRef *varptrs = new NomTypeParameterRef[varcount];
    for (size_t i = 0; i < varcount; i++) {
      varptrs[i] = &vars[i];
      new (vars + i) NomTypeParameterLoaded(this, i, tparams[i]->UpperBound,
                                            tparams[i]->LowerBound);
    }
    this->TypeParameters =
        llvm::ArrayRef<NomTypeParameterRef>(varptrs, varcount);
    initialized = true;
  }
...
const llvm::ArrayRef<NomTypeRef>
NomMemberContext::GetDirectTypeVariables() const {
  if (directVariables.data() == nullptr) {
    auto directParams = GetDirectTypeParameters();
    NomTypeRef *tarr =
        (NomTypeRef *)(nmalloc(sizeof(NomTypeRef) * directParams.size()));
...
// SimpleNomMemberContext::SimpleNomMemberContext(const NomMemberContext*
// parent, ConstantID typeParametersID, const std::string& symbolName) :
// NomMemberContext(parent, typeParametersID), symbolName(symbolName)
//{
// }

// SimpleNomMemberContext::SimpleNomMemberContext(const NomMemberContext*
// parent, llvm::ArrayRef<NomTypeVarRef> typeParameters, const std::string&
// symbolName) : NomMemberContext(parent, typeParameters),
...
const llvm::ArrayRef<NomTypeParameterRef>
NomMemberContext::GetAllTypeParameters() const {
  ...
  if (allParameters.data() == nullptr) {
    ...
    NomTypeParameterRef *parr =
        (NomTypeParameterRef *)(nmalloc(sizeof(NomTypeParameterRef) * count));
