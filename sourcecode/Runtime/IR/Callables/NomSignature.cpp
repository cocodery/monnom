...
NomSignature::NomSignature(
    const std::vector<NomTypeParameterRef> &&typeArguments,
    const std::vector<NomTypeRef> &&argTypes, const NomTypeRef returnType)
...
NomSignature::Substitute(const NomSubstitutionContext *context) const {
  ...
  return NomSignature(std::move(targs), std::move(ntr),
                      ReturnType->SubstituteSubtyping(context));
}
bool NomSignature::SatisfiesArguments(const NomSignature &other,
  ...
  while (ret && i > 0) {
    i--;
    ret = ret &&
          (other.ArgumentTypes[i]->IsSubtype(ArgumentTypes[i], optimistic));
  }
  return ret;
}
bool NomSignature::Satisfies(const NomSignature &other, bool optimistic) const {
  bool ret = (ReturnType->IsSubtype(other.ReturnType, optimistic)) &&
             SatisfiesArguments(other, optimistic);
  return ret;
}
