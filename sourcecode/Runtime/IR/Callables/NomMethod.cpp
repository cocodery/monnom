...
namespace Nom {
namespace Runtime {
NomMethodLoaded::NomMethodLoaded(const NomInterface *container,
                                 const std::string &name,
                                 const std::string &qname,
                                 const ConstantID returnType,
                                 const ConstantID argTypes, RegIndex regcount,
                                 ConstantID typeParameters, bool isFinal)
    : NomCallableLoaded(name, container, qname, regcount, typeParameters,
                        argTypes, false, false),
...
bool NomMethod::IsOffsetSet() { return offsetSet; }
...
int NomMethod::GetOffset() const { return offset; }
...
bool NomMethod::Overrides(const NomMethod *other) const {
  ...
  if (!GetReturnType(&thissc)->IsSubtype(other->GetReturnType(&othersc),
                                         true)) {
    return false;
  }
...
NomMethodInternal::NomMethodInternal(const NomInterface *container,
                                     const std::string &name,
                                     const std::string &qname, bool isFinal)
    : NomCallableInternal(name, qname, container), isFinal(isFinal),
      Container(container) {}
...
NomTypeRef
NomMethodInternal::GetReturnType(const NomSubstitutionContext *context) const {
  if (context == nullptr || context->GetTypeArgumentCount() == 0 ||
      !returnType->ContainsVariables()) {
    return returnType;
  }
  return returnType->SubstituteSubtyping(context);
}
llvm::Function *NomMethodInternal::createLLVMElement(
    Module &mod, llvm::GlobalValue::LinkageTypes linkage) const {
  NomBuilder builder;
  ...
  BasicBlock *forwardblock =
      BasicBlock::Create(LLVMCONTEXT, GetName() + "start", wfun);
  ...
  auto wcall = builder->CreateCall(
      ufun, llvm::ArrayRef<llvm::Value *>(argbuf, funtype->getNumParams()));
  wcall->setCallingConv(llvm::CallingConv::C);
  builder->CreateRet(wcall);
  return wfun;
}
llvm::ArrayRef<NomTypeParameterRef>
NomMethodInternal::GetArgumentTypeParameters() const {
  return GetDirectTypeParameters();
}
const NomInterface *NomMethodInternal::GetContainer() const {
  return Container;
}
llvm::Function *NomMethodLoaded::createLLVMElement(
    Module &mod, llvm::GlobalValue::LinkageTypes linkage) const {
  NomBuilder builder;
  ...
  InstanceMethodCompileEnv menv = InstanceMethodCompileEnv(
      regcount, name, fun, &phiNodes, this->GetDirectTypeParameters(),
      this->GetArgumentTypes(nullptr), selfType, this);
  CompileEnv *env = &menv;
...
NomTypeRef
NomMethodLoaded::GetReturnType(const NomSubstitutionContext *context) const {
  if (context != nullptr && context->GetTypeArgumentCount() > 0) {
    return NomConstants::GetType(context, returnType);
  } else {
    if (returnTypeBuf == nullptr) {
      NomSubstitutionContextMemberContext nscmc =
          NomSubstitutionContextMemberContext(this);
...
llvm::ArrayRef<NomTypeParameterRef>
NomMethodLoaded::GetArgumentTypeParameters() const {
  return GetDirectTypeParameters();
}

const NomInterface *NomMethodLoaded::GetContainer() const { return Container; }

llvm::FunctionType *NomMethod::GetRawInvokeLLVMFunctionType(
  ...
  args--;
  return llvm::FunctionType::get(GetReturnType(context)->GetLLVMType(),
                                 ArrayRef<llvm::Type *>(args, argcount), false);
}

llvm::FunctionType *
NomMethod::GetLLVMFunctionType(const NomSubstitutionContext *context) const {
...
NomMethodDeclLoaded::NomMethodDeclLoaded(const NomInterface *container,
                                         const std::string &name,
                                         const std::string &qname,
                                         const ConstantID returnType,
                                         const ConstantID argTypes,
                                         ConstantID typeParameters)
    : NomCallableLoaded(name, container, qname, 0, typeParameters, argTypes,
                        true, false),
      returnTypeID(returnType) {}

NomTypeRef NomMethodDeclLoaded::GetReturnType(
    const NomSubstitutionContext *context) const {
  if (context != nullptr && context->GetTypeArgumentCount() > 0) {
...
llvm::Function *NomMethodDeclLoaded::createLLVMElement(
...
NomMethodDeclInternal::NomMethodDeclInternal(NomInterface *container,
                                             const std::string &name,
                                             const std::string &qname)
    : NomCallableInternal(name, qname, container), Container(container) {}
...
llvm::Function *NomMethodDeclInternal::createLLVMElement(
    llvm::Module &mod, llvm::GlobalValue::LinkageTypes linkage) const {
  throw new std::exception(); // declaration should never actually be emitted
}

NomTypeRef NomMethodDeclInternal::GetReturnType(
    const NomSubstitutionContext *context) const {
  if (context != nullptr && context->GetTypeArgumentCount() &&
      returnType->ContainsVariables()) {
    return returnType->SubstituteSubtyping(context);
  }
  return returnType;
...
llvm::ArrayRef<NomTypeParameterRef>
NomMethodDeclInternal::GetArgumentTypeParameters() const {
  return GetDirectTypeParameters();
}
