...
NomCallableLoaded::NomCallableLoaded(const std::string &name,
                                     const NomMemberContext *parent,
                                     const std::string &qname,
                                     const RegIndex regcount,
                                     const ConstantID typeArgs,
                                     const ConstantID argTypes, bool declOnly,
                                     bool cppWrapper)
    : NomMemberContextLoaded(parent, typeArgs), declOnly(declOnly),
      cppWrapper(cppWrapper), name(name), qname(qname), regcount(regcount),
      argTypesID(argTypes) {}
// NomCallableLoaded::NomCallableLoaded(const std::string& name, const
// NomMemberContext* parent, const std::string& qname, const RegIndex regcount,
// const llvm::ArrayRef<NomTypeVarRef> typeParameters, bool declOnly, bool
// cppWrapper) : NomMemberContextLoaded(parent, typeParameters),
// declOnly(declOnly), cppWrapper(cppWrapper), name(name), qname(qname),
// regcount(regcount)
//{
// }
...
TypeList NomCallableLoaded::GetArgumentTypes(
...
bool NomCallable::Satisfies(const NomSubstitutionContext *context,
  ...
  for (size_t i = 0; i < typeArgs.size(); i++) {
    ...
    if (!typeArgs[i]->IsSubtype(this->GetArgumentTypeParameters()[i]
                                    ->GetUpperBound()
                                    ->SubstituteSubtyping(&substC))) {
      return false;
    }
    if (!typeArgs[i]->IsSupertype(this->GetArgumentTypeParameters()[i]
                                      ->GetLowerBound()
                                      ->SubstituteSubtyping(&substC))) {
      return false;
    }
...
//	if (ret == nullptr)
//	{
//		//std::cout << "\nCreating function Proto for static method " +
// name + "\n"; 		ret = Function::Create(GetLLVMFunctionType(),
// Function::ExternalLinkage, GetSymbolName(), mod);;
//	}
//	return ret;
// }
...
const std::string *NomCallable::GetSymbolName() const {
  if (symname.empty()) {
    if (GetName().size() > 0 && GetName().at(0) == '$') {
      ...
      symname += GetName() + "$$" +
                 std::to_string(GetDirectTypeParametersCount()) + "$$";
      int pastfirst = false;
...
llvm::FunctionType *
NomCallable::GetLLVMFunctionType(const NomSubstitutionContext *context) const {
  if (llvmType == nullptr) {
    ...
    llvmType =
        FunctionType::get(GetReturnType(context)->GetLLVMType(), args, false);
  }
  return llvmType;
}
...
TypeList NomCallableInternal::GetArgumentTypes(
    const NomSubstitutionContext *context) const {
  if (context == nullptr || context->GetTypeArgumentCount() == 0 ||
      argTypes.size() == 0) {
    return argTypes;
  ...
  NomTypeRef *tarr =
      (NomTypeRef *)(gcalloc(sizeof(NomTypeRef) * argTypes.size()));
  for (size_t i = 0; i < argTypes.size(); i++) {
    tarr[i] = argTypes[i]->SubstituteSubtyping(context);
  }
  return TypeList(tarr, argTypes.size());
}
int NomCallableInternal::GetArgumentCount() const { return argTypes.size(); }
} // namespace Runtime
} // namespace Nom