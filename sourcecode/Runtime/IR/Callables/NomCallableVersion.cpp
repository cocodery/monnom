...
NomCallableVersion::NomCallableVersion(const NomCallable *callable,
                                       llvm::FunctionType *functionType)
    : cvid(cvcounter()), Callable(callable), FunType(functionType) {}
llvm::Function *NomCallableVersion::createLLVMElement(
    llvm::Module &mod, llvm::GlobalValue::LinkageTypes linkage) const {
  std::string refname =
      "RT_NOM_CV_" + to_string(cvid) + "_" + *Callable->GetSymbolName();
  ...
  auto callinst = builder->CreateCall(
      cfun, llvm::ArrayRef<llvm::Value *>(argbuf, argcount));
  ...
  if (callinst->getType() == fun->getReturnType()) {
    ...
    } else if (fun->getReturnType()->isIntegerTy(
