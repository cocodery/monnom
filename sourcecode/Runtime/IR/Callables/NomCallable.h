...
class NomCallable : public virtual NomMemberContext,
                    public AvailableExternally<llvm::Function> {
private:
...
protected:
  ...
  void InitializePhis(NomBuilder &builder, llvm::Function *fun,
...
public:
  ...
  virtual NomTypeRef
  GetReturnType(const NomSubstitutionContext *context = nullptr) const = 0;
  virtual TypeList
  GetArgumentTypes(const NomSubstitutionContext *context) const = 0;
  virtual int GetArgumentCount() const = 0;
  virtual llvm::FunctionType *GetLLVMFunctionType(
      const NomSubstitutionContext *context = nullptr) const = 0;
  virtual const std::string &GetName() const = 0;
  virtual const std::string &GetQName() const = 0;
  virtual llvm::ArrayRef<NomTypeParameterRef>
  GetArgumentTypeParameters() const = 0;
  bool Satisfies(const NomSubstitutionContext *context, const TypeList typeArgs,
                 const TypeList argTypes) const;
  const std::string *GetSymbolName() const override;
  virtual const NomSignature
  Signature(const NomSubstitutionContext *context = nullptr) const {
    if (context != nullptr && context->GetTypeArgumentCount() > 0) {
      return NomSignature(GetArgumentTypeParameters(),
                          GetArgumentTypes(context), GetReturnType(context));
    }
    if (!signatureInitialized) {
      signature =
          NomSignature(GetArgumentTypeParameters(), GetArgumentTypes(context),
...
class NomCallableLoaded : public virtual NomCallable,
                          public NomMemberContextLoaded {
private:
...
public:
  NomCallableLoaded(const std::string &name, const NomMemberContext *parent,
                    const std::string &qname, const RegIndex regcount,
                    const ConstantID typeArgs, const ConstantID argTypes,
                    bool declOnly = false, bool cppWrapper = false);
  // NomCallableLoaded(const std::string& name, const NomMemberContext* parent,
  // const std::string& qname, const RegIndex regcount, const
  // llvm::ArrayRef<NomTypeVarRef> typeParameters, bool declOnly = false, bool
  // cppWrapper = false);
  virtual ~NomCallableLoaded() override;

  const std::string &GetName() const override { return name; }
  const std::string &GetQName() const override { return qname; }
...
class NomCallableInternal : public virtual NomCallable,
...
protected:
  NomCallableInternal(const std::string &name, const std::string &qname,
                      const NomMemberContext *parent = nullptr)
      : NomMemberContextInternal(parent), name(name), qname(qname) {}
...
// public:
//	NomLoadedReturnType(const ConstantID returnType) :
// returnType(returnType) {} 	~NomLoadedReturnType() {}
...
//
// public:
//	NomLoadedArgumentTypes(const ConstantID argumentTypes) :
// argumentTypes(argumentTypes) {} 	~NomLoadedArgumentTypes() {} protected:
// const ConstantID argumentTypes; public: 	TypeList GetArgumentTypes()
// const override { 		return
// NomConstants::GetTypeList(argumentTypes)->GetTypeList((T*)this);
//	}
