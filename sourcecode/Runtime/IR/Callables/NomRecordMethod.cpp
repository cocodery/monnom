...
namespace Nom {
namespace Runtime {
NomRecordMethod::NomRecordMethod(const NomRecord *container, std::string &name,
                                 std::string &qname, ConstantID typeParameters,
...
llvm::Function *NomRecordMethod::createLLVMElement(
  ...
  StructMethodCompileEnv menv = StructMethodCompileEnv(
      regcount, name, fun, &phiNodes, this->GetDirectTypeParameters(),
      this->GetArgumentTypes(nullptr), this);
  CompileEnv *env = &menv;
...
// TypeList NomRecordMethod::GetArgumentTypes(const NomSubstitutionContext*
// context) const
//{
...
// }
llvm::FunctionType *NomRecordMethod::GetLLVMFunctionType(
  ...
  return FunctionType::get(
      GetReturnType(context)->GetLLVMType(),
      ArrayRef<Type *>(args, targs_.size() + args_.size() + 1), false);
}
llvm::ArrayRef<NomTypeParameterRef>
NomRecordMethod::GetArgumentTypeParameters() const {
