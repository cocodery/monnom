...
NomConstructorLoaded::NomConstructorLoaded(
    const NomClass *cls, const std::string &name, const std::string &qname,
    const ConstantID arguments, const RegIndex regcount,
    const ConstantID typeArgs, bool declOnly, bool cppWrapper)
    : NomConstructor(), NomCallableLoaded(name, cls, qname, regcount, typeArgs,
                                          arguments, declOnly, cppWrapper),
      /*argumentTypes(arguments),*/ returnTypeBuf(nullptr), Class(cls) {}

NomConstructorLoaded::~NomConstructorLoaded() {}
llvm::Function *NomConstructorLoaded::createLLVMElement(
  ...
  if (cppWrapper) {
    ...
    BasicBlock *forwardblock =
        BasicBlock::Create(LLVMCONTEXT, name + "start", wfun);
    builder->SetInsertPoint(forwardblock);
    auto ufun = Function::Create(
        funtype,
        (declOnly ? llvm::GlobalValue::LinkageTypes::ExternalLinkage : linkage),
        qname, &mod);
  ...
  auto fun = llvm::Function::Create(GetLLVMFunctionType(), linkage,
                                    *GetSymbolName(), &mod);
  ...
  ConstructorCompileEnv cenv =
      ConstructorCompileEnv(regcount, *GetSymbolName(), fun, &(this->phiNodes),
                            this->GetAllTypeParameters(),
                            this->GetArgumentTypes(nullptr), selfType, this);
  CompileEnv *env = &cenv;

  BasicBlock *startBlock =
      BasicBlock::Create(LLVMCONTEXT, *GetSymbolName() + "$start", fun);
  ...
  for (size_t pos = 0; pos < this->Class->GetTypeParametersCount(); pos++) {
    ObjectHeader::GenerateWriteTypeArgument(builder, (*env)[0], offset + pos,
                                            env->GetTypeArgument(builder, pos));
  }
  ...
  if (superclass.HasElem() &&
      superclass.Elem != NomObjectClass::GetInstance()) {
    NomValue *superArgsBuf =
        (NomValue *)(nmalloc(sizeof(NomValue) * superConstructorArgs.size()));
    auto scasize = superConstructorArgs.size();
    for (decltype(scasize) i = 0; i < scasize; i++) {
      superArgsBuf[i] = (*env)[superConstructorArgs[i]];
    }
    superclass.Elem->GenerateConstructorCall(
        builder, env, superclass.TypeArgs, (*env)[0],
        llvm::ArrayRef<NomValue>(superArgsBuf, superConstructorArgs.size()));
  }
...
NomTypeRef NomConstructorLoaded::GetReturnType(
...
NomConstructorLoaded::GetArgumentTypeParameters() const {
  return GetAllTypeParameters();
}

llvm::FunctionType *NomConstructor::GetLLVMFunctionType(
    const NomSubstitutionContext *context) const {
...
NomConstructorInternal::NomConstructorInternal(const std::string &qname,
                                               const NomClass *cls)
    : NomCallableInternal("CONSTRUCT$", qname, cls), Container(cls) {}
NomConstructorInternal::~NomConstructorInternal() {}
llvm::Function *NomConstructorInternal::createLLVMElement(
    llvm::Module &mod, llvm::GlobalValue::LinkageTypes linkage) const {
  NomBuilder builder;
  auto funtype = GetLLVMFunctionType();
  auto wfun = Function::Create(funtype, linkage, *GetSymbolName(), &mod);
  wfun->setCallingConv(NOMCC);
  BasicBlock *forwardblock = BasicBlock::Create(
      LLVMCONTEXT, Container->GetName()->ToStdString() + "$CONSTRUCT$start",
      wfun);
  builder->SetInsertPoint(forwardblock);
...
NomTypeRef NomConstructorInternal::GetReturnType(
    const NomSubstitutionContext *context) const {
  if (context == nullptr) {
...
llvm::ArrayRef<NomTypeParameterRef>
NomConstructorInternal::GetArgumentTypeParameters() const {
  return GetAllTypeParameters();
}
const NomClass *NomConstructorInternal::GetClass() const { return Container; }
} // namespace Runtime
} // namespace Nom