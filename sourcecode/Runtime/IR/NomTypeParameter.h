...
protected:
  ...
  const int index;
  NomTypeParameter(const NomMemberContext *parent, int index)
      : variable(this), parent(parent), index(index) {}
...
public:
  ...
  int GetIndex() const;
  const NomTypeVarRef GetVariable() const { return &variable; }
};
...
public:
  NomTypeParameterInternal(const NomMemberContext *parent, int index,
                           NomTypeRef upperBound, NomTypeRef lowerBound)
      : NomTypeParameter(parent, index), upperBound(upperBound),
        lowerBound(lowerBound) {}
  virtual ~NomTypeParameterInternal() override = default;
  virtual NomTypeRef GetUpperBound() const override { return upperBound; }
  virtual NomTypeRef GetLowerBound() const override { return lowerBound; }
  virtual size_t GetHashCode() const override {
    return this->GetIndex() + (this->GetUpperBound()->GetHashCode() * 31 +
                               this->GetLowerBound()->GetHashCode()) *
                                  31;
  }
};
...
public:
  NomTypeParameterLoaded(const NomMemberContext *parent, int index,
                         ConstantID upperBound, ConstantID lowerBound);
  ...
  virtual size_t GetHashCode() const override {
    return this->GetIndex() +
           ((size_t)lowerBoundID * 31 + (size_t)upperBoundID) * 31;
  }
};
} // namespace Runtime
} // namespace Nom