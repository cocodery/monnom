...
namespace Nom {
namespace Runtime {
class NomString : public NomStringType,
                  public AvailableExternally<llvm::Constant> {
private:
...
public:
  ...
  NomString(const uint64_t length, CharStream &stream)
      : NomStringType(length, 0) {
    for (uint64_t i = 0; i < length; i++) {
      std::array<unsigned char, sizeof(NomChar)> chars =
          stream.read_chars<sizeof(NomChar)>();
  ...
  // llvm::GlobalVariable * GetLLVMStringStruct(llvm::Module &mod, bool refOnly
  // = false) const;
...
namespace std {
template <> struct hash<Nom::Runtime::NomString> {
...
template <> struct equal_to<Nom::Runtime::NomString> {
  typedef Nom::Runtime::NomString argument_type;
  constexpr bool operator()(const argument_type &lhs,
                            const argument_type &rhs) const noexcept {
