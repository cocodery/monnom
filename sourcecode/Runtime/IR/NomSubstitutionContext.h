...
protected:
  mutable llvm::ArrayRef<NomTypeRef> arr =
      llvm::ArrayRef<NomTypeRef>((const NomTypeRef *)nullptr, (size_t)0);
  const NomSubstitutionContext *const left;
  const NomSubstitutionContext *const right;

public:
  NomSubstitutionContextCombination(const NomSubstitutionContext *left,
                                    const NomSubstitutionContext *right)
  ...
  virtual llvm::ArrayRef<NomTypeRef> GetTypeParameters() const override {
    if (arr.data() == nullptr) {
      ...
      NomTypeRef *tarr =
          (NomTypeRef *)nmalloc(sizeof(NomTypeRef) * (leftcount + rightcount));
...
public:
  NomSubstitutionContextList(
      llvm::ArrayRef<NomTypeRef> typeParameters = nullptr)
