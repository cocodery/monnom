...
ConstantID
NomConstantContext::TryGetGlobalID(const LocalConstantID local) const {
...
ConstantID NomConstantContext::AddString(LocalConstantID lid,
                                         const NomString &text) {
  idmap[lid] = NomConstants::AddString(text, TryGetGlobalID(lid));
  return idmap[lid];
}
ConstantID NomConstantContext::AddClass(LocalConstantID lid,
                                        const LocalConstantID library,
                                        const LocalConstantID name) {
  idmap[lid] = NomConstants::AddClass(GetGlobalID(library), GetGlobalID(name),
                                      TryGetGlobalID(lid));
  return idmap[lid];
}

ConstantID NomConstantContext::AddInterface(LocalConstantID lid,
                                            const LocalConstantID library,
                                            const LocalConstantID name) {
  idmap[lid] = NomConstants::AddInterface(
      GetGlobalID(library), GetGlobalID(name), TryGetGlobalID(lid));
  return idmap[lid];
}
...
ConstantID NomConstantContext::AddSuperClass(LocalConstantID lid,
                                             const LocalConstantID cls,
                                             const LocalConstantID args) {
  idmap[lid] = NomConstants::AddSuperClass(GetGlobalID(cls), GetGlobalID(args),
                                           TryGetGlobalID(lid));
  return idmap[lid];
}

ConstantID NomConstantContext::AddSuperInterfaces(
    LocalConstantID lid,
    const llvm::SmallVector<std::tuple<LocalConstantID, LocalConstantID>, 4>
        &entries) {
  llvm::SmallVector<std::tuple<ConstantID, ConstantID>, 4> gentries;
  for (size_t i = 0; i < entries.size(); i++) {
    gentries.emplace_back(GetGlobalID(std::get<0>(entries[i])),
                          GetGlobalID(std::get<1>(entries[i])));
  }
  idmap[lid] = NomConstants::AddSuperInterfaces(std::move(gentries),
                                                TryGetGlobalID(lid));
  return idmap[lid];
}

ConstantID NomConstantContext::AddMethod(LocalConstantID lid,
                                         const LocalConstantID cls,
                                         const LocalConstantID name,
                                         const LocalConstantID typeArgs,
                                         const LocalConstantID argTypes) {
  idmap[lid] = NomConstants::AddMethod(
      GetGlobalID(cls), GetGlobalID(name), GetGlobalID(typeArgs),
      GetGlobalID(argTypes), TryGetGlobalID(lid));
  return idmap[lid];
}
ConstantID NomConstantContext::AddStaticMethod(LocalConstantID lid,
                                               const LocalConstantID cls,
                                               const LocalConstantID name,
                                               const LocalConstantID typeArgs,
                                               const LocalConstantID argTypes) {
  idmap[lid] = NomConstants::AddStaticMethod(
      GetGlobalID(cls), GetGlobalID(name), GetGlobalID(typeArgs),
      GetGlobalID(argTypes), TryGetGlobalID(lid));
  return idmap[lid];
}
ConstantID NomConstantContext::AddTypeList(
    LocalConstantID lid, const llvm::SmallVector<LocalConstantID, 8> types) {
...
ConstantID NomConstantContext::AddClassType(LocalConstantID lid,
                                            const LocalConstantID cls,
                                            const LocalConstantID typeArgs) {
  idmap[lid] = NomConstants::AddClassType(
      GetGlobalID(cls), GetGlobalID(typeArgs), TryGetGlobalID(lid));
  return idmap[lid];
}
...
ConstantID NomConstantContext::AddTypeParameters(
    LocalConstantID lid,
    llvm::ArrayRef<NomTypeParameterConstant *> parameters) {
  ...
  //	{
  //		argarr[i] = new NomTypeParameterConstant(parameters[i].Variance,
  // GetGlobalID(parameters[i].LowerBound),
  // GetGlobalID(parameters[i].UpperBound));
  //	}
  // }
  idmap[lid] = NomConstants::AddTypeParameters(
      /*llvm::ArrayRef<NomTypeParameterConstant *>(argarr, argsize)*/
      parameters, TryGetGlobalID(lid));
  return idmap[lid];
}

ConstantID NomConstantContext::AddConstructor(LocalConstantID lid,
                                              const LocalConstantID cls,
                                              const LocalConstantID typeArgs,
                                              const LocalConstantID argTypes) {
  idmap[lid] =
      NomConstants::AddStaticMethod(GetGlobalID(cls), GetGlobalID(typeArgs),
                                    GetGlobalID(argTypes), TryGetGlobalID(lid));
  return idmap[lid];
}

ConstantID NomConstantContext::AddMaybeType(LocalConstantID lid,
                                            const LocalConstantID ptype) {
  idmap[lid] = NomConstants::AddMaybeType(GetGlobalID(ptype));
  return idmap[lid];
}

NomClassTypeRef
NomClassTypeConstant::GetClassType(NomSubstitutionContextRef context) const {
  // TODO: possible memleak
  NomConstantType ct = NomConstants::Get(this->cls)->Type;
  if (ct == NomConstantType::CTClass) {
    value = NomConstants::GetClass(this->cls)->GetClass()->GetType(
        NomConstants::GetTypeList(this->args)->GetTypeList(context));
  } else {
    value = NomConstants::GetInterface(this->cls)->GetInterface()->GetType(
        NomConstants::GetTypeList(this->args)->GetTypeList(context));
  }
  return value;
}
...
NomInstantiationRef<NomClass>
NomSuperClassConstant::GetClassType(NomSubstitutionContextRef context) const {
  if (value.Elem == nullptr) {
    value = NomInstantiationRef<NomClass>(
        NomConstants::GetClass(this->SuperClass)->GetClass(),
        NomConstants::GetTypeList(this->Arguments)->GetTypeList(context));
  }
  return value;
}
...
NomMethodConstant::GetMethod(NomSubstitutionContextRef context) const {
  // TODO : possible memleak
  {
    auto clstype =
        NomConstants::GetClassType(classConstant)->GetClassType(context);
    ...
    if (clstype->Named->IsInterface()) {
      method =
          (dynamic_cast<const NomInterface *>(clstype->Named))
              ->GetMethod(
                  &nsc, NomConstants::GetString(methodName)->GetText(), targs,
                  NomConstants::GetTypeList(argTypes)->GetTypeList(&nsc));
    } else {
      method =
          (dynamic_cast<const NomClass *>(clstype->Named))
              ->GetMethod(
                  &nsc, NomConstants::GetString(methodName)->GetText(), targs,
                  NomConstants::GetTypeList(argTypes)->GetTypeList(&nsc));
    }
...
NomStaticMethodConstant::GetStaticMethod(
    NomSubstitutionContextRef context) const {
  // TODO : MEMLEAK
  // MEMLEAK : AAAH
  // if (method.Elem == nullptr)
  {
    method = NomConstants::GetClass(ClassConstant)
                 ->GetClass()
                 ->GetStaticMethod(
                     NomConstants::GetString(MethodName)->GetText(),
                     NomConstants::GetTypeList(TypeArgs)->GetTypeList(context),
                     NomConstants::GetTypeList(ArgTypes)->GetTypeList(context));
  }
  return method;
}
...
const NomClass *NomClassConstant::GetClass() {
  if (cls == nullptr) {
    ...
    ////cls =
    /// NomClass::getClass(((NomStringConstant*)NomConstants::Get(Name))->GetText());
    // auto libname =
    // NomConstants::GetString(this->Library)->GetText()->ToStdString(); auto
    // clsname = NomConstants::GetString(this->Name)->GetText(); cls =
    // Loader::GetInstance()->GetLibrary(&libname)->GetClass(clsname);
  }
  return cls;
}
...
void NomClassConstant::EnsureClassLoaded(NomModule *mod) {
  if (cls == nullptr) {
    ...
    // NomClass::getClass(((NomStringConstant*)NomConstants::Get(Name))->GetText());
    auto libname =
        NomConstants::GetString(this->Library)->GetText()->ToStdString();
...
llvm::ArrayRef<NomTypeRef>
NomTypeListConstant::GetTypeList(NomSubstitutionContextRef context)
    const // delete copy-constructors, default move-constructors
{
  // TODO : MEMLEAK
  // MEMLEAK : AAAH
  // if (typeRefs == nullptr)
...
const NomInterface *NomInterfaceConstant::GetInterface() const {
  if (iface == nullptr) {
    throw new std::exception();
    //	//iface =
    // NomInterface::GetInterface(((NomStringConstant*)NomConstants::Get(Name))->GetText());
    //	auto libname =
    // NomConstants::GetString(Library)->GetText()->ToStdString(); 	auto
    // ifacename = NomConstants::GetString(Name)->GetText(); 	iface =
    // Loader::GetInstance()->GetLibrary(&libname)->GetInterface(ifacename);
  }
  return iface;
}
...
//	if (constant == 0)
//	{
//		static NomClassTypeListConstant defaultConst{
// llvm::SmallVector<ConstantID, 8>() }; 		return &defaultConst;
//	}
...
NomStringConstant *NomConstants::GetString(const ConstantID constant) {
  if (constant == 0) {
    static NomStringConstant emptyStringConstant =
        NomStringConstant(NomString(""));
...
NomStaticMethodConstant *
NomConstants::GetStaticMethod(const ConstantID constant) {
  NomConstant *cnstnt = constants()[constant];
...
NomConstructorConstant *
NomConstants::GetConstructor(const ConstantID constant) {
  NomConstant *cnstnt = constants()[constant];
...
NomSuperInterfacesConstant *
NomConstants::GetSuperInterfaces(const ConstantID constant) {
  if (constant == 0) {
...
NomTypeParametersConstant *
NomConstants::GetTypeParameters(const ConstantID constant) {
  if (constant == 0) {
    static NomTypeParametersConstant defaultConst(
        llvm::ArrayRef<NomTypeParameterConstant *>(
            (NomTypeParameterConstant **)nullptr, (size_t)0));
    return &defaultConst;
  }
  NomConstant *cnstnt = constants()[constant];
...
NomTypeRef NomConstants::GetType(NomSubstitutionContextRef context,
...
ConstantID NomConstants::AddClass(const ConstantID library,
                                  const ConstantID name, ConstantID cid) {
  if (cid == 0) {
    cid = GetConstantID();
  }
...
ConstantID NomConstants::AddInterface(const ConstantID library,
                                      const ConstantID name, ConstantID cid) {
  if (cid == 0) {
    cid = GetConstantID();
  }
...
// static ConstantID AddTypeParameter(ParameterVariance variance, const
// ConstantID lowerBound, const ConstantID upperBound, ConstantID cid = 0)
//{
//	if (cid == 0)
//	{
//		cid = GetConstantID();
//	}
//	constants()[cid] = (new NomTypeParameterConstant(variance, lowerBound,
// upperBound)); 	return cid;
// }

ConstantID NomConstants::AddMethod(const ConstantID cls, const ConstantID name,
                                   const ConstantID typeArgs,
                                   const ConstantID argTypes, ConstantID cid) {
  if (cid == 0) {
    cid = GetConstantID();
  }
...
ConstantID NomConstants::AddStaticMethod(const ConstantID cls,
  ...
  constants()[cid] =
      (new NomStaticMethodConstant(cls, name, typeArgs, argTypes));
  return cid;
}

ConstantID NomConstants::AddConstructor(const ConstantID cls,
                                        const ConstantID typeArgs,
                                        const ConstantID argTypes,
                                        ConstantID cid) {
  if (cid == 0) {
...
ConstantID NomConstants::AddTypeList(const llvm::ArrayRef<ConstantID> types,
                                     ConstantID cid) {
  if (cid == 0) {
    cid = GetConstantID();
  }
...
ConstantID NomConstants::AddClassType(const ConstantID cls,
                                      const ConstantID typeArgs,
                                      ConstantID cid) {
  if (cid == 0) {
    cid = GetConstantID();
...
ConstantID NomConstants::AddTypeParameters(
    llvm::ArrayRef<NomTypeParameterConstant *> typeParameters, ConstantID cid) {
  if (cid == 0) {
    cid = GetConstantID();
  }
...
ConstantID NomConstants::AddSuperClass(const ConstantID cls,
                                       const ConstantID args, ConstantID cid) {
  if (cid == 0) {
    cid = GetConstantID();
  }
...
ConstantID NomConstants::AddSuperInterfaces(
...
NomConstructorConstant::GetConstructor(
    NomSubstitutionContextRef context) const {
  if (constructor.Elem == nullptr) {
    constructor =
        NomConstants::GetClass(ClassConstant)
            ->GetClass()
            ->GetConstructor(
                NomConstants::GetTypeList(TypeArgs)->GetTypeList(context),
                NomConstants::GetTypeList(ArgTypes)->GetTypeList(context));
...
size_t NomTypeListConstant::GetSize() const { return types.size(); }
const llvm::ArrayRef<NomInstantiationRef<NomInterface>>
NomSuperInterfacesConstant::GetSuperInterfaces(
    NomSubstitutionContextRef context) const {
  if (ifaces.size() == 0 && entries.size() != 0) {
    for (auto entry : entries) {
      ifaces.push_back(NomInstantiationRef<NomInterface>(
          NomConstants::GetInterface(std::get<0>(entry))->GetInterface(),
          NomConstants::GetTypeList(std::get<1>(entry))->GetTypeList(context)));
    }
  }
  return ifaces;
}
...
NomTypeRef
NomTypeVarConstant::GetTypeRef(NomSubstitutionContextRef context) const {
  return context->GetTypeVariable(index);
}
void NomTypeVarConstant::Print(bool resolve) {
  cout << "TypeVar " << std::dec << index;
}
NomLambda *NomLambdaConstant::GetLambda() const { return lambda; }
...
void NomLambdaConstant::Print(bool resolve) { cout << "Lambda"; }
...
void NomBottomConstant::Print(bool resolve) { cout << "Bottom"; }
void NomDynamicTypeConstant::Print(bool resolve) { cout << "Dyn"; }
NomRecord *NomRecordConstant::GetRecord() const { return this->structure; }
void NomRecordConstant::SetStruct(NomRecord *structure) const {
  this->structure = structure;
}
void NomRecordConstant::Print(bool resolve) { cout << "Record"; }
NomMaybeTypeRef
NomMaybeTypeConstant::GetTypeRef(NomSubstitutionContextRef context) const {
  if (value == nullptr) {
    value = NomMaybeType::GetMaybeType(
        NomConstants::GetType(context, ptype, false));
  }
  return value;
}
...
void NomMaybeTypeConstant::FillConstantDependencies(
