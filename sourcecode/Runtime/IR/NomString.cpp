...
void *NomString::GetStringObject() const {
  void *ret =
      (void *)(((char **)gcalloc(ObjectHeader::SizeOf() + sizeof(char *))) + 1);
  GetWriteVTableFunction()(ret, *((void **)(NomJIT::Instance()
                                                .lookup("NOM_CDREF_String_0")
                                                ->getAddress())));
...
const std::string *getStringID(NomStringRef str) {
  static std::unordered_map<NomStringRef, std::string, NomStringHash,
                            NomStringEquality>
      stringKeys;
...
llvm::Constant *
NomString::createLLVMElement(llvm::Module &mod,
                             llvm::GlobalValue::LinkageTypes linkage) const {
  return ObjectHeader::GetGlobal(
      mod, linkage, llvm::Twine(getStringID(this)->data()),
      NomStringClass::GetInstance()->GetLLVMElement(mod),
      llvm::ConstantArray::get(arrtype(REFTYPE, 1),
                               {llvm::ConstantExpr::getIntToPtr(
                                   MakeInt((const intptr_t)this), REFTYPE)}));
}
