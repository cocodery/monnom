...
template <typename T> class NomValueContainer {
private:
  T *val;
  NomTypeRef type;
  bool isFunctionCall;
...
public:
  ...
  NomValueContainer(T *val, NomTypeRef type, bool isFunctionCall = false)
      : val(val), type(type), isFunctionCall(isFunctionCall) {
#ifdef _DEBUG
    __NomValueContainerDebugCheck(type, val->getType());
#endif
  }
  NomValueContainer(T *val, bool isFunctionCall = false)
      : val(val), type(GetDynOrPrimitiveType(val)),
  ...
  NomTypeRef GetNomType() const { return type; }
  bool IsFunctionCall() const { return isFunctionCall; }
