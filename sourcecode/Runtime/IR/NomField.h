...
public:
  ...
  virtual NomValue GenerateRead(NomBuilder &builder, CompileEnv *env,
                                NomValue receiver) const = 0;
  virtual void GenerateWrite(NomBuilder &builder, CompileEnv *env,
                             NomValue receiver, NomValue value) const = 0;
...
public:
  NomTypedField(NomClass *cls, const ConstantID name, const ConstantID type,
                Visibility visibility, bool readonly, bool isvolatile);
  virtual ~NomTypedField() override;

  virtual NomTypeRef GetType() const override;
  virtual NomStringRef GetName() const override;
  virtual NomValue GenerateRead(NomBuilder &builder, CompileEnv *env,
                                NomValue receiver) const override;
  virtual void GenerateWrite(NomBuilder &builder, CompileEnv *env,
                             NomValue receiver, NomValue value) const override;
...
public:
  NomClosureField(NomLambda *lambda, const ConstantID name,
...
public:
  NomRecordField(NomRecord *structure, const ConstantID name,
                 const ConstantID type, bool isReadOnly, const int index,
