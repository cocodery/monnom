...
struct RTTypeArrHash {
  constexpr size_t
  operator()(const std::tuple<void **, int, size_t> &_Keyval) const noexcept {
    return std::get<2>(_Keyval);
  }
};
struct RTTypeArrEquals {
  constexpr bool
  operator()(const std::tuple<void **, int, size_t> &left,
             const std::tuple<void **, int, size_t> &right) const noexcept {
...
using RuntimeInstantiationDictionary = std::unordered_map<
    std::tuple<void **, int, size_t>, void *, RTTypeArrHash, RTTypeArrEquals,
    BoehmAllocator<std::pair<const std::tuple<void **, int, size_t>, void *>>>;
...
private:
  ...
  static std::unordered_map<NomStringRef, NomInterface *, NomStringHash,
                            NomStringEquality> &
  interfaces() {
    static std::unordered_map<NomStringRef, NomInterface *, NomStringHash,
                              NomStringEquality>
...
public:
  ...
  virtual bool IsFunctional() const override {
    return Methods.size() == 1 && Methods[0]->GetName().empty();
  }
  ...
  void AddMethod(NomMethod *method) { Methods.push_back(method); }

  virtual const llvm::SmallVector<NomClassTypeRef, 16>
  GetSuperNameds(llvm::ArrayRef<NomTypeRef> args) const override;
  virtual const llvm::ArrayRef<NomInstantiationRef<NomInterface>>
  GetSuperInterfaces(const NomSubstitutionContext *context = nullptr) const = 0;
  void
  AddInstantiation(const NomInstantiationRef<NomInterface> instantiation) const;
  const std::unordered_map<const NomInterface *, TypeList> &
  GetInstantiations() const;

  NomInstantiationRef<const NomMethod>
  GetMethod(const NomSubstitutionContext *context, NomStringRef methodName,
            const TypeList typeArgs, const TypeList argTypes) const;

  virtual llvm::Constant *
  GetMethodTable(llvm::Module &mod,
  ...
  virtual llvm::Constant *
  GetSignature(llvm::Module &mod,
               llvm::GlobalValue::LinkageTypes linkage) const;
  virtual llvm::Constant *
  GetCheckReturnTypeFunction(llvm::Module &mod,
                             llvm::GlobalValue::LinkageTypes linkage) const;
  virtual llvm::Constant *
...
public:
  ...
  NomInterfaceLoaded(NomInterfaceLoaded &&) = delete;
  NomInterfaceLoaded(const ConstantID name, ConstantID typeArgs,
                     const ConstantID superInterfaces,
                     const NomMemberContext *parent);
  virtual ~NomInterfaceLoaded() override = default;

  NomMethodLoaded *AddMethod(const std::string &name, const std::string &qname,
                             const ConstantID typeParameters,
                             const ConstantID returnType,
                             const ConstantID argumentTypes,
                             const RegIndex regcount, bool isFinal);
...
class NomInterfaceInternal : public virtual NomInterface,
                             public NomNamedInternal {
private:
  ...
  llvm::ArrayRef<NomInstantiationRef<NomInterface>> superInterfaces =
      llvm::ArrayRef<NomInstantiationRef<NomInterface>>(
          (NomInstantiationRef<NomInterface> *)nullptr, (size_t)0);

protected:
  NomInterfaceInternal(NomStringRef name,
                       const NomMemberContext *parent = nullptr);
  virtual void ResolveDependencies(NomModule *mod) const override;
...
public:
  ...
  virtual void GetClassDependencies(
      llvm::SmallVector<const NomClassInternal *, 4> &results) const;
  virtual void GetInterfaceDependencies(
