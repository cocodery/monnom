...
namespace Nom {
namespace Runtime {
NomNamedLoaded::NomNamedLoaded(const ConstantID name, ConstantID typeArgs,
                               const NomMemberContext *parent)
    : NomMemberContextLoaded(parent, typeArgs), name(name) {
...
NomNamed *NomNamed::GetNamed(NomStringRef name) { return namedTypes()[name]; }
const std::string NomNamed::GetSymbolRep() const {
  return GetName()->ToStdString();
}
intptr_t NomNamed::GetRTElement() const {
  return (intptr_t)NomJIT::Instance()
      .lookup("NOM_CD_" + (GetName()->ToStdString()))
      ->getAddress();
}
...
NomClassTypeRef NomNamed::GetType(llvm::ArrayRef<NomTypeRef> args) const {
  ...
  if (instances.count(args) == 0) {
    NomTypeRef *argsarr =
        (NomTypeRef *)(nmalloc(sizeof(NomTypeRef) * args.size()));
...
const std::string *NomNamedInternal::GetSymbolName() const { return &symname; }
NomNamedInternal::NomNamedInternal(NomStringRef name,
                                   const NomMemberContext *parent)
    : NomMemberContextInternal(parent), name(name),
      symname(name->ToStdString()) {}
NomStringRef NomNamedInternal::GetName() const { return name; }
} // namespace Runtime
} // namespace Nom