...
private:
  ...
  // mutable TypeList argumentTypes = TypeList((NomTypeRef *)nullptr,
  // (size_t)0);
public:
  NomLambda *const Parent;
  // ConstantID ArgTypes;
  ConstantID ReturnType;
  NomLambdaBody(NomLambda *parent, const RegIndex regcount,
  ...
  // virtual TypeList GetArgumentTypes(const NomSubstitutionContext* context)
  // const override; virtual int GetArgumentCount() const override;
...
public:
  const ConstantID ID;
  NomLambdaBody Body;
  NomLambda(ConstantID id, const NomMemberContext *parent,
