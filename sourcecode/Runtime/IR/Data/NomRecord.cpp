...
namespace Nom {
namespace Runtime {
NomRecord::NomRecord(ConstantID structID, NomMemberContext *context,
                     ConstantID closureTypeParams, RegIndex registerCount,
                     RegIndex endargregcount, ConstantID initializerArgTypes)
    : NomCallableLoaded("STRUCT_" + to_string(structID), context,
                        "STRUCT_" + to_string(structID) + ".$Constructor",
                        registerCount, closureTypeParams, initializerArgTypes),
      StructID(structID),
      /*InitializerArgTypes(initializerArgTypes),*/ EndArgRegisterCount(
          endargregcount) {
...
llvm::Function *
NomRecord::createLLVMElement(llvm::Module &mod,
                             llvm::GlobalValue::LinkageTypes linkage) const {
  Function *fun =
      Function::Create(GetLLVMFunctionType(), linkage, *GetSymbolName(), &mod);
  ...
  auto ddstructType = StructType::create(LLVMCONTEXT, "MONNOM_RT_STRUCTDD_" +
                                                          to_string(StructID));
  auto gvartype =
      StructType::get(LLVMCONTEXT, {arrtype(inttype(64), hasRawInvoke ? 1 : 0),
                                    RTRecord::GetLLVMType(), ddstructType});
  Constant *pushArr[1] = {MakeUInt(64, 0)};
  GlobalVariable *gv =
      new GlobalVariable(mod, gvartype, true, linkage, nullptr,
                         "RT_NOM_STRUCTDESC_" + to_string(StructID));
  ...
  for (decltype(IMTsize) i = 0; i < IMTsize; i++) {
    ddarr[i] = ConstantExpr::getPointerCast(
        ConstantExpr::getGetElementPtr(
            gvartype, gv,
            ArrayRef<Constant *>({MakeInt32(0), MakeInt32(2), MakeInt32(i)})),
        GetDynamicDispatchListEntryType()->getPointerTo());
  }

  auto ddreftable = ConstantArray::get(
  ...
  gv->setInitializer(ConstantStruct::get(
      ...
      {ConstantArray::get(arrtype(inttype(64), hasRawInvoke ? 1 : 0),
                          ArrayRef<Constant *>(pushArr, hasRawInvoke ? 1 : 0)),
       constant, ddtable}));

  StructInstantiationCompileEnv sice = StructInstantiationCompileEnv(
      regcount, fun, GetAllTypeParameters(), GetArgumentTypes(nullptr), this,
      EndArgRegisterCount);

  RecordHeader::GenerateConstructorCode(
      builder, ArrayRef<Value *>(typeArgBuf, targc), &sice,
      ConstantExpr::getGetElementPtr(
          gvartype, gv, ArrayRef<Constant *>({MakeInt32(0), MakeInt32(1)})),
      GetInstructions());

  return fun;
}
NomTypeRef
NomRecord::GetReturnType(const NomSubstitutionContext *context) const {
...
NomRecordMethod *NomRecord::AddMethod(std::string &name, std::string &qname,
                                      ConstantID typeParameters,
                                      ConstantID returnType,
                                      ConstantID argTypes, RegIndex regcount) {
  NomRecordMethod *meth = new NomRecordMethod(this, name, qname, typeParameters,
                                              returnType, argTypes, regcount);
  Methods.push_back(meth);
  return meth;
}

NomRecordField *NomRecord::AddField(const ConstantID name,
                                    const ConstantID type, bool isReadOnly,
                                    RegIndex valueRegister) {
  NomRecordField *field = new NomRecordField(this, name, type, isReadOnly,
                                             Fields.size(), valueRegister);
  Fields.push_back(field);
  return field;
}

llvm::Function *NomRecord::GetDynamicFieldLookup(
  ...
  if (fun == nullptr) {
    ...
    auto nameSwitch =
        builder->CreateSwitch(namearg, notfound, this->Fields.size());
    ...
    for (auto field : Fields) {
      ...
      builder->CreateRet(field->GenerateRead(
          builder, &scce,
          NomValue(
              thisarg,
              thisType))); // removed EnsurePacked from here, because struct
...
llvm::Function *
NomRecord::GetDynamicFieldStore(llvm::Module &mod,
                                llvm::GlobalValue::LinkageTypes linkage) const {
  auto targcount = this->GetDirectTypeParametersCount();
  NomTypeRef *selfArgsBuf = makealloca(NomTypeRef, targcount);
  for (decltype(targcount) i = 0; i < targcount; i++) {
...
llvm::Constant *NomRecord::GetInterfaceTableLookup(
    llvm::Module &mod, llvm::GlobalValue::LinkageTypes linkage) const {
  auto rettype = arrtype(GetIMTFunctionType()->getPointerTo(), IMTsize);
  auto retarr = makealloca(Constant *, IMTsize);

  for (decltype(IMTsize) i = 0; i < IMTsize; i++) {
    auto fun = Function::Create(GetIMTFunctionType(), linkage,
                                "MONNOM_RT_RECORDIMT_" + *GetSymbolName() +
                                    "_" + std::to_string(i),
                                mod);
    fun->setCallingConv(NOMCC);
    BasicBlock *startBlock = BasicBlock::Create(LLVMCONTEXT, "", fun);
    ...
    for (auto &meth : Methods) {
      if (NomNameRepository::Instance().GetNameID(meth->GetName()) % IMTsize ==
        ...
        //                     {lhs, llvm::ConstantInt::get(INTTYPE, 0,
        //                     false)});

        // builder->CreateCall(GetPrint(&mod),
        //                     {rhs, llvm::ConstantInt::get(INTTYPE, 0,
        //                     false)});
        ...
        CastedValueCompileEnv cvce = CastedValueCompileEnv(
            meth->GetDirectTypeParameters(), this->GetAllTypeParameters(), fun,
            2, paramCount,
    ...
    // builder->CreateCall(GetPrint(&mod),
    //                     {builder->CreatePtrToInt(callTagFun,
    //                     numtype(intptr_t)),
...
llvm::Constant *
NomRecord::GetDynamicDispatcherLookup(llvm::Module &mod,
  ...
  for (decltype(IMTsize) i = 0; i < IMTsize; i++) {
    ...
    for (auto &meth : methods) {
      ...
      std::string *errormsg =
          new std::string("Called record method " + meth->GetName() +
    ...
    auto bigptr = ConstantExpr::getGetElementPtr(
        RecordHeader::GetLLVMType(Fields.size()),
        ConstantPointerNull::get(
            RecordHeader::GetLLVMType(Fields.size())->getPointerTo()),
        ArrayRef<Constant *>(
            {MakeInt32(0), MakeInt32(StructHeaderFields::Fields)}));
    auto littleptr = ConstantExpr::getGetElementPtr(
        RecordHeader::GetLLVMType(),
        ConstantPointerNull::get(RecordHeader::GetLLVMType()->getPointerTo()),
        ArrayRef<Constant *>(
            {MakeInt32(0), MakeInt32(StructHeaderFields::Fields)}));
    auto fieldsOffset = ConstantExpr::getUDiv(
        ConstantExpr::getSub(
            ConstantExpr::getPtrToInt(bigptr, numtype(size_t)),
            ConstantExpr::getPtrToInt(littleptr, numtype(size_t))),
        MakeInt<size_t>(8));
    for (auto &field : fields) {
      entries[entryID] = GetDynamicDispatchListEntryConstant(
          ...
          ConstantExpr::getIntToPtr(
              ConstantExpr::getAdd(
                  MakeInt<size_t>((((size_t)(field->Index)) << 32)),
                  fieldsOffset),
...
llvm::FunctionType *
NomRecord::GetLLVMFunctionType(const NomSubstitutionContext *context) const {
...
llvm::ArrayRef<NomTypeParameterRef>
NomRecord::GetArgumentTypeParameters() const {
