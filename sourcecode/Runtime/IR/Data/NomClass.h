...
private:
  static auto &classes() {
    static std::unordered_map<NomStringRef, NomClass *, NomStringHash,
                              NomStringEquality>
        classes;
    return classes;
  }
...
public:
  ...
  virtual const NomInstantiationRef<NomClass>
  GetSuperClass(const NomSubstitutionContext *context = nullptr) const = 0;
  ...
  NomValue GenerateConstructorCall(NomBuilder &builder, CompileEnv *env,
                                   const TypeList typeArgs,
                                   llvm::Value *objpointer,
                                   llvm::ArrayRef<NomValue> args) const;
  NomInstantiationRef<const NomStaticMethod>
  GetStaticMethod(NomStringRef methodName, const TypeList typeArgs,
                  const TypeList argTypes) const;

  NomInstantiationRef<const NomConstructor>
  GetConstructor(const TypeList typeArgs, const TypeList argTypes) const;

public:
  virtual llvm::Constant *
  GetSuperInstances(llvm::Module &mod, llvm::GlobalValue::LinkageTypes linkage,
                    llvm::GlobalVariable *gvar,
                    llvm::StructType *stetype) const override;
  virtual llvm::ArrayType *
  GetSuperInstancesType(bool generic = true) const override;
  virtual const llvm::SmallVector<NomClassTypeRef, 16>
  GetSuperNameds(llvm::ArrayRef<NomTypeRef> args) const override;

  llvm::Constant *
  GetInterfaceTableLookup(llvm::Module &mod,
  ...
  llvm::Function *
  GetRawInvokeFunction(llvm::Module &mod,
                       llvm::GlobalValue::LinkageTypes linkage) const;
  ...
  virtual llvm::Constant *
  GetInterfaceDescriptor(llvm::Module &mod) const override;
  virtual llvm::Constant *
  GetCastFunction(llvm::Module &mod,
                  llvm::GlobalValue::LinkageTypes linkage) const override;
  virtual int GetSuperClassCount() const override;

protected:
  virtual llvm::Constant *
  GetDynamicDispatcherLookup(llvm::Module &mod,
                             llvm::GlobalValue::LinkageTypes linkage,
                             llvm::StructType *stype) const;
  virtual llvm::Function *
  GetDynamicFieldLookup(llvm::Module &mod,
                        llvm::GlobalValue::LinkageTypes linkage) const;
  virtual llvm::Function *
  GetDynamicFieldStore(llvm::Module &mod,
                       llvm::GlobalValue::LinkageTypes linkage) const;
...
public:
  ...
  void AddLambda(NomLambda *lambda) { Lambdas.push_back(lambda); }
  void AddStruct(NomRecord *strct) { Structs.push_back(strct); }
  void AddField(NomTypedField *field) { Fields.push_back(field); }
};
...
public:
  ...
  NomClassLoaded(const ConstantID name, ConstantID typeArgs,
                 ConstantID superClass, const ConstantID superInterfaces,
                 const NomMemberContext *parent);
  virtual ~NomClassLoaded() override = default;

  virtual const NomInstantiationRef<NomClass>
  GetSuperClass(const NomSubstitutionContext *context = nullptr) const override;

  NomStaticMethodLoaded *
  AddStaticMethod(const std::string &name, const std::string &qname,
                  const ConstantID typeArgs, const ConstantID returnType,
                  const ConstantID arguments, const RegIndex regcount);

  NomConstructorLoaded *AddConstructor(const ConstantID arguments,
                                       const RegIndex regcount);

  NomTypedField *AddField(const ConstantID name, const ConstantID type,
                          Visibility visibility, bool isReadOnly,
                          bool isVolatile);

  NomLambda *AddLambda(const ConstantID lambdaID, int regcount,
                       ConstantID closureTypeParams,
                       ConstantID closureArguments, ConstantID typeParams,
                       ConstantID argTypes, ConstantID returnType);
  NomRecord *AddStruct(const ConstantID structID, ConstantID closureTypeParams,
