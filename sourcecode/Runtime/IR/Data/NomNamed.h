...
class NomNamed : public AvailableExternally<llvm::Constant>,
                 public virtual NomMemberContext {
private:
  static std::unordered_map<NomStringRef, NomNamed *, NomStringHash,
                            NomStringEquality> &
  namedTypes() {
    static std::unordered_map<NomStringRef, NomNamed *, NomStringHash,
                              NomStringEquality>
  ...
  mutable std::unordered_map<
      llvm::ArrayRef<NomTypeRef>, NomClassTypeRef, NomTypeRefArrayRefHash,
      NomTypeRefArrayRefEquality,
      BoehmAllocator<
          std::pair<const llvm::ArrayRef<NomTypeRef>, NomClassTypeRef>>>
      instances;
...
public:
  ...
  GetSuperNameds(llvm::ArrayRef<NomTypeRef> args) const = 0;
...
public:
  NomNamedLoaded(const ConstantID name, const ConstantID typeArgs,
...
class NomNamedInternal : public virtual NomNamed,
