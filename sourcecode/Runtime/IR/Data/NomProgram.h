...
public:
  ...
  NomClassLoaded *AddClass(const ConstantID name, const ConstantID typeArgs,
                           const ConstantID superClass,
                           const ConstantID superInterfaces,
                           const NomMemberContext *parent) {
    auto elem =
        new NomClassLoaded(name, typeArgs, superClass, superInterfaces, parent);
    classes.push_back(elem);
    return elem;
  }
  NomInterfaceLoaded *AddInterface(const ConstantID name,
  ...
  LibraryVersion *AddLibraryVersion(
      const std::string *name, const std::string &mainclassname,
      const VersionNumber &version, const VersionNumber &compatibleFrom,
      const VersionNumber &compatibleTo, bool isDeprecated, bool isSecurityRisk,
      const std::vector<LibraryDependency> &&dependencies,
      ComponentSource *componentSource) {
    auto elem = new LibraryVersion(name, mainclassname, version, compatibleFrom,
                                   compatibleTo, isDeprecated, isSecurityRisk,
                                   std::move(dependencies), componentSource);
