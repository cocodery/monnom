...
NomClassLoaded::NomClassLoaded(const ConstantID name, ConstantID typeArgs,
                               ConstantID superClass,
                               const ConstantID superInterfaces,
                               const NomMemberContext *parent)
    : NomInterface(NomConstants::GetString(name)->GetText()->ToStdString()),
      NomInterfaceLoaded(name, typeArgs, superInterfaces, parent),
      /*NomMemberContextLoaded(parent, typeArgs), NomNamedLoaded(name, typeArgs,
         parent),*/
      superClass(superClass) {
  RegisterClass(NomConstants::GetString(name)->GetText(), this);
}

NomStaticMethodLoaded *NomClassLoaded::AddStaticMethod(
    const std::string &name, const std::string &qname,
    const ConstantID typeArgs, const ConstantID returnType,
    const ConstantID arguments, const RegIndex regcount) {
  NomStaticMethodLoaded *meth = new NomStaticMethodLoaded(
      name, this, qname, returnType, typeArgs, arguments, regcount);
  NomClass::AddStaticMethod(meth);
  return meth;
}

NomConstructorLoaded *NomClassLoaded::AddConstructor(const ConstantID arguments,
                                                     const RegIndex regcount) {
  auto name = "_Constructor_" + this->GetName()->ToStdString() + "_" +
              std::to_string(Constructors.size());
  NomConstructorLoaded *constr =
      new NomConstructorLoaded(this, name, name, arguments, regcount, 0, false);
  NomClass::AddConstructor(constr);
  return constr;
}

NomTypedField *NomClassLoaded::AddField(const ConstantID name,
                                        const ConstantID type,
                                        Visibility visibility, bool isReadOnly,
                                        bool isVolatile) {
  NomTypedField *field =
      new NomTypedField(this, name, type, visibility, isReadOnly, isVolatile);
  NomClass::AddField(field);
  return field;
}

NomLambda *NomClassLoaded::AddLambda(const ConstantID lambdaID, int regcount,
                                     ConstantID closureTypeParams,
                                     ConstantID closureArguments,
                                     ConstantID typeParams, ConstantID argTypes,
                                     ConstantID returnType) {
  NomLambda *lambda =
      new NomLambda(lambdaID, nullptr, regcount, closureTypeParams,
                    closureArguments, typeParams, argTypes, returnType);
  NomClass::AddLambda(lambda);
  return lambda;
}

NomRecord *NomClassLoaded::AddStruct(const ConstantID structID,
                                     ConstantID closureTypeParams,
                                     RegIndex regcount, RegIndex endargregcount,
                                     ConstantID initializerArgTypes) {
  NomRecord *structure =
      new NomRecord(structID, nullptr, closureTypeParams, regcount,
                    endargregcount, initializerArgTypes);
...
NomValue NomClass::GenerateConstructorCall(
    NomBuilder &builder, CompileEnv *env, const TypeList typeArgs,
    llvm::Value *objpointer, llvm::ArrayRef<NomValue> args) const {
  auto argssize = args.size();
  NomTypeRef *argTypesBuf =
      (NomTypeRef *)(nmalloc(sizeof(NomTypeRef) * argssize));
  llvm::Value **argValuesBuf = (llvm::Value **)(nmalloc(
      sizeof(llvm::Value *) * (argssize + 1 + typeArgs.size())));
  size_t argspos = 0;
  ...
  auto argValues = llvm::ArrayRef<llvm::Value *>(
      argValuesBuf, argssize + 1 + typeArgs.size());
  NomSubstitutionContextList nscl(typeArgs);
  for (auto constructor : Constructors) {
    if (constructor->Satisfies(
            &NomSubstitutionContextList::EmptyContext(), typeArgs,
            argTypes)) // TODO: make pessimistic and go to dispatcher later
    {
      ...
      for (size_t i = 0; i < argssize; i++) {
        if (!argTypesBuf[i]->IsSubtype(constructorSignature.ArgumentTypes[i],
                                       false)) {
          argValuesBuf[argspos + i] = CastInstruction::MakeCast(
              builder, env, args[i], constructorSignature.ArgumentTypes[i]);
        }
        auto paramtype = funtype->getParamType(argspos + i);
        if (paramtype->isIntegerTy() &&
            !argValuesBuf[argspos + i]->getType()->isIntegerTy()) {
          argValuesBuf[argspos + i] =
              UnpackInt(builder, argValuesBuf[argspos + i]);
        } else if (paramtype == FLOATTYPE &&
                   argValuesBuf[argspos + i]->getType() != FLOATTYPE) {
          argValuesBuf[argspos + i] =
              UnpackFloat(builder, argValuesBuf[argspos + i]);
        } else if ((!paramtype->isIntegerTy()) &&
                   argValuesBuf[argspos + i]->getType()->isIntegerTy()) {
          argValuesBuf[argspos + i] =
              PackInt(builder, argValuesBuf[argspos + i]);
        } else if (paramtype != FLOATTYPE &&
                   argValuesBuf[argspos + i]->getType() == FLOATTYPE) {
          argValuesBuf[argspos + i] =
              PackFloat(builder, argValuesBuf[argspos + i]);
        }
      }
...
NomInstantiationRef<const NomStaticMethod>
NomClass::GetStaticMethod(NomStringRef methodName, const TypeList typeArgs,
                          const TypeList argTypes) const {
  for (auto meth : StaticMethods) {
    if (meth->GetName() == methodName->ToStdString() &&
        meth->Satisfies(&NomSubstitutionContextList::EmptyContext(), typeArgs,
                        argTypes)) {
...
NomInstantiationRef<const NomConstructor>
NomClass::GetConstructor(const TypeList typeArgs,
                         const TypeList argTypes) const {
  for (auto constructor : Constructors) {
    if (constructor->Satisfies(&NomSubstitutionContextList::EmptyContext(),
                               typeArgs, argTypes)) {
...
llvm::Constant *NomClass::GetSuperInstances(
    llvm::Module &mod, llvm::GlobalValue::LinkageTypes linkage,
    llvm::GlobalVariable *gvar, llvm::StructType *stetype) const {
  ...
  while (curSuper.HasElem()) {
    ...
    fieldArr[pos + 1] =
        ConstantArray::get(arrtype(TYPETYPE, instasize),
                           ArrayRef<Constant *>(instaArgBuf, instasize));
  ...
  for (auto &instantiation : instantiations) {
    if (instantiation.first->IsInterface()) {
      ...
      llvm::Constant **instaArgBuf = makealloca(llvm::Constant *, instasize);
      for (size_t i = 0; i < instasize; i++) {
        instaArgBuf[instasize - (i + 1)] =
            instantiation.second[i]->GetLLVMElement(mod);
      }
      fieldArr[pos + 1] =
          ConstantArray::get(arrtype(TYPETYPE, instasize),
                             ArrayRef<Constant *>(instaArgBuf, instasize));
  ...
  while (pos > 0) {
    ...
    size_t instasize = instantiation.size();
    entryArr[pos] = ConstantStruct::get(
        SuperInstanceEntryType(),
        ConstantExpr::getPointerCast(
            orderedInstas[pos]->GetLLVMElement(mod),
            RTInterface::GetLLVMType()->getPointerTo()),
        ConstantExpr::getGetElementPtr(
            gvar->getValueType(), gvar,
            ArrayRef<Constant *>(
                {MakeInt32(0),
                 MakeInt32(gvar->getValueType()->getStructNumElements() - 1),
                 MakeInt32(pos + 1), MakeInt32(instasize)})));
  }
  fieldArr[0] =
      ConstantArray::get(arrtype(SuperInstanceEntryType(), instaCount),
                         ArrayRef<Constant *>(entryArr, instaCount));
  return ConstantStruct::get(stetype,
                             ArrayRef<Constant *>(fieldArr, instaCount + 1));
}

llvm::ArrayType *NomClass::GetSuperInstancesType(bool generic) const {
  return arrtype(SuperInstanceEntryType(),
                 (generic ? 0 : GetInstantiations().size()));
}
...
llvm::Constant *NomClass::findLLVMElement(llvm::Module &mod) const {
  ...
  llvm::StringRef nameref =
      t.concat(this->GetName()->ToStdString()).toStringRef(buf);
  return RTClass::FindConstant(mod, nameref);
  // return mod.getGlobalVariable(nameref, false);
}
llvm::Constant *
NomClass::createLLVMElement(llvm::Module &mod,
  ...
  if (ret == nullptr) {
    StructType *clsStructType = StructType::create(
        LLVMCONTEXT, "MONNOM_CLSDD_" + *this->GetSymbolName());
    StructType *clsSupersStructType = StructType::create(
        LLVMCONTEXT, "MONNOM_CLSSUPERS_" + *this->GetSymbolName());
    auto gvartype = RTClass::GetConstantType(
        GetHasRawInvoke() ? 24 : 16, GetMethodTableType(false), clsStructType,
        clsSupersStructType);
    GlobalVariable *gvar =
        new GlobalVariable(mod, gvartype, true, linkage, nullptr, nameref);
    auto superClassCount = GetSuperClassCount();
    ret = RTClass::CreateConstant(
        gvar, gvartype, this, GetDynamicFieldLookup(mod, linkage),
        GetDynamicFieldStore(mod, linkage),
        GetDynamicDispatcherLookup(mod, linkage, clsStructType),
        MakeInt(GetFieldCount()), MakeInt(this->GetTypeParametersCount()),
        ...
        ConstantPointerNull::get(
            GetCheckReturnValueFunctionType()->getPointerTo()),
        GetInterfaceTableLookup(mod, linkage), GetSignature(mod, linkage),
        GetCastFunction(mod, linkage));
    new GlobalVariable(mod, ret->getType(), true, linkage, ret,
                       "NOM_CDREF_" + this->GetName()->ToStdString());
...
size_t NomClass::GetTypeArgOffset() const {
  ...
  if (sclass.HasElem()) {
    offset =
        sclass.Elem->GetTypeArgOffset() + sclass.Elem->GetTypeParametersCount();
  }
  return offset;
}
...
llvm::Function *
NomClass::GetRawInvokeFunction(llvm::Module &mod,
                               llvm::GlobalValue::LinkageTypes linkage) const {
  ...
  std::string name = "RT_NOM_RAWINVOKE_" + this->GetName()->ToStdString();
  llvm::Function *fun =
      Function::Create(GetIMTFunctionType(), linkage, name, mod);
  ...
  for (auto &meth : methods) {
    ...
    for (decltype(paramCount) j = 0; j < paramCount; j++) {
      ...
      if (j < RTConfig_NumberOfVarargsArguments ||
          RTConfig_NumberOfVarargsArguments + 1 == paramCount) {
        curArg = varargs[j];
      } else {
        curArg = MakeInvariantLoad(
            builder,
            builder->CreateGEP(
                varargs[RTConfig_NumberOfVarargsArguments],
                MakeInt32(j - RTConfig_NumberOfVarargsArguments)),
            "varArg", AtomicOrdering::NotAtomic);
      }
    ...
    auto callResult = builder->CreateCall(
        implFunctionType, method->GetLLVMElement(mod),
        ArrayRef<Value *>(argsarr, paramCount), method->GetQName());
    callResult->setCallingConv(NOMCC);
    auto actualResult = EnsurePackedUnpacked(
        builder,
        EnsurePackedUnpacked(builder, callResult,
                             calledFunctionType->getReturnType()),
        POINTERTYPE);
  ...
  {
    BasicBlock *callBlock =
        BasicBlock::Create(LLVMCONTEXT, method->GetName(), fun);
    BasicBlock *failBlock = RTOutput_Fail::GenerateFailOutputBlock(
        builder, "Invoked object with wrong arity!");
    auto lambdaCallTag = NomLambdaCallTag::GetCallTag(
        method->GetDirectTypeParametersCount(), method->GetArgumentCount());
    auto callTagMatch = builder->CreateICmpEQ(
        builder->CreatePtrToInt(callTag, numtype(intptr_t)),
        ConstantExpr::getPtrToInt(lambdaCallTag->GetLLVMElement(mod),
                                  numtype(intptr_t)),
        "callTagMatch");
    builder->CreateIntrinsic(Intrinsic::expect, {inttype(1)},
                             {callTagMatch, MakeUInt(1, 1)});
    builder->CreateCondBr(callTagMatch, callBlock, failBlock,
                          GetLikelyFirstBranchMetadata());
    ...
    CastedValueCompileEnv cvce = CastedValueCompileEnv(
        method->GetDirectTypeParameters(), this->GetAllTypeParameters(), fun, 2,
        paramCount,
        ObjectHeader::GeneratePointerToTypeArguments(builder, varargs[0]));
    for (decltype(paramCount) j = 0; j < paramCount; j++) {
      Value *curArg = nullptr;
      if (j < RTConfig_NumberOfVarargsArguments ||
          RTConfig_NumberOfVarargsArguments + 1 == paramCount) {
      ...
      if (j >= method->GetDirectTypeParametersCount()) {
        ...
        if (j > method->GetDirectTypeParametersCount()) {
          curArg = RTCast::GenerateCast(
              builder, &cvce, curArg,
              method->GetArgumentTypes(
                  &nscmc)[j - (method->GetDirectTypeParametersCount() + 1)]);
        }
      }
      curArg = EnsurePackedUnpacked(builder, curArg, expectedType);
      argsarr[j] = curArg;
    ...
    callResult->setCallingConv(NOMCC);
    auto actualResult = EnsurePackedUnpacked(
        builder, EnsurePackedUnpacked(builder, callResult, REFTYPE),
        POINTERTYPE);
    builder->CreateRet(actualResult);
  }
...
llvm::Function *
NomClass::GetDynamicFieldLookup(llvm::Module &mod,
                                llvm::GlobalValue::LinkageTypes linkage) const {
  ...
  if (fun == nullptr) {
    ...
    auto nameSwitch =
        builder->CreateSwitch(namearg, notfound, this->AllFields.size());

    SimpleClassCompileEnv scce = SimpleClassCompileEnv(
        fun, this, nullarray(NomTypeParameterRef),
        TypeList({NomIntClass::GetInstance()->GetType()}), thisType);
    scce[0] = thisarg;

    for (auto field : AllFields) {
      std::string fieldName = field->GetName()->ToStdString();
      BasicBlock *fieldBlock =
          BasicBlock::Create(LLVMCONTEXT, "field:" + fieldName, fun);
      nameSwitch->addCase(
          MakeInt<size_t>(NomNameRepository::Instance().GetNameID(fieldName)),
          fieldBlock);
      builder->SetInsertPoint(fieldBlock);
      builder->CreateRet(EnsurePacked(
          builder,
          field->GenerateRead(builder, &scce, NomValue(thisarg, thisType))));
    }
...
llvm::Function *
NomClass::GetDynamicFieldStore(llvm::Module &mod,
                               llvm::GlobalValue::LinkageTypes linkage) const {
  auto thisType = this->GetType(GetAllTypeVariables());
  ...
  if (fun == nullptr) {
    ...
    BasicBlock *errorBlock =
        BasicBlock::Create(LLVMCONTEXT, "invalidWriteValue", fun);

    builder->SetInsertPoint(start);
    auto nameSwitch =
        builder->CreateSwitch(namearg, notfound, this->AllFields.size());

    SimpleClassCompileEnv scce =
        SimpleClassCompileEnv(fun, this, nullarray(NomTypeParameterRef),
                              TypeList({NomIntClass::GetInstance()->GetType(),
                                        &NomDynamicType::Instance()}),
                              thisType);
    scce[0] = thisarg;
    ...
    for (auto field : AllFields) {
      ...
      if (field->GetType()->IsSubtype(NomIntClass::GetInstance()->GetType(),
                                      false)) {
      ...
      } else if (field->GetType()->IsSubtype(
                     NomFloatClass::GetInstance()->GetType(), false)) {
      ...
      field->GenerateWrite(builder, &scce, NomValue(thisarg, thisType),
                           NomValue(writeValue, field->GetType()));
    ...
    builder
        ->CreateCall(RTOutput_Fail::GetLLVMElement(*fun->getParent()),
                     GetLLVMPointer(generic_errorMessage))
        ->setCallingConv(
            RTOutput_Fail::GetLLVMElement(*fun->getParent())->getCallingConv());
    CreateDummyReturn(builder, fun);
    llvm::raw_os_ostream out(std::cout);
    if (verifyFunction(*fun, &out)) {
      out.flush();
...
llvm::Constant *
NomClass::GetCastFunction(llvm::Module &mod,
                          llvm::GlobalValue::LinkageTypes linkage) const {
  return ConstantPointerNull::get(GetCastFunctionType()->getPointerTo());
}

llvm::Constant *
NomClass::GetDynamicDispatcherLookup(llvm::Module &mod,
                                     llvm::GlobalValue::LinkageTypes linkage,
                                     llvm::StructType *stype) const {
  auto thisType = this->GetType(GetAllTypeVariables());
  auto rettype =
  ...
  for (decltype(IMTsize) i = 0; i < IMTsize; i++) {
    ...
    for (auto &mte : MethodTable) {
      if (NomNameRepository::Instance().GetNameID(mte->Method->GetName()) %
              IMTsize ==
          i) {
        bool found = false;
        for (auto &meth : methods) {
    ...
    for (auto field : AllFields) {
      if (field->GetType()->GetLLVMType() == REFTYPE &&
          NomNameRepository::Instance().GetNameID(
              field->GetName()->ToStdString()) %
                  IMTsize ==
              i) {
    ...
    for (auto &meth : methods) {
      auto fun =
          Function::Create(GetIMTFunctionType(), linkage,
                           "NOMMON_RT_DD_" + *meth->GetSymbolName(), mod);
      fun->setCallingConv(NOMCC);
      ...
      std::string *errormsg =
          new std::string("Called method " + meth->GetName() +
                          " with invalid number of arguments!");
      std::string *typeerrormsg =
          new std::string("Invalid argument arguments!");
      builder->SetInsertPoint(startBlock);
      BasicBlock *wrongArgumentCountBlock =
          RTOutput_Fail::GenerateFailOutputBlock(builder, errormsg->c_str());
      BasicBlock *invalidArgumentBlock = RTOutput_Fail::GenerateFailOutputBlock(
          builder, typeerrormsg->c_str());
      ...
      auto callTagMatch = CreatePointerEq(
          builder, callTag,
          NomLambdaCallTag::GetCallTag(meth->GetDirectTypeParametersCount(),
                                       meth->GetArgumentCount())
              ->GetLLVMElement(mod));
      builder->CreateIntrinsic(Intrinsic::expect, {inttype(1)},
                               {callTagMatch, MakeUInt(1, 1)});
      builder->CreateCondBr(callTagMatch, doCallBlock, wrongArgumentCountBlock,
                            GetLikelyFirstBranchMetadata());
      ...
      CastedValueCompileEnv cvce = CastedValueCompileEnv(
          meth->GetDirectTypeParameters(), this->GetAllTypeParameters(), fun, 2,
          paramCount,
          ObjectHeader::GeneratePointerToTypeArguments(builder, varargs[0]));
      for (decltype(paramCount) j = 0; j < paramCount; j++) {
        Value *curArg = nullptr;
        ...
        if (j >= meth->GetDirectTypeParametersCount()) {
          ...
          if (j > meth->GetDirectTypeParametersCount()) {
            curArg = RTCast::GenerateCast(
                builder, &cvce, curArg,
                meth->GetArgumentTypes(
                    &nscmc)[j - (meth->GetDirectTypeParametersCount() + 1)]);
          }
        }
        curArg = EnsurePackedUnpacked(builder, curArg, expectedType);
        argsarr[j] = curArg;
      ...
      entries[entryID] = GetDynamicDispatchListEntryConstant(
          MakeInt<size_t>(
              NomNameRepository::Instance().GetNameID(meth->GetName())),
          MakeInt<size_t>(0), fun);
      entryID++;
    }
    for (auto &field : fields) {
      entries[entryID] = GetDynamicDispatchListEntryConstant(
          MakeInt<size_t>(NomNameRepository::Instance().GetNameID(
              field->GetName()->ToStdString())),
          MakeInt<size_t>(1),
          ConstantExpr::getIntToPtr(
              MakeInt<size_t>(
                  ((size_t)(field->Index + (GetHasRawInvoke() ? 1 : 0))) << 32),
              GetIMTFunctionType()->getPointerTo()));
      entryID++;
    }
    entries[entryID] = GetDynamicDispatchListEntryConstant(
        MakeInt<size_t>(0), MakeInt<size_t>(0),
        ConstantPointerNull::get(GetIMTFunctionType()->getPointerTo()));
    retarr[i] = ConstantArray::get(
        arrtype(GetDynamicDispatchListEntryType(), entryID + 1),
        ArrayRef<Constant *>(entries, entryID + 1));
...
llvm::Constant *NomClass::GetInterfaceTableLookup(
  ...
  for (decltype(IMTsize) i = 0; i < IMTsize; i++) {
    ...
    for (auto super : supers) {
      for (auto &meth : super->Methods) {
        if (meth->GetContainer() == super && meth->GetIMTIndex() == i &&
            (NomLambdaOptimizationLevel == 0 || !meth->GetName().empty())) {
          methods.push_back(meth);
        }
      }
    }

    auto fun = Function::Create(
        GetIMTFunctionType(), linkage,
        "MONNOM_RT_IMT_" + *GetSymbolName() + "_" + std::to_string(i), mod);
    fun->setCallingConv(NOMCC);
    BasicBlock *startBlock = BasicBlock::Create(LLVMCONTEXT, "", fun);
    NomBuilder builder;
    builder->SetInsertPoint(startBlock);
    ...
    for (auto &meth : methods) {
      ...
      for (auto &mte : MethodTable) {
        if (mte->Method->Overrides(meth)) {
          ...
          auto callResult = builder->CreateCall(
              implFunctionType, mte->Method->GetLLVMElement(mod),
              ArrayRef<Value *>(argsarr, paramCount), mte->Method->GetQName());
          callResult->setCallingConv(NOMCC);
          auto actualResult = EnsurePackedUnpacked(
              builder,
              EnsurePackedUnpacked(builder, callResult,
    ...
    if (methods.size() != 1 && !RTConfig_RunUnncessesaryCallTagChecks) {
      RTOutput_Fail::MakeBlockFailOutputBlock(
          builder, "RUNTIME ERROR: Called invalid IMT method!",
...
const NomInstantiationRef<NomClass>
NomClassLoaded::GetSuperClass(const NomSubstitutionContext *context) const {
  ...
  if (!superClassRef.HasElem() && superClass != 0) {
    ...
    superClassRef =
        NomConstants::GetSuperClass(superClass)->GetClassType(&nscmc);
  }
  return superClassRef;
}

const llvm::SmallVector<NomClassTypeRef, 16>
NomClass::GetSuperNameds(llvm::ArrayRef<NomTypeRef> args) const {
  ...
  if (sc.HasElem()) {
    ...
    ret.push_back(
        sc.Elem->GetType(llvm::ArrayRef<NomTypeRef>(targs, targcount /** 2*/)));
  }
  return ret;
}
...
void NomClass::PreprocessInheritance() const {
  ...
  for (int sii = 0; sii < superinsts.size(); sii++) {
    ...
    for (auto &inst : super.Elem->GetInstantiations()) {
      AddInstantiation(
          NomInstantiationRef<NomInterface>(inst.first, inst.second));
    }
  ...
  for (auto &sinst : superClassRef.Elem->GetInstantiations()) {
    AddInstantiation(
        NomInstantiationRef<NomInterface>(sinst.first, sinst.second));
  }
  AddInstantiation(NomInstantiationRef<NomInterface>(superClassRef.Elem,
                                                     superClassRef.TypeArgs));
  ...
  for (auto meth : mthis->Methods) {
    for (size_t i = 0, s = MethodTable.size(); i != s; i++) {
      if (meth->Overrides(MethodTable[i]->Method)) {
        mthis->MethodTable[i] = new NomMethodTableEntry(
            meth, MethodTable[i]->CallableVersion->FunType,
            MethodTable[i]->Offset);
        if (meth->GetLLVMFunctionType() ==
            MethodTable[i]->CallableVersion->FunType) {
  ...
  for (auto meth : mthis->Methods) {
    if (!meth->IsOffsetSet()) {
      ...
      mthis->MethodTable.push_back(new NomMethodTableEntry(
          meth, meth->GetLLVMFunctionType(), meth->GetOffset()));
    }
  }
  ...
  for (auto super : supers) {
    ...
    for (size_t mts = mthis->MethodTable.size(), itoffset = 0; itoffset <= mts;
         itoffset++) {
      bool match = true;
      for (size_t i = 0, smts = super.Elem->MethodTable.size(); i != smts;
           i++) {
        auto superSig = super.Elem->MethodTable[i]->Method->Signature(&nscl);
        if (itoffset + i < mts) {
          if (!MethodTable[itoffset + i]->PerfectOverrideMatch(
                  super.Elem->MethodTable[i], super.TypeArgs)) {
            match = false;
            break;
          }
        } else {
          ...
          for (auto meth : MethodTable) {
            ...
            if (meth->Method->GetName() ==
                super.Elem->MethodTable[i]->Method->GetName()) {
              ...
              if (methSig.Satisfies(superSig, false)) {
                ...
                mthis->MethodTable.push_back(new NomMethodTableEntry(
                    meth->Method,
                    super.Elem->MethodTable[i]->CallableVersion->FunType,
                    mthis->MethodTable.size()));
                break;
              } else {
...
NomClassInternal::NomClassInternal(NomStringRef name,
                                   const NomMemberContext *parent)
    : NomInterface(name->ToStdString()), NomInterfaceInternal(name, parent),
      symname(name->ToStdString()) {
...
void NomClassInternal::SetSuperClass() {
  SetSuperClass(
      NomInstantiationRef<NomClass>(NomObjectClass::GetInstance(), TypeList()));
}

const NomInstantiationRef<NomClass>
NomClassInternal::GetSuperClass(const NomSubstitutionContext *context) const {
  if (context == nullptr || context->GetTypeArgumentCount() == 0 ||
      superClass.TypeArgs.size() == 0) {
    return superClass;
  }
