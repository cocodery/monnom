...
namespace Nom {
namespace Runtime {
NomTypeParameterLoaded::NomTypeParameterLoaded(const NomMemberContext *parent,
                                               int index, ConstantID upperBound,
                                               ConstantID lowerBound)
    : NomTypeParameter(parent, index), upperBoundID(upperBound),
      lowerBoundID(lowerBound) {}
...
int NomTypeParameter::GetIndex() const {
  if (parent == nullptr) {
    ...
    return index + parent->GetTypeParametersStart();
  }
}
} // namespace Runtime
} // namespace Nom