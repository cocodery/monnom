...
private:
  ...
  NomClassType(const NomNamed *named,
               const llvm::ArrayRef<NomTypeRef> args = {});
  const uint64_t id;
...
public:
  static llvm::ArrayRef<NomTypeRef>
  MeetArguments(const llvm::ArrayRef<NomTypeRef> left,
                const llvm::ArrayRef<NomTypeRef> right);
  typedef void *(*InitFunctionPointer)(void *, void *, size_t, const NomType *,
                                       int, const NomTypeRef *);
  ...
  bool IsSubtype(NomBottomTypeRef other,
  ...
  bool IsSupertype(NomBottomTypeRef other,
                   bool optimistic = false) const override;
  bool IsSupertype(NomClassTypeRef other,
                   bool optimistic = false) const override;
...
public:
  virtual TypeKind GetKind() const override { return TypeKind::TKClass; }
