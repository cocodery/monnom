...
NomClassType::MeetArguments(const llvm::ArrayRef<NomTypeRef> left,
                            const llvm::ArrayRef<NomTypeRef> right) {
...
NomClassType::InitFunctionPointer NomClassType::GetCPPInitializerFunction() {
  return (
      InitFunctionPointer)((intptr_t)(NomJIT::Instance()
                                          .lookup("RT_NOM_ClassTypeInitializer")
                                          ->getAddress()));
}
llvm::Function *NomClassType::GetInitializerFunction(llvm::Module &mod) {
  ...
  if (fun == nullptr) {
    FunctionType *funtype = llvm::FunctionType::get(
        RTTypeHead::GetLLVMType()->getPointerTo(),
        {RTClassType::GetLLVMType()->getPointerTo(),
         RTClass::GetLLVMType()->getPointerTo(), numtype(size_t), POINTERTYPE,
         numtype(int),
         RTTypeHead::GetLLVMType()->getPointerTo()->getPointerTo()},
        false);
    fun = llvm::Function::Create(
        funtype,
        (&mod == initializerModule)
            ? llvm::GlobalValue::LinkageTypes::ExternalLinkage
            : llvm::GlobalValue::LinkageTypes::AvailableExternallyLinkage,
        "RT_NOM_ClassTypeInitializer", &mod);
    NomBuilder builder;
    ...
    StoreInst *store = MakeStore(
        builder, mod, MakeInt((unsigned char)TypeKind::TKClass),
        builder->CreateGEP(ctypepointer,
                           {MakeInt32(0),
                            MakeInt32((unsigned char)RTClassTypeFields::Head),
                            MakeInt32((unsigned char)RTTypeHeadFields::Kind)}));
    store = MakeStore(
        builder, mod, hash,
        builder->CreateGEP(ctypepointer,
                           {MakeInt32(0),
                            MakeInt32((unsigned char)RTClassTypeFields::Head),
                            MakeInt32((unsigned char)RTTypeHeadFields::Hash)}));
    store = MakeStore(
        builder, mod, nomtype,
        builder->CreateGEP(
            ctypepointer,
            {MakeInt32(0), MakeInt32((unsigned char)RTClassTypeFields::Head),
             MakeInt32((unsigned char)RTTypeHeadFields::NomType)}));
    store =
        MakeStore(builder, mod,
                  builder->CreatePointerCast(
                      clspointer, RTInterface::GetLLVMType()->getPointerTo()),
                  builder->CreateGEP(
                      ctypepointer,
                      {MakeInt32(0),
                       MakeInt32((unsigned char)RTClassTypeFields::Class)}));
    builder->CreateBr(loopHead);

    builder->SetInsertPoint(loopHead);
    PHINode *phi = builder->CreatePHI(numtype(int), 2);
    phi->addIncoming(argcount, mainBlock);
    builder->CreateCondBr(builder->CreateICmpEQ(phi, MakeInt<int>(0)), retBlock,
                          loop);
    ...
    store =
        MakeStore(builder, mod, load,
                  builder->CreateGEP(
                      ctypepointer,
                      {MakeInt32(0),
                       MakeInt32((unsigned char)RTClassTypeFields::TypeArgs),
                       builder->CreateNeg(phi)}));
    builder->CreateBr(loopHead);

    builder->SetInsertPoint(retBlock);
    builder->CreateRet(builder->CreateGEP(
        ctypepointer,
        {MakeInt32(0), MakeInt32((unsigned char)RTClassTypeFields::Head)}));
  }
  return fun;
}
bool NomClassType::ArgumentsSubtypes(const TypeList &left,
                                     const TypeList &right, bool optimistic) {
  size_t size = left.size();
  if (size != right.size()) {
    throw new std::exception();
  }
...
NomClassType::NomClassType(const NomNamed *named,
                           const llvm::ArrayRef<NomTypeRef> arguments)
    : NomType(), GloballyNamed(&gnprefix), id(Count()), head(), Named(named),
      Arguments(arguments) {}
...
NomTypeRef
NomClassType::SubstituteSubtyping(const NomSubstitutionContext *context) const {
  ...
  if (foundChange) {
    BoehmVector<NomTypeRef> *allocnewargs =
        new BoehmVector<NomTypeRef>(this->Arguments.size());
...
llvm::Constant *
NomClassType::createLLVMElement(llvm::Module &mod,
                                llvm::GlobalValue::LinkageTypes linkage) const {
  llvm::StructType *ttt = RTClassType::GetLLVMType(this->Arguments.size());
  llvm::GlobalVariable *gv = new llvm::GlobalVariable(mod, ttt, true, linkage,
                                                      nullptr, GetGlobalName());

  Constant *funptr;
  if (!this->ContainsVariables()) {
    Function *fun =
        Function::Create(GetCastFunctionType(), linkage,
                         "MONNOM_RT_TYPECASTFUN_CLS_" + GetGlobalName(), mod);
    funptr = fun;
    {
      ...
      if (refValueBlock != nullptr) {
        ...
        if (!this->Named->IsInterface()) {
          ...
          auto directClassMatch = CreatePointerEq(
              builder, clsptr, this->Named->GetInterfaceDescriptor(mod));
          builder->CreateCondBr(directClassMatch, directMatchBlock,
                                noDirectMatchBlock);
          ...
          if (this->Arguments.size() != 0) {
            ...
            for (auto &arg : this->Arguments) {
              BasicBlock *nextBlock =
                  BasicBlock::Create(LLVMCONTEXT, "next", fun);
              auto targ = ObjectHeader::GenerateReadTypeArgument(
                  builder, castedValue, argpos);
        ...
        BasicBlock *nominalObjectBlock =
            BasicBlock::Create(LLVMCONTEXT, "nominalObject", fun);
        BasicBlock *structuralObjectBlock =
            BasicBlock::Create(LLVMCONTEXT, "structuralObject", fun);
        auto isNominalObject =
            RTVTable::GenerateIsNominalValue(builder, vtable);
        builder->CreateIntrinsic(Intrinsic::expect, {inttype(1)},
                                 {isNominalObject, MakeUInt(1, 1)});
        builder->CreateCondBr(isNominalObject, nominalObjectBlock,
                              structuralObjectBlock,
        ...
        builder->SetInsertPoint(nominalObjectBlock);
        {
          ...
          if (this->Arguments.size() > 0) {
            foundMatchBlock =
                BasicBlock::Create(LLVMCONTEXT, "foundMatch", fun);
            builder->SetInsertPoint(foundMatchBlock);
            typeArgsPHI = builder->CreatePHI(
                SuperInstanceEntryType()->getPointerTo(), 2, "matchingEntry");
            auto origTypeArgs = ObjectHeader::GeneratePointerToTypeArguments(
                builder, castedValue);
            auto typeArgs =
                MakeInvariantLoad(builder, typeArgsPHI,
                                  MakeInt32(SuperInstanceEntryFields::TypeArgs),
            ...
            rssv.MakeReleaseBlocks(builder, successBlock, &outBlock[0],
                                   failBlock, &outBlock[1]);
            int pos = 0;
            for (auto &arg : this->Arguments) {
              ...
              auto isTypeEq = builder->CreateCall(
                  ...
                  {targ, rssv, arg->GetLLVMElement(mod),
                   ConstantPointerNull::get(
                       RTSubstStack::GetLLVMType()->getPointerTo())});
          ...
          if (this->Named->IsInterface()) {
            ...
            auto supers =
                RTInterface::GenerateReadSuperInterfaces(builder, clsptr);
            ...
            builder->CreateCondBr(stillEntriesLeft, loopBodyBlock, failBlock,
                                  GetLikelyFirstBranchMetadata());
            ...
            auto superMatch = CreatePointerEq(
                builder, superIface, this->Named->GetInterfaceDescriptor(mod));
            posPHI->addIncoming(
                builder->CreateAdd(posPHI, MakeIntLike(posPHI, 1)),
            ...
            auto supers =
                RTInterface::GenerateReadSuperInstances(builder, clsptr);
            ...
            auto superIndex =
                MakeIntLike(supercount, this->Named->GetSuperClassCount());
            auto lessThan = builder->CreateICmpULT(superIndex, supercount);
            CreateExpect(builder, lessThan, MakeUInt(1, 1));
            builder->CreateCondBr(lessThan, checkSuperClassBlock, failBlock,
                                  GetLikelyFirstBranchMetadata());
            ...
            auto superMatch = CreatePointerEq(
                builder, superClass, this->Named->GetInterfaceDescriptor(mod));
            CreateExpect(builder, lessThan, MakeUInt(1, 1));
            builder->CreateCondBr(superMatch, foundMatchBlock, failBlock,
                                  GetLikelyFirstBranchMetadata());
            if (typeArgsPHI != nullptr) {
              typeArgsPHI->addIncoming(entry, builder->GetInsertBlock());
            }
        ...
        builder->SetInsertPoint(structuralObjectBlock);
        {
          if (this->Named->IsInterface()) {
            StructuralValueHeader::GenerateMonotonicStructuralCast(
                builder, fun, successBlock, failBlock, castedValue, this,
                ConstantPointerNull::get(
                    RTSubstStack::GetLLVMType()->getPointerTo()));
          } else {
            BasicBlock *structClassFailBlock =
                RTOutput_Fail::GenerateFailOutputBlock(
                    builder, "Tried to cast structural value to class!");
      ...
      if (intBlock != nullptr) {
        if (NomIntClass::GetInstance()->GetType()->IsSubtype(this, false)) {
          ...
          RTOutput_Fail::MakeBlockFailOutputBlock(
              builder, "Tried to cast integer to wrong type!", intBlock);
        }
      }
      if (floatBlock != nullptr) {
        if (NomFloatClass::GetInstance()->GetType()->IsSubtype(this, false)) {
          ...
          RTOutput_Fail::MakeBlockFailOutputBlock(
              builder, "Tried to cast float to wrong type!", floatBlock);
        }
      }
...
llvm::Value *NomClassType::GenerateRTInstantiation(NomBuilder &builder,
                                                   CompileEnv *env) const {
  if (this->ContainsVariables()) {
    llvm::Value *argarr[2];
    argarr[0] = this->Named->GetLLVMElement(*(env->Module));
    if (this->Arguments.size() > 0) {
      llvm::AllocaInst *typesarr =
          builder->CreateAlloca(RTTypeHead::GetLLVMType()->getPointerTo(),
                                MakeInt<int32_t>(this->Arguments.size()));
      for (size_t i = 0; i < this->Arguments.size(); i++) {
        MakeStore(builder, (*(env->Module)),
                  this->Arguments[i]->GenerateRTInstantiation(builder, env),
                  builder->CreateGEP(typesarr, MakeInt<int32_t>(i)));
      }
      argarr[1] = typesarr;
    }

    return builder->CreateCall(
        env->Module->getFunction("NOM_RTInstantiateClass"), argarr);
  } else {
...
llvm::Constant *NomClassType::findLLVMElement(llvm::Module &mod) const {
  ...
  return llvm::ConstantExpr::getGetElementPtr(
      var->getType()->getElementType(), var,
      llvm::ArrayRef<llvm::Constant *>(
          {MakeInt32(0), MakeInt32((unsigned char)RTClassTypeFields::Head)}));
}
intptr_t NomClassType::GetRTElement() const {
  ...
  //	throw new std::exception();
  // }
  // void * rtct = (nmalloc(RTClassType::SizeOf()+sizeof(intptr_t
  // *)*this->Arguments.size())); auto initfun = GetCPPInitializerFunction();
  // return (intptr_t)initfun(rtct, (void*) this->Named->GetRTElement(),
  // this->GetHashCode(), this, this->Arguments.size(), this->Arguments.data());
}
NomClassTypeRef
NomClassType::GetClassInstantiation(const NomNamed *named) const {
  if (named == this->Named) {
    return this;
  }
  ...
  for (auto snamed : this->Named->GetSuperNameds(this->Arguments)) {
    ...
    if (super != nullptr) {
      ...
      ret = named->GetType(
          NomClassType::MeetArguments(ret->Arguments, super->Arguments));
    }
  }
  return ret;
}
...
bool NomClassType::IsDisjoint(NomBottomTypeRef other) const { return true; }
...
bool NomClassType::IsDisjoint(NomTopTypeRef other) const { return false; }
...
bool NomClassType::PossiblyPrimitive() const {
  return NomIntClass::GetInstance()->GetType()->IsSubtype(this) ||
         NomFloatClass::GetInstance()->GetType()->IsSubtype(this) ||
         NomIntClass::GetInstance()->GetType()->GetClassInstantiation(
             this->Named) != nullptr ||
         NomFloatClass::GetInstance()->GetType()->GetClassInstantiation(
             this->Named) != nullptr;
}
bool NomClassType::UncertainlyPrimitive() const {
  ...
  return !((IsDisjoint(inttype) && IsDisjoint(floattype)) ||
           IsSubtype(inttype, false) || IsSubtype(floattype, false));
}
...
TypeReferenceType NomClassType::GetTypeReferenceType() const {
  ...
  // if (Named == NomBoolClass::GetInstance()) //Dangerous for interaction with
  // c++ code, need to adjust at least the signatures of the internal wrapper
  // methods
  //{
  //	return TypeReferenceType::UnpackedBool;
  // }
  return TypeReferenceType::
      Reference; // llvm::StructType::create(LLVMCONTEXT)->getPointerTo();
}
...
extern "C" intptr_t NOM_RTInstantiateClass(intptr_t cls, intptr_t *args) {
  ...
  return ncls->GetType(TypeList(argtypes, ncls->GetTypeParametersCount()))
      ->GetRTElement();
}