...
NomTypeVar::NomTypeVar(const NomTypeParameter *ref)
    : GloballyNamed(&gnprefix), referenceParameter(ref) {}
...
int NomTypeVar::GetIndex() const { return referenceParameter->GetIndex(); }
...
bool NomTypeVar::IsSubtype(NomTypeVarRef other, bool optimistic) const {
  return this->GetIndex() == other->GetIndex() ||
         this->GetUpperBound()->IsSubtype(other->GetLowerBound(), optimistic);
}
...
bool NomTypeVar::IsSupertype(NomTypeVarRef other, bool optimistic) const {
  return this->GetIndex() == other->GetIndex() ||
         this->GetLowerBound()->IsSupertype(other->GetUpperBound(), optimistic);
}
NomTypeRef
NomTypeVar::SubstituteSubtyping(const NomSubstitutionContext *context) const {
...
llvm::Type *NomTypeVar::GetLLVMType() const { return REFTYPE; }
size_t NomTypeVar::GetHashCode() const {
  return referenceParameter->GetHashCode();
}
llvm::Constant *
NomTypeVar::createLLVMElement(llvm::Module &mod,
                              llvm::GlobalValue::LinkageTypes linkage) const {
  auto var = new llvm::GlobalVariable(mod, RTTypeVar::GetLLVMType(), true,
                                      linkage, nullptr, GetGlobalName());
  auto cnst =
      RTTypeVar::GetConstant(GetIndex(), GetLowerBound()->GetLLVMElement(mod),
                             GetUpperBound()->GetLLVMElement(mod), this);
  var->setInitializer(cnst);
  return llvm::ConstantExpr::getGetElementPtr(
      cnst->getType(), var,
      llvm::ArrayRef<llvm::Constant *>(
          {MakeInt32(0), MakeInt32((unsigned char)RTTypeVarFields::Head)}));
}
...
bool NomTypeVar::IsDisjoint(NomBottomTypeRef other) const { return true; }
bool NomTypeVar::IsDisjoint(NomClassTypeRef other) const {
  NomSingleSubstitutionContext nssc =
      NomSingleSubstitutionContext(this, GetDynamicType());
...
bool NomTypeVar::UncertainlyPrimitive() const {
  ...
  return !((ub->IsDisjoint(inttype) && ub->IsDisjoint(floattype)) ||
           ub->IsSubtype(inttype, false) || ub->IsSubtype(floattype, false));
}
