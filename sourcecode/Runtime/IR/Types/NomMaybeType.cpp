...
NomMaybeType::NomMaybeType(NomTypeRef potentialType)
    : GloballyNamed(&nomMaybeTypePrefix), PotentialType(potentialType) {}
...
llvm::Constant *
NomMaybeType::createLLVMElement(llvm::Module &mod,
  ...
  return llvm::ConstantExpr::getGetElementPtr(
      ...
      llvm::ArrayRef<llvm::Constant *>(
          {MakeInt32(0), MakeInt32((unsigned char)RTMaybeTypeFields::Head)}));
}
...
llvm::Value *NomMaybeType::GenerateRTInstantiation(NomBuilder &builder,
  ...
  if (PotentialType->ContainsVariables()) {
    ...
    return builder->CreateCall(
        env->Module->getFunction("NOM_RTInstantiateMaybe"), argarr);
  } else {
...
bool NomMaybeType::IsSubtype(NomClassTypeRef other, bool optimistic) const {
  return PotentialType->IsSubtype(other, optimistic) &&
         NomNullClass::GetInstance()->GetType()->IsSubtype(other);
}
...
bool NomMaybeType::IsSupertype(NomClassTypeRef other, bool optimistic) const {
  return PotentialType->IsSupertype(other) ||
         NomNullClass::GetInstance()->GetType()->IsSupertype(other);
}
...
bool NomMaybeType::IsDisjoint(NomBottomTypeRef other) const { return true; }
bool NomMaybeType::IsDisjoint(NomClassTypeRef other) const {
  return PotentialType->IsDisjoint(other) &&
         NomNullClass::GetInstance()->GetType()->IsDisjoint(other);
}
bool NomMaybeType::IsDisjoint(NomTopTypeRef other) const { return false; }
...
NomTypeRef
NomMaybeType::SubstituteSubtyping(const NomSubstitutionContext *context) const {
  return NomMaybeType::GetMaybeType(
      PotentialType->SubstituteSubtyping(context));
}
llvm::Type *NomMaybeType::GetLLVMType() const { return REFTYPE; }
const std::string NomMaybeType::GetSymbolRep() const {
  return "RT_NOM_MAYBE$$" + PotentialType->GetSymbolRep();
}
TypeKind NomMaybeType::GetKind() const { return TypeKind::TKMaybe; }
intptr_t NomMaybeType::GetRTElement() const {
  ...
  return (intptr_t)initfun(rtct, (void *)this->PotentialType->GetRTElement(),
                           this->GetHashCode(), this);
}
NomClassTypeRef
NomMaybeType::GetClassInstantiation(const NomNamed *named) const {
  throw new std::exception();
}
...
llvm::Function *NomMaybeType::GetInitializerFunction(llvm::Module &mod) {
  ...
  if (fun == nullptr) {
    FunctionType *funtype =
        llvm::FunctionType::get(RTTypeHead::GetLLVMType()->getPointerTo(),
                                {RTMaybeType::GetLLVMType()->getPointerTo(),
                                 RTTypeHead::GetLLVMType()->getPointerTo(),
                                 numtype(size_t), POINTERTYPE},
                                false);
    fun = llvm::Function::Create(
        ...
        "RT_NOM_MaybeTypeInitializer", &mod);
    NomBuilder builder;
    ...
    StoreInst *store = MakeStore(
        ...
        builder->CreateGEP(mbtypepointer,
                           {MakeInt32(0),
                            MakeInt32((unsigned char)RTMaybeTypeFields::Head),
    ...
    store = MakeStore(
        builder, mod, ptypepointer,
        builder->CreateGEP(
            mbtypepointer,
            {MakeInt32(0),
             MakeInt32((unsigned char)RTMaybeTypeFields::PotentialType)}));
