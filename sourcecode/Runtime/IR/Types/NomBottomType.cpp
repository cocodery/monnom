...
NomTypeRef NomBottomType::SubstituteSubtyping(
    const NomSubstitutionContext *context) const {
  return this;
}
llvm::Value *NomBottomType::GenerateRTInstantiation(NomBuilder &builder,
...
const std::string NomBottomType::GetSymbolRep() const { return "Nothing"; }
llvm::Constant *NomBottomType::createLLVMElement(
    llvm::Module &mod, llvm::GlobalValue::LinkageTypes linkage) const {
  Function *fun = Function::Create(GetCastFunctionType(), linkage,
                                   "MONNOM_RT_TYPECASTFUN_BOTTOM", mod);
  {
    fun->setCallingConv(NOMCC);
    BasicBlock *startBlock = BasicBlock::Create(LLVMCONTEXT, "", fun);
    NomBuilder builder;
    RTOutput_Fail::MakeBlockFailOutputBlock(
        builder, "Cast failed: value cannot be subtype of bottom!", startBlock);
  }
  return new llvm::GlobalVariable(
      mod, RTTypeHead::GetLLVMType(), true, linkage,
      RTTypeHead::GetConstant(TypeKind::TKBottom, MakeInt(GetHashCode()), this,
                              fun),
      "RT_NOM_BottomType");
}
...
NomClassTypeRef
NomBottomType::GetClassInstantiation(const NomNamed *named) const {
  throw new std::exception();
  // return named->GetGeneralBottomType();
}
bool NomBottomType::IsDisjoint(NomTypeRef other) const { return true; }
bool NomBottomType::IsDisjoint(NomBottomTypeRef other) const { return true; }
bool NomBottomType::IsDisjoint(NomClassTypeRef other) const { return true; }
bool NomBottomType::IsDisjoint(NomTopTypeRef other) const { return true; }
bool NomBottomType::IsDisjoint(NomTypeVarRef other) const { return true; }
bool NomBottomType::IsSubtype(NomDynamicTypeRef other, bool optimistic) const {
  return true;
}
bool NomBottomType::IsSupertype(NomDynamicTypeRef other,
                                bool optimistic) const {
  return optimistic;
}
bool NomBottomType::PossiblyPrimitive() const { return false; }
bool NomBottomType::UncertainlyPrimitive() const { return false; }
...
bool NomBottomType::IsDisjoint(NomMaybeTypeRef other) const { return true; }
TypeReferenceType NomBottomType::GetTypeReferenceType() const {
  return TypeReferenceType::Reference;
}
bool NomBottomType::ContainsVariableIndex(int index) const { return false; }
} // namespace Runtime
} // namespace Nom