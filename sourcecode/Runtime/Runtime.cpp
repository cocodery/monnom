...
int test() {
  ...
  NomClassLoaded *mainclass =
      prog->AddClass(context[4], 0, context[7], 0, nullptr);
  mod.Classes.emplace_front(mainclass);
  std::unique_ptr<NomInstruction> i1 = std::unique_ptr<NomInstruction>(
      new LoadStringConstantInstruction(2, context[1]));
  std::unique_ptr<NomInstruction> i2 = std::unique_ptr<NomInstruction>(
      new CallCheckedInstanceMethod(3, context[9], 0, 2));
  std::unique_ptr<NomInstruction> i3 =
      std::unique_ptr<NomInstruction>(new ReturnInstruction(2));
  NomStaticMethodLoaded *meth = mainclass->AddStaticMethod(
      NomConstants::GetString(context[10])->GetText()->ToStdString(),
      mainclass->GetName()->ToStdString() + "." +
          NomConstants::GetString(context[10])->GetText()->ToStdString(),
      0, context[7], 0, 4);
...
int run(const std::vector<std::string> args) {
  ...
  const NomStaticMethod *mainMethod =
      mainCls->GetStaticMethod(lib->GetMainMethodName(), TypeList(), TypeList())
          .Elem;
