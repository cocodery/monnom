...
llvm::FunctionType *GetFieldReadFunctionType() {
  static auto ft = FunctionType::get(
      RefValueHeader::GetUninitializedLLVMType()->getPointerTo(),
      {numtype(size_t),
       RefValueHeader::GetUninitializedLLVMType()->getPointerTo()},
      false);
  return ft;
...
llvm::FunctionType *GetIMTFunctionType() {
  ...
  if (!once) {
    ...
    ft = FunctionType::get(
        POINTERTYPE,
        ArrayRef<Type *>(argtypes, 2 + RTConfig_NumberOfVarargsArguments),
        false);
    once = true;
  }
...
llvm::StructType *GetDynamicDispatchListEntryType() {
  static StructType *st =
      StructType::create(LLVMCONTEXT, "MONNOM_RT_DynamicDispatchListEntry");
...
llvm::Constant *
GetDynamicDispatchListEntryConstant(llvm::Constant *key, llvm::Constant *flags,
                                    llvm::Constant *dispatcherPtr) {
  return ConstantStruct::get(GetDynamicDispatchListEntryType(), key, flags,
                             dispatcherPtr);
}

llvm::StructType *GetDynamicDispatcherLookupResultType() {
  static llvm::StructType *st = StructType::create(
      LLVMCONTEXT, {GetIMTFunctionType()->getPointerTo(), POINTERTYPE},
      "DynamicDispatcherResultPair");
  return st;
}

llvm::FunctionType *GetCheckReturnValueFunctionType() {
  // static auto ft = FunctionType::get(inttype(1), {
  // GetIMTCastFunctionType()->getPointerTo(), TYPETYPE, POINTERTYPE,
  // POINTERTYPE, POINTERTYPE, POINTERTYPE }, false); return ft;
  ...
  if (!once) {
    ...
    argtypes[2] = REFTYPE; // return value; always packed because it comes from
                           // struct dispatcher
    for (decltype(RTConfig_NumberOfVarargsArguments) i = 0;
         i < RTConfig_NumberOfVarargsArguments; i++) {
...
llvm::Function *GenerateCheckReturnTypesFunction(
    Module *mod, GlobalValue::LinkageTypes linkage, const llvm::Twine name,
    SmallVector<tuple<NomInterfaceCallTag *, Function *, NomType *>, 8>
        &imtPairs) {
    ...
    for (int i = 0; i < cases; i++) {
      ...
      if (i < cases - 1) {
        ...
        auto keyMatch = builder->CreateICmpEQ(
            givenKey, ConstantExpr::getPtrToInt(
                          std::get<0>(pair)->GetLLVMElement(*mod), INTTYPE));
      ...
      for (auto &argSpec : callFun->args()) {
        if (argpos > 2 && argcount > 4) {
          argBuf.push_back(MakeLoad(
              builder, builder->CreateGEP(
                           builder->CreatePointerCast(
                               args[3], args[3]->getType()->getPointerTo()),
                           MakeInt32(argpos - 3))));
        } else {
        ...
        if (argSpec.getType()->isPointerTy()) {
          argBuf[argpos] =
              builder->CreatePointerCast(argBuf[argpos], argSpec.getType());
        }
        if (argSpec.getType()->isIntegerTy(INTTYPE->getIntegerBitWidth())) {
          ...
          argBuf[argpos] = builder->CreateBitCast(
              builder->CreatePtrToInt(argBuf[argpos], INTTYPE), FLOATTYPE);
        } else if (argSpec.getType()->isIntegerTy(
                       BOOLTYPE->getIntegerBitWidth())) {
      ...
      if (result->getType()->isPointerTy()) {
        ...
        if (result->getType()->isIntegerTy()) {
          ...
          result = builder->CreateIntToPtr(
              builder->CreateBitCast(result, INTTYPE), POINTERTYPE);
        } else {
