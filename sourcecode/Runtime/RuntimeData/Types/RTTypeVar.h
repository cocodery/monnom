...
namespace Runtime {
class NomTypeVar;
enum class RTTypeVarFields : unsigned char {
  Head = 0,
  Index = 1,
  LowerBound = 2,
  UpperBound = 3
};
class RTTypeVar : public RTConcreteType,
                  public ARTRep<RTTypeVar, RTTypeVarFields> {
private:
  // intptr_t index;
public:
  static llvm::StructType *GetLLVMType();
  RTTypeVar(const char *entry /*, intptr_t offset = 0*/)
      : ARTRep<RTTypeVar, RTTypeVarFields>(entry /*, offset*/) {}
  RTTypeVar(const void *entry /*, intptr_t offset = 0*/)
      : ARTRep<RTTypeVar, RTTypeVarFields>(entry /*, offset*/) {}
  static llvm::Constant *GetConstant(int index, llvm::Constant *lowerBound,
                                     llvm::Constant *upperBound,
                                     const NomTypeVar *type);
  static int32_t HeadOffset();
  // static int32_t IndexOffset()
  //{
  //	static const int32_t offset = GetLLVMLayout()->getElementOffset(1);
  // return offset;
  // }
  ...
  //	return false;
  //}
  // RTTypeHead SubstituteSubtyping(const RTTypeHead * const args, size_t
  // argcount) const; RTTypeHead GetIntersectedForm() const; std::list<const
  // std::tuple<unsigned int, intptr_t>> GetVariableReferences(intptr_t start)
  // const;
