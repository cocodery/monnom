...
llvm::StructType *RTMaybeType::GetLLVMType() {
  static llvm::StructType *llvmtype =
      llvm::StructType::create(LLVMCONTEXT, "LLVMRTMaybeType");
  static bool dontrepeat = true;
  if (dontrepeat) {
    dontrepeat = false;
    llvmtype->setBody(RTTypeHead::GetLLVMType(),                // header
...
llvm::Constant *GetMaybeTypeCastFunction(llvm::Module &mod) {
  ...
  if (fun == nullptr) {
    fun = Function::Create(GetCastFunctionType(), linkage,
                           "MONNOM_RT_TYPECAST_MAYBE", mod);
    ...
    builder->CreateIntrinsic(Intrinsic::expect, {inttype(1)},
                             {isNullObj, MakeUInt(1, 1)});
    builder->CreateCondBr(isNullObj, successBlock, recurseBlock,
    ...
    auto recurseCall = builder->CreateCall(
        GetCastFunctionType(), castfun,
        {ptype, castValue, ConstantPointerNull::get(POINTERTYPE),
...
llvm::Constant *RTMaybeType::GetConstant(llvm::Module &mod,
                                         const NomMaybeType *mbt) {
  return ConstantStruct::get(
      GetLLVMType(),
      RTTypeHead::GetConstant(TypeKind::TKMaybe,
                              MakeInt<size_t>(mbt->GetHashCode()), mbt,
                              GetMaybeTypeCastFunction(mod)),
      mbt->PotentialType->GetLLVMElement(mod));
}
uint64_t RTMaybeType::HeadOffset() {
  static const uint64_t offset =
      GetLLVMLayout()->getElementOffset((unsigned char)RTMaybeTypeFields::Head);
  return offset;
}
llvm::Value *RTMaybeType::GenerateReadPotentialType(NomBuilder &builder,
                                                    llvm::Value *type) {
  return MakeLoad(
      builder, builder->CreatePointerCast(type, GetLLVMType()->getPointerTo()),
      MakeInt<RTMaybeTypeFields>(RTMaybeTypeFields::PotentialType));
}
} // namespace Runtime
} // namespace Nom