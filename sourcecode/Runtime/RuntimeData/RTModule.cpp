...
std::forward_list<void *> &RTModule::structRecords() {
  static std::forward_list<void *> lst;
  return lst;
}
...
std::forward_list<RuntimeInstantiationDictionary *> &
RTModule::instantiationDictionaries() {
  static std::forward_list<RuntimeInstantiationDictionary *> lst;
  return lst;
}
...
RTModule::RTModule(NomModule *mod)
    : theModule(std::make_unique<llvm::Module>("module", LLVMCONTEXT)) {
  ...
  llvm::Function::Create(
      ...
      llvm::GlobalValue::LinkageTypes::ExternalLinkage, "RT_NOM_PRINT_STORE",
      theModule.get());
  llvm::Function::Create(
      llvm::FunctionType::get(llvm::Type::getVoidTy(LLVMCONTEXT),
                              {{POINTERTYPE, POINTERTYPE}}, false),
      llvm::GlobalValue::LinkageTypes::ExternalLinkage, "RT_NOM_PRINT_LOAD",
      theModule.get());
  ...
  for (auto &iface : ifaces) {
    ...
    instantiationDictionaries().push_front(
        const_cast<RuntimeInstantiationDictionary *>(
            &iface->runtimeInstantiations));
  }
  for (auto &cls : clses) {
    cls->PreprocessInheritance();
    instantiationDictionaries().push_front(
        const_cast<RuntimeInstantiationDictionary *>(
            &cls->runtimeInstantiations));
  }
  ...
  if (NomVerbose) {
    ...
    for (auto &f : *theModule.get()) {
      if (f.getBasicBlockList().size() > 0) {
        if (f.getName().empty()) {
          std::string funname =
              "AnonymousFunction" + std::to_string(funnamecount++);
          f.setName(funname);
        }
  ...
  for (auto &gv : theModule.get()->getGlobalList()) {
    if (gv.getLinkage() == GlobalValue::LinkageTypes::ExternalLinkage) {
      if (gv.getName().substr(0, structdescprefix.length()) ==
          structdescprefix) {
        structDescriptorNames.push_back(gv.getName().str());
      } else if (gv.getName().substr(0, lambdadescprefix.length()) ==
                 lambdadescprefix) {
  ...
  for (auto &glbl : theModule.get()->globals()) {
    if (glbl.getParent() != theModule.get() ||
        glbl.getLinkage() != GlobalValue::LinkageTypes::ExternalLinkage) {
  ...
  for (auto &glbl : globalnames) {
    ...
    if (NomVerbose) {
      std::cout << "\n"
                << glbl << " : " << std::hex
                << std::move(evalSymbol->getAddress()) << std::dec;
      pobof << "\n"
            << glbl << " : " << std::hex << std::move(evalSymbol->getAddress())
            << std::dec;
    }
  }
  if (NomVerbose) {
