...
namespace Nom {
namespace Runtime {
template <class C, typename Flds> class ARTRep {
private:
  const char *entry;
...
const llvm::StructLayout *ARTRep<C, Flds>::GetLLVMLayout() {
  static const llvm::StructLayout *layout =
      GetNomJITDataLayout().getStructLayout(C::GetLLVMType());
  return layout;
}
template <class C, typename Flds>
llvm::ArrayType *ARTRep<C, Flds>::GetLLVMArrayType() {
  static llvm::ArrayType *type = llvm::ArrayType::get(C::GetLLVMType(), 0);
  return type;
}
template <class C, typename Flds>
...
void *ARTRep<C, Flds>::Entry(uint64_t fieldoffset) const {
  ...
  if (offset == 0) {
    return (void *)(entry + fieldoffset);
  } else {
    return (
        void *)((entry +
                 GetNomJITDataLayout().getIndexedOffsetInType(
                     GetLLVMArrayType(),
                     llvm::ArrayRef<llvm::Value *>({llvm::ConstantInt::get(
                         llvm::IntegerType::get(LLVMCONTEXT, bitsin(intptr_t)),
                         offset, true)}))) +
                fieldoffset);
  }
}
...
void *ARTRep<C, Flds>::Entry(const Flds &field) const {
  ...
  return (void *)(entry +
                  GetLLVMLayout()->getElementOffset((unsigned char)field));
}
template <class C, typename Flds>
ARTRep<C, Flds>::ARTRep(const char *const entry /*, intptr_t offset*/)
    : entry(entry) /*, offset(offset)*/
{}
template <class C, typename Flds>
ARTRep<C, Flds>::ARTRep(const void *const entry /*, intptr_t offset*/)
    : entry((const char *const)entry) /*, offset(offset)*/
{}
} // namespace Runtime
} // namespace Nom
