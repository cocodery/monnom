...
#define llvmsizeof(t)                                                          \
  (llvm::ConstantExpr::getPtrToInt(                                            \
      ::llvm::ConstantExpr::getGetElementPtr(                                  \
          (t), llvm::ConstantPointerNull::get((t)->getPointerTo()),            \
...
#define LLVMCONSTANTINDEXTYPE                                                  \
  (::Nom::Runtime::BytecodeTypes::GetConstantIndexType())
#define TYPETYPE (::Nom::Runtime::BytecodeTypes::GetTypeType())

#define NOM_CONSTANT_DEPENCENCY_CONTAINER                                      \
  llvm::SmallVector<Nom::Runtime::ConstantID, 16>

#ifdef _WIN32
#define makealloca(t, n)                                                       \
  ((t *)((n) > 0 ? (_malloca(sizeof(t) * (n))) : nullptr))
#else
#define makealloca(t, n) ((t *)((n) > 0 ? (alloca(sizeof(t) * (n))) : nullptr))
#endif
#define nullarray(t) (::llvm::ArrayRef<t>(static_cast<t *>(nullptr), (size_t)0))
#define notnullarray(t, arr, n)                                                \
  (::llvm::ArrayRef<t>((arr == nullptr ? reinterpret_cast<t *>(this) : (arr)), \
                       (arr == nullptr ? 0 : (n))))
...
using NomStringType = std::basic_string<NomChar, std::char_traits<NomChar>,
                                        BoehmAtomicAllocator<NomChar>>;
class NomString;
...
public:
  ...
  static llvm::Type *GetConstantIndexType() {
    static llvm::Type *ciType =
        llvm::Type::getIntNTy(LLVMCONTEXT, bitsin(::Nom::Runtime::ConstantID));
    return ciType;
  }

  static llvm::PointerType *GetPointerType() {
    static llvm::PointerType *ptrType =
        llvm::Type::getIntNTy(LLVMCONTEXT, bitsin(char))->getPointerTo();
    return ptrType;
  }
