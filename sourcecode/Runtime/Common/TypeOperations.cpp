...
llvm::Value *EnsureUnpackedInt(NomBuilder &builder, CompileEnv *env,
                               NomValue &val) {
  if (val->getType()->isIntegerTy(INTTYPE->getIntegerBitWidth())) {
    return val;
  }
  return UnpackInt(
      builder, CastInstruction::MakeCast(
                   builder, env, val, NomIntClass::GetInstance()->GetType()));
}

llvm::Value *EnsureUnpackedFloat(NomBuilder &builder, CompileEnv *env,
                                 NomValue &val) {
  ...
  return UnpackFloat(
      builder, CastInstruction::MakeCast(
                   builder, env, val, NomFloatClass::GetInstance()->GetType()));
}

llvm::Value *EnsureUnpackedBool(NomBuilder &builder, CompileEnv *env,
                                NomValue &val) {
  ...
  return UnpackBool(
      builder, CastInstruction::MakeCast(
                   builder, env, val, NomBoolClass::GetInstance()->GetType()));
}
...
llvm::Value *EnsureType(NomBuilder &builder, CompileEnv *env, NomValue &val,
                        NomTypeRef type, llvm::Type *expected) {

  if (type == NomBoolClass::GetInstance()->GetType()) {
    if (expected->isIntegerTy(1)) {
      return EnsureUnpackedBool(builder, env, val);
    } else if (expected == REFTYPE &&
               val.GetNomType()->IsSubtype(
...
llvm::Value *WrapAsPointer(NomBuilder &builder, llvm::Value *val) {
  if (val->getType() != POINTERTYPE) {
    ...
    if (val->getType()->isDoubleTy()) {
      return builder->CreateIntToPtr(builder->CreateBitCast(val, INTTYPE),
                                     POINTERTYPE);
    }
...
llvm::Value *EnsurePackedUnpacked(NomBuilder &builder, llvm::Value *val,
                                  llvm::Type *type) {
  ...
  if (valType == POINTERTYPE) {
    if (type == INTTYPE) {
      ...
      return builder->CreateBitCast(builder->CreatePtrToInt(val, INTTYPE),
                                    FLOATTYPE);
    ...
    if (valType == REFTYPE && type == INTTYPE) {
      return UnpackInt(builder,
                       NomValue(val, NomIntClass::GetInstance()->GetType()));
    } else if (valType == REFTYPE && type == FLOATTYPE) {
      return UnpackFloat(
          builder, NomValue(val, NomFloatClass::GetInstance()->GetType()));
    } else if (valType == REFTYPE && type == BOOLTYPE) {
      return UnpackFloat(builder,
                         NomValue(val, NomBoolClass::GetInstance()->GetType()));
...
/// Step 2: Check whether the result equals the original value, which means the
/// top three bits are all equal, i.e. 000 or 111.
///   - if so, the packed int is the result of the left shift by 2 plus an or
///   with 3 (to marke the to LSBs as 11).
///   - for all other combinations, the integer is boxed into a newly allocated
///   reference value
/// Thus, all small enough (|n| ~ < 2^61) are masked, while bigger absolute
/// values are boxed
/// </summary>
...
llvm::Value *PackInt(NomBuilder &builder, llvm::Value *intval) {
  ...
  if (NomCastStats) {
    builder->CreateCall(
        GetIncIntPacksFunction(
            *builder->GetInsertBlock()->getParent()->getParent()),
        {});
  }
  BasicBlock *incomingBlock = builder->GetInsertBlock();
  auto fun = incomingBlock->getParent();
  BasicBlock *maskedIntBlock =
      BasicBlock::Create(LLVMCONTEXT, "maskIntBlock", fun);
  BasicBlock *boxedIntBlock =
      BasicBlock::Create(LLVMCONTEXT, "boxIntBlock", fun);
  BasicBlock *outBlock = BasicBlock::Create(LLVMCONTEXT, "packedIntBlock", fun);

  auto shlresult =
      builder->CreateShl(intval, MakeIntLike(intval, 2), "leftShiftedValue");
  auto shrresult = builder->CreateAShr(shlresult, MakeIntLike(shlresult, 2),
                                       "rightShiftedValue");
  auto shiftseq = builder->CreateICmpEQ(shrresult, intval, "shiftedValuesEq");
  CreateExpect(builder, shiftseq, MakeUInt(1, 1));
  builder->CreateCondBr(shiftseq, maskedIntBlock, boxedIntBlock,
  ...
  if (NomCastStats) {
    builder->CreateCall(
        GetIncIntBoxesFunction(
            *builder->GetInsertBlock()->getParent()->getParent()),
        {});
  }
  auto allocfun = GetNewAlloc(fun->getParent());
  auto boxedInt = builder->CreateCall(
      allocfun, {MakeInt<size_t>(1), MakeInt<size_t>(0)}, "boxedInt");
  RefValueHeader::GenerateWriteVTablePointer(
      builder, boxedInt,
      NomIntClass::GetInstance()->GetLLVMElement(*fun->getParent()));
  ObjectHeader::WriteField(builder, boxedInt, MakeInt32(0),
                           builder->CreateIntToPtr(intval, REFTYPE), false);
...
llvm::Value *UnpackInt(NomBuilder &builder, NomValue value) {
  if (NomCastStats) {
    builder->CreateCall(
        GetIncIntUnpacksFunction(
            *builder->GetInsertBlock()->getParent()->getParent()),
        {});
  }
  Function *fun = builder->GetInsertBlock()->getParent();
  BasicBlock *refValueBlock = nullptr, *maskedIntBlock = nullptr,
  ...
  RefValueHeader::GenerateRefOrPrimitiveValueSwitch(
      builder, value, &refValueBlock, &maskedIntBlock, &floatBlock, false,
      &primitiveIntBlock, &primitiveIntVal, nullptr, nullptr, nullptr, nullptr,
      30, 100, 1, 1);

  int phiCases = (refValueBlock != nullptr ? 1 : 0) +
                 (maskedIntBlock != nullptr ? 1 : 0) +
                 (primitiveIntBlock != nullptr ? 1 : 0);

  BasicBlock *mergeBlock =
      BasicBlock::Create(LLVMCONTEXT, "intUnpackMerge", fun);
  ...
  if (refValueBlock != nullptr) {
    ...
    if (!value.GetNomType()->IsSubtype(GetIntClassType(), false)) {
      static const char *const errormsg =
          "Tried to unpack non-integer value as an Int!";
      BasicBlock *unboxErrorBlock =
          RTOutput_Fail::GenerateFailOutputBlock(builder, errormsg);
      BasicBlock *unboxValidBlock =
          BasicBlock::Create(LLVMCONTEXT, "intUnboxValid", fun);
      auto vtableptr = builder->CreatePtrToInt(
          ObjectHeader::GenerateReadVTablePointer(builder, value),
          numtype(intptr_t));
    ...
    if (NomCastStats) {
      builder->CreateCall(
          GetIncIntUnboxesFunction(
              *builder->GetInsertBlock()->getParent()->getParent()),
          {});
    }
    auto boxField = builder->CreatePtrToInt(
  ...
  if (floatBlock != nullptr) {
    RTOutput_Fail::MakeBlockFailOutputBlock(
        builder, "Tried to unpack packed Float value as an Int!", floatBlock);
  }
...
/// Otherwise, if float is positive or negative zero, we return a reference to
/// the default objects for those values. Otherwise, float is boxed.
/// </summary>
/// <param name="builder"></param>
...
llvm::Value *PackFloat(NomBuilder &builder, llvm::Value *floatval) {
  ...
  if (packFloatDbgFun == nullptr) {
    packFloatDbgFun =
        Function::Create(FunctionType::get(Type::getVoidTy(LLVMCONTEXT),
                                           {FLOATTYPE, INTTYPE}, false),
                         GlobalValue::LinkageTypes::ExternalLinkage,
                         "RT_NOM_DBG_PrintPackFloatDebug", mod);
  }
  Function *boxFloatDbgFun = mod.getFunction("RT_NOM_DBG_PrintBoxFloatDebug");
  if (boxFloatDbgFun == nullptr) {
    boxFloatDbgFun =
        Function::Create(FunctionType::get(Type::getVoidTy(LLVMCONTEXT),
                                           {FLOATTYPE, POINTERTYPE}, false),
                         GlobalValue::LinkageTypes::ExternalLinkage,
                         "RT_NOM_DBG_PrintBoxFloatDebug", mod);
  }
#endif
  BasicBlock *checkPosZeroBlock =
      BasicBlock::Create(LLVMCONTEXT, "checkPosZeroFloat", fun);
  BasicBlock *checkNegZeroFloatBlock =
      BasicBlock::Create(LLVMCONTEXT, "checkNegZeroFloat", fun);
  ...
  auto isPositiveGoodExponent = builder->CreateICmpEQ(
      builder->CreateAnd(floatAsInt, MakeIntLike(floatAsInt, exponentMask)),
      MakeIntLike(floatAsInt, posExpTag));
  auto isNegativeGoodExponent = builder->CreateICmpEQ(
      builder->CreateAnd(floatAsInt, MakeIntLike(floatAsInt, exponentMask)),
      MakeIntLike(floatAsInt, negExpTag));
  auto isGoodExponent =
      builder->CreateOr(isPositiveGoodExponent, isNegativeGoodExponent);
  auto funnelShiftedFloat = builder->CreateIntToPtr(
      builder->CreateIntrinsic(
          Intrinsic::fshl, {floatAsInt->getType()},
          {floatAsInt, floatAsInt, ConstantInt::get(floatAsInt->getType(), 3)}),
      REFTYPE, "maskedFloat");
  CreateExpect(builder, isGoodExponent, MakeIntLike(isGoodExponent, 1));
  builder->CreateCondBr(isGoodExponent, outBlock, checkPosZeroBlock,
                        GetLikelyFirstBranchMetadata());

  builder->SetInsertPoint(checkPosZeroBlock);
  auto isPosZero =
      builder->CreateFCmpOEQ(floatval, ConstantFP::get(FLOATTYPE, 0.0));
  CreateExpect(builder, isPosZero, MakeIntLike(isPosZero, 1));
  builder->CreateCondBr(isPosZero, outBlock, checkNegZeroFloatBlock,
                        GetLikelyFirstBranchMetadata());

  builder->SetInsertPoint(checkNegZeroFloatBlock);
  auto isNegZero =
      builder->CreateFCmpOEQ(floatval, ConstantFP::getNegativeZero(FLOATTYPE));
  CreateExpect(builder, isNegZero, MakeIntLike(isNegZero, 1));
  builder->CreateCondBr(isNegZero, outBlock, boxFloatBlock,
                        GetLikelyFirstBranchMetadata());

  builder->SetInsertPoint(boxFloatBlock);
  if (NomCastStats) {
    builder->CreateCall(
        GetIncFloatBoxesFunction(
            *builder->GetInsertBlock()->getParent()->getParent()),
        {});
  }
  auto allocfun = GetNewAlloc(fun->getParent());
  auto boxedFloat = builder->CreateCall(
      allocfun, {MakeInt<size_t>(1), MakeInt<size_t>(0)}, "boxedFloat");
  RefValueHeader::GenerateWriteVTablePointer(
      builder, boxedFloat,
      NomFloatClass::GetInstance()->GetLLVMElement(*fun->getParent()));
  ObjectHeader::WriteField(
      builder, boxedFloat, MakeInt32(0),
      builder->CreateIntToPtr(builder->CreateBitCast(floatval, INTTYPE),
                              REFTYPE),
      false);
#ifdef FLOATDEBUG
  builder->CreateCall(boxFloatDbgFun, {floatval, builder->CreatePointerCast(
                                                     boxedFloat, POINTERTYPE)});
#endif
  ...
  floatPHI->addIncoming(NomFloatObjects::GetPosZero(*fun->getParent()),
                        checkPosZeroBlock);
  floatPHI->addIncoming(NomFloatObjects::GetNegZero(*fun->getParent()),
                        checkNegZeroFloatBlock);
...
llvm::Value *UnpackFloat(NomBuilder &builder, NomValue value) {
  if (NomCastStats) {
    builder->CreateCall(
        GetIncFloatUnpacksFunction(
            *builder->GetInsertBlock()->getParent()->getParent()),
        {});
  }
  Function *fun = builder->GetInsertBlock()->getParent();
  BasicBlock *refValueBlock = nullptr, *maskedFloatBlock = nullptr,
             *intBlock = nullptr, *primitiveFloatBlock = nullptr;
  Value *primitiveFloatVal = nullptr;

  int phiCases = RefValueHeader::GenerateRefOrPrimitiveValueSwitch(
      builder, value, &refValueBlock, &intBlock, &maskedFloatBlock, false,
      nullptr, nullptr, &primitiveFloatBlock, &primitiveFloatVal, nullptr,
      nullptr, 30, 1, 100, 1);

  BasicBlock *mergeBlock =
      BasicBlock::Create(LLVMCONTEXT, "floatUnpackMerge", fun);
  builder->SetInsertPoint(mergeBlock);
  ...
  if (refValueBlock != nullptr) {
    ...
    if (!value.GetNomType()->IsSubtype(GetFloatClassType(), false)) {
      static const char *const errormsg =
          "Tried to unpack non-float value as a Float!";
      BasicBlock *unboxErrorBlock =
          RTOutput_Fail::GenerateFailOutputBlock(builder, errormsg);
      BasicBlock *unboxValidBlock =
          BasicBlock::Create(LLVMCONTEXT, "floatUnboxValid", fun);
      auto vtableptr = builder->CreatePtrToInt(
          ObjectHeader::GenerateReadVTablePointer(builder, value),
          numtype(intptr_t));
      auto isFloat = builder->CreateICmpEQ(
          vtableptr,
          ConstantExpr::getPtrToInt(
              NomFloatClass::GetInstance()->GetLLVMElement(*fun->getParent()),
              numtype(intptr_t)));
    ...
    if (NomCastStats) {
      builder->CreateCall(
          GetIncFloatUnboxesFunction(
              *builder->GetInsertBlock()->getParent()->getParent()),
          {});
    }
    auto boxField = builder->CreateBitCast(
  ...
  if (intBlock != nullptr) {
    RTOutput_Fail::MakeBlockFailOutputBlock(
        builder, "Tried to unpack packed Int value as a Float!", intBlock);
  }
...
NomValue UnpackMaskedFloat(NomBuilder &builder, llvm::Value *refAsInt) {
  ...
  return builder->CreateBitCast(
      builder->CreateIntrinsic(
          Intrinsic::fshr, {INTTYPE},
          {refAsInt, refAsInt, ConstantInt::get(refAsInt->getType(), 3)}),
      FLOATTYPE, "unmaskedFloat");
}
...
#ifdef FLOATDEBUG
extern "C" DLLEXPORT void RT_NOM_DBG_PrintUnpackFloatDebug(double f,
                                                           int64_t i) {
  cout << "Unpacking " << i << " as float: " << f << "\n";
}

extern "C" DLLEXPORT void RT_NOM_DBG_PrintUnboxFloatDebug(double f, void *i) {
  cout << "Unboxing " << std::hex << (intptr_t)i << std::dec
       << " as float: " << f << "\n";
}
