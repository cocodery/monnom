...
struct stringref_equalto {
  constexpr bool operator()(const std::string *lhs,
...
template <typename T>
using StringDict = std::unordered_map<const std::string *, T, stringref_hash,
                                      stringref_equalto>;
...
template <typename T>
using NomStringDict =
    std::unordered_map<const NomString *, T, nomstringref_hash,
                       nomstringref_equalto>;
} // namespace Runtime
} // namespace Nom