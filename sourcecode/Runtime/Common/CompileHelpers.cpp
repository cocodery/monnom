...
namespace Nom {
namespace Runtime {
CallInst *GenerateFunctionCall(NomBuilder &builder, Module &mod, Value *fun,
                               FunctionType *ftype, ArrayRef<Value *> &args,
                               bool fastcall) {
  ...
  CallInst *ci = builder->CreateCall(
      ftype, fun, llvm::ArrayRef<llvm::Value *>(pargs, argsize));
  if (fastcall) {
...
CallInst *GenerateFunctionCall(NomBuilder &builder, Module &mod, Function *fun,
                               ArrayRef<Value *> &args, bool fastcall) {
  size_t argsize = args.size();
  Value **pargs = makealloca(Value *, argsize);
  ...
  CallInst *ci =
      builder->CreateCall(fun, llvm::ArrayRef<llvm::Value *>(pargs, argsize));
  if (fastcall) {
    ci->setCallingConv(NOMCC);
  }
  return ci;
...
llvm::Value *GenerateIsNull(NomBuilder &builder, llvm::Value *val) {
  ...
  return builder->CreateICmpEQ(
      builder->CreatePtrToInt(val, numtype(intptr_t)),
...
llvm::StoreInst *MakeStore(NomBuilder &builder, llvm::Module &mod,
  ...
  {
          pval = builder->CreateIntToPtr(builder->CreateBitCast(pval, INTTYPE),
  POINTERTYPE);
  }
  builder->CreateCall(mod.getFunction("RT_NOM_PRINT_STORE"), {
  {builder->CreatePointerCast(pval,POINTERTYPE),builder->CreatePointerCast(ptr,POINTERTYPE)}
  });*/
  llvm::StoreInst *store = builder->CreateStore(val, ptr);
  store->setAlignment(
      llvm::Align(GetNomJITDataLayout().getTypeSizeInBits(val->getType())));
  store->setAtomic(ordering);
  return store;
}

llvm::StoreInst *MakeStore(NomBuilder &builder, llvm::Value *val,
                           llvm::Value *ptr, llvm::AtomicOrdering ordering) {
  return MakeStore(builder,
                   *builder->GetInsertBlock()->getParent()->getParent(), val,
                   ptr, ordering);
}
...
llvm::StoreInst *MakeStore(NomBuilder &builder, llvm::Value *val,
  ...
  llvm::Value *actualpointer = builder->CreateGEP(
      ptr, llvm::ArrayRef<llvm::Value *>(indexbuf, indexcount + 1));
  llvm::StoreInst *store = builder->CreateStore(val, actualpointer);
  store->setAlignment(llvm::Align(GetNomJITDataLayout().getTypeSizeInBits(
      ((PointerType *)actualpointer->getType())->getElementType())));
  store->setAtomic(ordering);
  return store;
}

llvm::StoreInst *MakeStore(NomBuilder &builder, llvm::Value *val,
                           llvm::Value *ptr, llvm::PointerType *asType,
                           llvm::ArrayRef<llvm::Value *> indices,
                           llvm::AtomicOrdering ordering) {
  return MakeStore(builder, val, builder->CreatePointerCast(ptr, asType),
                   indices, ordering);
}
...
llvm::StoreInst *MakeStore(NomBuilder &builder, llvm::Value *val,
                           ...
                           llvm::AtomicOrdering ordering) {
  llvm::Value *actualpointer = builder->CreateGEP(
      ptr, {MakeInt32(0), builder->CreateZExtOrTrunc(index, inttype(32))});
  llvm::StoreInst *store = builder->CreateStore(val, actualpointer);
  store->setAlignment(llvm::Align(GetNomJITDataLayout().getTypeSizeInBits(
      ((PointerType *)actualpointer->getType())->getElementType())));
  store->setAtomic(ordering);
...
llvm::StoreInst *MakeStore(NomBuilder &builder, llvm::Value *val,
                           llvm::Value *ptr, llvm::PointerType *asType,
                           llvm::Value *index, llvm::AtomicOrdering ordering) {
  return MakeStore(builder, val, builder->CreatePointerCast(ptr, asType), index,
                   ordering);
}

llvm::StoreInst *MakeInvariantStore(NomBuilder &builder, llvm::Module &mod,
                                    llvm::Value *val, llvm::Value *ptr,
                                    llvm::AtomicOrdering ordering) {
...
llvm::StoreInst *MakeInvariantStore(NomBuilder &builder, llvm::Value *val,
                                    llvm::Value *ptr,
                                    llvm::AtomicOrdering ordering) {
...
llvm::LoadInst *MakeLoad(NomBuilder &builder, llvm::Module &mod,
                         llvm::Value *ptr, llvm::AtomicOrdering ordering) {
  ...
  builder->CreateCall(mod.getFunction("RT_NOM_PRINT_LOAD"), {
  {builder->CreatePointerCast(pval,POINTERTYPE),
  builder->CreatePointerCast(ptr,POINTERTYPE)} });*/
  // load->setAtomic(ordering);
  load->setAlignment(llvm::Align(GetNomJITDataLayout().getTypeSizeInBits(
      ((PointerType *)ptr->getType())->getElementType())));
  load->setAtomic(ordering);
  return load;
}

llvm::LoadInst *MakeLoad(NomBuilder &builder, llvm::Value *ptr,
                         llvm::Twine name, llvm::AtomicOrdering ordering) {
  auto li =
      MakeLoad(builder, *builder->GetInsertBlock()->getParent()->getParent(),
               ptr, ordering);
  ;
  li->setName(name);
  return li;
...
llvm::LoadInst *MakeLoad(NomBuilder &builder, llvm::Value *ptr,
                         llvm::PointerType *asType,
                         llvm::ArrayRef<llvm::Value *> indices,
                         llvm::Twine name, llvm::AtomicOrdering ordering) {
  return MakeLoad(builder, builder->CreatePointerCast(ptr, asType), indices,
                  name, ordering);
}

// Adds a zero index in front of the given index
llvm::LoadInst *MakeLoad(NomBuilder &builder, llvm::Value *ptr,
                         llvm::Value *index, llvm::Twine name,
                         llvm::AtomicOrdering ordering) {
  llvm::Value *actualpointer = builder->CreateGEP(
      ptr, {MakeInt32(0), builder->CreateZExtOrTrunc(index, inttype(32))});
  llvm::LoadInst *load = builder->CreateLoad(actualpointer, name);
...
llvm::LoadInst *MakeLoad(NomBuilder &builder, llvm::Value *ptr,
                         llvm::PointerType *asType, llvm::Value *index,
                         llvm::Twine name, llvm::AtomicOrdering ordering) {
  return MakeLoad(builder, builder->CreatePointerCast(ptr, asType), index, name,
                  ordering);
}

llvm::LoadInst *MakeInvariantLoad(NomBuilder &builder, llvm::Module &mod,
                                  llvm::Value *ptr,
                                  llvm::AtomicOrdering ordering) {
...
llvm::LoadInst *MakeInvariantLoad(NomBuilder &builder, llvm::Value *ptr,
                                  llvm::Twine name,
                                  llvm::AtomicOrdering ordering) {
...
ReadFieldFunction GetReadFieldFunction() {
  static ReadFieldFunction fun =
      (ReadFieldFunction)(intptr_t)(NomJIT::Instance()
                                        .lookup("RT_NOM_READFIELD")
                                        ->getAddress());
  return fun;
}
WriteFieldFunction GetWriteFieldFunction() {
  static WriteFieldFunction fun =
      (WriteFieldFunction)(intptr_t)(NomJIT::Instance()
                                         .lookup("RT_NOM_WRITEFIELD")
                                         ->getAddress());
  return fun;
}
FieldAddrFunction GetFieldAddrFunction() {
  static FieldAddrFunction fun =
      (FieldAddrFunction)(intptr_t)(NomJIT::Instance()
                                        .lookup("RT_NOM_GETFIELDADDR")
                                        ->getAddress());
  return fun;
}
ReadFieldFunction GetReadFieldFunction_ForInvoke() {
  static ReadFieldFunction fun =
      (ReadFieldFunction)(intptr_t)(NomJIT::Instance()
                                        .lookup("RT_NOM_READFIELD_FORINVOKABLE")
                                        ->getAddress());
  return fun;
}
WriteFieldFunction GetWriteFieldFunction_ForInvoke() {
  static WriteFieldFunction fun =
      (WriteFieldFunction)(intptr_t)(NomJIT::Instance()
                                         .lookup(
                                             "RT_NOM_WRITEFIELD_FORINVOKABLE")
                                         ->getAddress());
  return fun;
}
ReadTypeArgFunction GetReadTypeArgFunction() {
  static ReadTypeArgFunction fun =
      (ReadTypeArgFunction)(intptr_t)(NomJIT::Instance()
                                          .lookup("RT_NOM_READTYPEARG")
                                          ->getAddress());
  return fun;
}
WriteTypeArgFunction GetWriteTypeArgFunction() {
  static WriteTypeArgFunction fun =
      (WriteTypeArgFunction)(intptr_t)(NomJIT::Instance()
                                           .lookup("RT_NOM_WRITETYPEARG")
                                           ->getAddress());
  return fun;
}
WriteVTableFunction GetWriteVTableFunction() {
  static WriteVTableFunction fun =
      (WriteVTableFunction)(intptr_t)(NomJIT::Instance()
                                          .lookup("RT_NOM_WRITEVTABLE")
                                          ->getAddress());
  return fun;
}
...
void *GetBooleanTrue() {
  static void *bval = (void *)(*(
      (intptr_t *)(NomJIT::Instance().lookup("RT_NOM_TRUE")->getAddress())));
  return bval;
}
void *GetBooleanFalse() {
  static void *bval = (void *)(*(
      (intptr_t *)(NomJIT::Instance().lookup("RT_NOM_FALSE")->getAddress())));
  return bval;
}
void *GetVoidObj() {
  static void *voidobj =
      (void *)((intptr_t)(Nom::Runtime::NomJIT::Instance().getSymbolAddress(
          "RT_NOM_VOIDOBJ")));
  return voidobj;
}
...
llvm::Value *EnsureIntegerSize(NomBuilder &builder, llvm::Value *val,
                               int bitwidth) {
...
llvm::Value *UnmaskDictionaryValue(NomBuilder &builder, llvm::Value *val) {
  ...
  BasicBlock *ref =
      BasicBlock::Create(LLVMCONTEXT, "unmaskRef", incoming->getParent());
  BasicBlock *continueBlock =
      BasicBlock::Create(LLVMCONTEXT, "continue", incoming->getParent());
  auto tag = builder->CreateTrunc(
      builder->CreatePtrToInt(val, numtype(intptr_t)), inttype(2), "tag");
  builder->CreateCondBr(builder->CreateICmpEQ(tag, MakeUInt(2, 0)), ref,
                        continueBlock);

  builder->SetInsertPoint(ref);
  auto unmaskedRef = builder->CreateIntToPtr(
      builder->CreateAnd(builder->CreatePtrToInt(val, numtype(intptr_t)),
                         GetMask(64, 0, 3)),
      REFTYPE);
...
llvm::Constant *MakeLLVMStruct(llvm::ArrayRef<llvm::Constant *> args) {
  ...
  return llvm::ConstantStruct::get(
      llvm::StructType::get(
          LLVMCONTEXT, llvm::ArrayRef<llvm::Type *>(argtypes, args.size())),
      args);
}
llvm::ConstantInt *MakeUInt(size_t size, uint64_t val) {
  return llvm::ConstantInt::get(llvm::IntegerType::get(LLVMCONTEXT, size), val,
                                false);
}
...
llvm::ConstantInt *MakeInt(size_t size, int64_t val) {
  return llvm::ConstantInt::get(llvm::IntegerType::get(LLVMCONTEXT, size),
                                static_cast<uint64_t>(val), true);
}
llvm::ConstantInt *MakeInt32(int32_t val) {
  return llvm::ConstantInt::get(llvm::IntegerType::get(LLVMCONTEXT, 32),
                                bit_cast<uint32_t, int32_t>(val), true);
}
llvm::ConstantInt *MakeInt32(uint32_t val) {
  return llvm::ConstantInt::get(llvm::IntegerType::get(LLVMCONTEXT, 32), val,
                                true);
}
llvm::ConstantInt *MakeInt32(uint64_t val) { return MakeInt32((uint32_t)val); }
llvm::ConstantInt *MakeInt32(int64_t val) { return MakeInt32((int32_t)val); }
...
llvm::Constant *GetLLVMRef(const void *const ptr) {
  ...
  return llvm::ConstantExpr::getIntToPtr(
      llvm::ConstantInt::get(llvm::Type::getIntNTy(LLVMCONTEXT, bitsin(void *)),
                             reinterpret_cast<uint64_t>(ptr), false),
      REFTYPE);
}
...
llvm::Constant *GetMask(int length, int leadingZeroes, int trailingZeroes) {
  auto ret = ConstantExpr::getXor(
      llvm::ConstantInt::getAllOnesValue(
          IntegerType::get(LLVMCONTEXT, length - leadingZeroes)),
      ConstantExpr::getZExt(
          llvm::ConstantInt::getAllOnesValue(
              IntegerType::get(LLVMCONTEXT, trailingZeroes)),
          IntegerType::get(LLVMCONTEXT, length - leadingZeroes)));
  if (leadingZeroes > 0) {
...
llvm::Value *CreatePointerEq(NomBuilder &builder, llvm::Value *left,
                             llvm::Value *right, const llvm::Twine &name) {
  return builder->CreateICmpEQ(
      builder->CreatePtrToInt(left, numtype(intptr_t)),
      builder->CreatePtrToInt(right, numtype(intptr_t)), name);
}
void CreateExpect(NomBuilder &builder, llvm::Value *value,
                  llvm::Value *expected) {
  builder->CreateIntrinsic(Intrinsic::expect, {value->getType()},
                           {value, expected});
}
} // namespace Runtime
} // namespace Nom