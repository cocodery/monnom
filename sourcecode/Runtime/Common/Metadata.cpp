...
MDNode *getStructDescriptorInvariantNode() { return getGeneralInvariantNode(); }
...
GetBranchWeightsForBlocks(llvm::ArrayRef<llvm::BasicBlock *> blocks) {
  ...
  return MDNode::get(LLVMCONTEXT,
                     ArrayRef<Metadata *>(args, blocks.size() + 1));
}
llvm::MDNode *GetBranchWeights(llvm::ArrayRef<uint64_t> weights) {
  ...
  return MDNode::get(LLVMCONTEXT,
                     ArrayRef<Metadata *>(args, weights.size() + 1));
}
} // namespace Runtime
} // namespace Nom